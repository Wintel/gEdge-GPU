...
public:
  ...
  Vector3 normalized() { return Vector3(clone() / mag()); }
  ...
  float innerProduct(Vector3 p) { return x * p.x + y * p.y + z * p.z; }
};
...
public:
  ...
  Vector2 normalized() { return Vector2(clone() / mag()); }

  float dot(Vector2 p) { return x * p.x + y * p.y; }
