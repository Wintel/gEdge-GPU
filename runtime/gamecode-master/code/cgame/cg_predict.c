...
static void CG_TouchItem(centity_t *cent) {
  ...
  if (cgs.gametype == GT_CTF || cgs.gametype == GT_CTF_ELIMINATION ||
    ...
    if (cg.predictedPlayerState.persistant[PERS_TEAM] == TEAM_RED &&
        item->giType == IT_TEAM && item->giTag == PW_BLUEFLAG &&
        (!(cgs.elimflags & EF_ONEWAY) || cgs.attackingTeam == TEAM_RED))
      canBePicked = qtrue;
    if (cg.predictedPlayerState.persistant[PERS_TEAM] == TEAM_BLUE &&
        item->giType == IT_TEAM && item->giTag == PW_REDFLAG &&
        (!(cgs.elimflags & EF_ONEWAY) || cgs.attackingTeam == TEAM_BLUE))
      canBePicked = qtrue;
  }

  // Currently we don't predict anything in Double Domination because it looks
  // like we take a flag
  if (cgs.gametype == GT_DOUBLE_D) {
    if (cgs.redflag == TEAM_NONE)
      return; // Can never pick if just one flag is NONE (because then the other
              // is too)
    if (item->giTag == PW_REDFLAG) { // at point A
      if (cgs.redflag !=
          cg.predictedPlayerState.persistant[PERS_TEAM]) // not already taken
        trap_S_StartLocalSound(cgs.media.hitSound, CHAN_ANNOUNCER);
      return;
    }
    if (item->giTag == PW_BLUEFLAG) { // at point B
      if (cgs.blueflag !=
          cg.predictedPlayerState.persistant[PERS_TEAM]) // already taken
        trap_S_StartLocalSound(cgs.media.hitSound, CHAN_ANNOUNCER);
      return;
    }
  }
  ...
  if (canBePicked) {
    ...
    // if its a weapon, give them some predicted ammo so the autoswitch will
    // work
...
static void CG_TouchTriggerPrediction(void) {
  ...
  if (cg.predictedPlayerState.jumppad_frame !=
      cg.predictedPlayerState.pmove_framecount) {
    ...
    // if it's the same as the last one
      ...
      for (i = cg.stateHead; i != cg.stateTail;
        ...
        if (cg.savedPmoveStates[i].commandTime ==
          ...
          int errorcode = IsUnacceptableError(&cg.predictedPlayerState,
    ...
    if (cg.predictedPlayerState.commandTime == oldPlayerState.commandTime) {
      ...
      if (cg.thisFrameTeleport) {
        ...
        CG_AdjustPositionForMover(
            ...
            adjusted, cg.predictedPlayerState.viewangles, new_angles);
