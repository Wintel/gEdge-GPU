...
static void CG_OldRocketTrail(centity_t *ent, const weaponInfo_t *wi) {
  ...
  for (; t <= ent->trailTime; t += step) {
    ...
    smoke = CG_SmokePuff(lastPos, up, wi->trailRadius, 1, 1, 1, 0.33f,
                         wi->wiTrailTime, t, 0, 0, cgs.media.smokePuffShader);
    // use the optimized local entity add
    smoke->leType = LE_SCALE_FADE;
  }
}
...
static void CG_LeiPlasmaTrail(centity_t *ent, const weaponInfo_t *wi) {
  ...
  for (; t <= ent->trailTime; t += step) {
    ...
    smoke = CG_SmokePuff(lastPos, up, 27, 1, 1, 1, 0.9f, wi->wiTrailTime, t, 0,
                         0, cgs.media.lsmkShader1);
    // use the optimized local entity add
    smoke->leType = LE_SCALE_FADE;
...
static void CG_NailTrail(centity_t *ent, const weaponInfo_t *wi) {
  ...
  for (; t <= ent->trailTime; t += step) {
    ...
    smoke = CG_SmokePuff(lastPos, up, wi->trailRadius, 1, 1, 1, 0.33f,
                         wi->wiTrailTime, t, 0, 0, cgs.media.nailPuffShader);
    // use the optimized local entity add
    smoke->leType = LE_SCALE_FADE;
  }
}
...
void CG_RegisterWeapon(int weaponNum) {
  ...
  if (!weaponInfo->flashModel) {
    weaponInfo->flashModel = trap_R_RegisterModel(
        "models/weapons2/shotgun/shotgun_flash.md3"); // default flash
  }
  ...
  if (!weaponInfo->flashModel_type1)
    weaponInfo->flashModel_type1 =
        trap_R_RegisterModel("models/muzzle/flash1.md3");
  if (!weaponInfo->flashModel_type2)
    weaponInfo->flashModel_type2 =
        trap_R_RegisterModel("models/muzzle/flash2.md3");
  if (!weaponInfo->flashModel_type2a)
    weaponInfo->flashModel_type2a =
        trap_R_RegisterModel("models/muzzle/flash2a.md3");
  if (!weaponInfo->flashModel_type3)
    weaponInfo->flashModel_type3 =
        trap_R_RegisterModel("models/muzzle/flash3.md3");
  if (!weaponInfo->flashModel_type4)
    weaponInfo->flashModel_type4 =
        trap_R_RegisterModel("models/muzzle/flash4.md3");
  if (!weaponInfo->flashModel_type5)
    weaponInfo->flashModel_type5 =
        trap_R_RegisterModel("models/muzzle/flash5.md3");
  if (!weaponInfo->flashModel_type5a)
    weaponInfo->flashModel_type5a =
        trap_R_RegisterModel("models/muzzle/flash5a.md3");
  ...
  case WP_GAUNTLET:
    ...
    weaponInfo->firingSound =
        trap_S_RegisterSound("sound/weapons/melee/fstrun.wav", qfalse);
  ...
  case WP_LIGHTNING:
    ...
    weaponInfo->readySound =
        trap_S_RegisterSound("sound/weapons/melee/fsthum.wav", qfalse);
    weaponInfo->firingSound =
        trap_S_RegisterSound("sound/weapons/lightning/lg_hum.wav", qfalse);

    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/lightning/lg_fire.wav", qfalse);
    cgs.media.lightningShader = trap_R_RegisterShader("lightningBoltNew");
    cgs.media.lightningExplosionModel =
        trap_R_RegisterModel("models/weaphits/crackle.md3");
    cgs.media.sfx_lghit1 =
        trap_S_RegisterSound("sound/weapons/lightning/lg_hit.wav", qfalse);
    cgs.media.sfx_lghit2 =
        trap_S_RegisterSound("sound/weapons/lightning/lg_hit2.wav", qfalse);
    cgs.media.sfx_lghit3 =
        trap_S_RegisterSound("sound/weapons/lightning/lg_hit3.wav", qfalse);
    weaponInfo->lfx = 0; //  no effect
    break;
  ...
  case WP_GRAPPLING_HOOK:
    ...
    weaponInfo->missileModel =
        trap_R_RegisterModel("models/ammo/hook/hook.md3");
    ...
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/grapple/grapfire.wav", qfalse);
    weaponInfo->missileSound =
        trap_S_RegisterSound("sound/weapons/grapple/grappull.wav", qfalse);
    // cgs.media.lightningShader = trap_R_RegisterShader( "lightningBoltNew");
    weaponInfo->lfx = 0; //  no effect
    break;
  ...
  case WP_CHAINGUN:
    weaponInfo->firingSound =
        trap_S_RegisterSound("sound/weapons/vulcan/wvulfire.wav", qfalse);
    weaponInfo->loopFireSound = qtrue;
    MAKERGB(weaponInfo->flashDlightColor, 1, 1, 0);
    // leilei -testing a looping firing sound instead of lots of flash sounds
    // for performance reasons
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/vulcan/vulcanf1b.wav", qfalse);
    weaponInfo->flashSound[1] =
        trap_S_RegisterSound("sound/weapons/vulcan/vulcanf2b.wav", qfalse);
    weaponInfo->flashSound[2] =
        trap_S_RegisterSound("sound/weapons/vulcan/vulcanf3b.wav", qfalse);
    weaponInfo->flashSound[3] =
        trap_S_RegisterSound("sound/weapons/vulcan/vulcanf4b.wav", qfalse);
  ...
  case WP_MACHINEGUN:
    MAKERGB(weaponInfo->flashDlightColor, 1, 1, 0);
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/machinegun/machgf1b.wav", qfalse);
    weaponInfo->flashSound[1] =
        trap_S_RegisterSound("sound/weapons/machinegun/machgf2b.wav", qfalse);
    weaponInfo->flashSound[2] =
        trap_S_RegisterSound("sound/weapons/machinegun/machgf3b.wav", qfalse);
    weaponInfo->flashSound[3] =
        trap_S_RegisterSound("sound/weapons/machinegun/machgf4b.wav", qfalse);
    weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
    cgs.media.bulletExplosionShader = trap_R_RegisterShader("bulletExplosion");
    weaponInfo->lfx = 62;
    break;
  ...
  case WP_ROCKET_LAUNCHER:
    weaponInfo->missileModel =
        trap_R_RegisterModel("models/ammo/rocket/rocket.md3");
    weaponInfo->missileSound =
        trap_S_RegisterSound("sound/weapons/rocket/rockfly.wav", qfalse);
    ...
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/rocket/rocklf1a.wav", qfalse);
  ...
  case WP_PROX_LAUNCHER:
    weaponInfo->missileModel =
        trap_R_RegisterModel("models/weaphits/proxmine.md3");
    weaponInfo->missileTrailFunc = CG_GrenadeTrail;
    weaponInfo->wiTrailTime = 700;
    weaponInfo->trailRadius = 32;
    MAKERGB(weaponInfo->flashDlightColor, 1, 0.70f, 0);
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/proxmine/wstbfire.wav", qfalse);
  ...
  case WP_GRENADE_LAUNCHER:
    ...
    MAKERGB(weaponInfo->flashDlightColor, 1, 0.70f, 0);
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/grenade/grenlf1a.wav", qfalse);
    cgs.media.grenadeExplosionShader =
        trap_R_RegisterShader("grenadeExplosion");
    weaponInfo->lfx = 64;
    break;
  ...
  case WP_PLASMAGUN:
    ...
    MAKERGB(weaponInfo->flashDlightColor, 0.6f, 0.6f, 1.0f);
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/plasma/hyprbf1a.wav", qfalse);
  ...
  case WP_RAILGUN:
    weaponInfo->readySound =
        trap_S_RegisterSound("sound/weapons/railgun/rg_hum.wav", qfalse);
  ...
  case WP_BFG:
    weaponInfo->readySound =
        trap_S_RegisterSound("sound/weapons/bfg/bfg_hum.wav", qfalse);
    MAKERGB(weaponInfo->flashDlightColor, 1, 0.7f, 1);
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/bfg/bfg_fire.wav", qfalse);
...
void CG_RegisterItemVisuals(int itemNum) {
  ...
  if (itemNum < 0 || itemNum >= bg_numItems) {
    CG_Error("CG_RegisterItemVisuals: itemNum %d out of range [0-%d]", itemNum,
             bg_numItems - 1);
  }
...
static void CG_CalculateWeaponPosition(vec3_t origin, vec3_t angles) {
  ...
  if (cg_bobmodel.integer) {
    ...
    if (cg_bobmodel.integer == 1) {
      ...
      bob = cos(scale * 0.07 * cg.bobfracsin * 0.05) -
            cos(scale * 0.07 * cg.bobfracsin * 0.1);
    ...
    if (cg_bobmodel.integer == 3) {
      ...
      bob = cos(scale * 0.07 * cg.bobfraccos * 0.05) -
            sin(scale * 0.07 * cg.bobfracsin * 0.1);
      VectorMA(origin, bob, up, origin);
    }
...
static void CG_LightningBolt(centity_t *cent, vec3_t origin) {
  ...
  //  if the entity is us, unlagged is on server-side, and we've got it on for
  //  the lightning gun
  if ((cent->currentState.number == cg.predictedPlayerState.clientNum) &&
    ...
    if ((cent->currentState.number == cg.predictedPlayerState.clientNum) &&
      ...
      for (i = 0; i < 3; i++) {
        float a = cent->lerpAngles[i] -
                  viewangles[i]; // unlagged: was cg.refdefViewAngles[i];
        if (a > 180) {
          a -= 360;
        }
        if (a < -180) {
        ...
        angle[i] = viewangles[i] /*unlagged: was cg.refdefViewAngles[i]*/ +
                   a * (1.0 - cg_trueLightning.value);
        if (angle[i] < 0) {
          angle[i] += 360;
        }
        if (angle[i] > 360) {
...
static void MuzzleFlashStyleQ1(refEntity_t *parent, playerState_t *ps,
                               centity_t *cent, weapon_t weaponNum,
                               centity_t *nonPredictedCent, refEntity_t *gun,
                               weaponInfo_t *weapon) {
  vec3_t angles;
  refEntity_t flash;
  ...
  if (ps || cg.renderingThirdPerson ||
    ...
    // add lightning bolt
    if (cg.time - cent->muzzleFlashTime < 100 &&
        !cent->pe.railgunFlash) // leilei - don't prolong the lightning
    ...
    if (cg.time - cent->muzzleFlashTime < 100) // flash for 0.1 sec
      if (weapon->flashDlightColor[0] || weapon->flashDlightColor[1] ||
        ...
        trap_R_AddLightToScene(parent->origin, 300, 0.95, 0.95,
                               0.95); // mono lighting
      }
  }
}
...
void CG_AddPlayerWeapon(refEntity_t *parent, playerState_t *ps, centity_t *cent,
  ...
  if (!ps) {
    ...
    if ((cent->currentState.eFlags & EF_FIRING) && weapon->firingSound) {
      // lightning gun and guantlet make a different sound when fire is held
      // down
  ...
  if (cg_muzzleflashStyle.integer == 2) {
    MuzzleFlashStyleQ1(parent, ps, cent, weaponNum, nonPredictedCent, &gun,
                       weapon);
    return; // Override third person flash so we can have a different model and
            // lighting origin
  } else if (cg_muzzleflashStyle.integer == 3) // "fake '99"
  {
    ...
    if (cent->currentState.number == cg.snap->ps.clientNum &&
        !cg.renderingThirdPerson) // use 3rd person flash model
    ...
    if (ps || cg.renderingThirdPerson ||
      ...
      if (weapon->flashDlightColor[0] || weapon->flashDlightColor[1] ||
        ...
        trap_R_AddLightToScene(
            parent->origin, 150, weapon->flashDlightColor[0],
            weapon->flashDlightColor[1],
            weapon->flashDlightColor[2]); // also comes from player center
      }
    }
    return; // Override third person flash so we can have a different model and
            // lighting origin
  } else if (cg_muzzleflashStyle.integer ==
             4) // "64". alpha muzzle sprites only, is a shader sequence
  {
    if ((weaponNum == WP_LIGHTNING || weaponNum == WP_GAUNTLET ||
         weaponNum == WP_GRAPPLING_HOOK) &&
        (nonPredictedCent->currentState.eFlags & EF_FIRING)) {
    ...
    return; // Override third person flash so we can have a different model and
            // lighting origin
             ...
             5) // anime. the idea is similar to fake2003, but scaling differs
  ...
  {             // and allow artist creativity to define how it should fade away
    ...
    if (fadeout < 0)
      return; // if we faded, we don't draw. ok
    ...
    if (ps || cg.renderingThirdPerson ||
      ...
      if (weapon->flashDlightColor[0] || weapon->flashDlightColor[1] ||
          weapon->flashDlightColor[2]) {
        trap_R_AddLightToScene(flash.origin, 500 * fadeout,
                               weapon->flashDlightColor[0] * (fadeout * 4),
                               weapon->flashDlightColor[1] * (fadeout * 4),
                               weapon->flashDlightColor[2] * (fadeout * 4));
      }
    }
    return;
  ...
  } else if (cg_muzzleflashStyle.integer == 6) // "fake2003"
  {
    ...
    if (cg.time - cent->muzzleFlashTime <
        5) // hopefully no one will notice the jitter at this rate
    ...
    if (ps || cg.renderingThirdPerson ||
      ...
      if (weapon->flashDlightColor[0] || weapon->flashDlightColor[1] ||
          weapon->flashDlightColor[2]) {
        trap_R_AddLightToScene(parent->origin, 200,
                               weapon->flashDlightColor[0] * fadeout,
                               weapon->flashDlightColor[1] * fadeout,
                               weapon->flashDlightColor[2] * fadeout);
      }
    }
    return;
  }
  ...
  if (ps || cg.renderingThirdPerson ||
    ...
    if (weapon->flashDlightColor[0] || weapon->flashDlightColor[1] ||
        weapon->flashDlightColor[2]) {
      trap_R_AddLightToScene(
          flash.origin, 300 + (rand() & 31), weapon->flashDlightColor[0],
...
cg_brassTime.integer / 4 ) * random();
        ...
        xoffset[0] = offset[0] * v[0][0] + offset[1] * v[1][0] + offset[2] *
v[2][0]; xoffset[1] = offset[0] * v[0][1] + offset[1] * v[1][1] + offset[2] *
v[2][1]; xoffset[2] = offset[0] * v[0][2] + offset[1] * v[1][2] + offset[2] *
v[2][2]; VectorAdd( cent->lerpOrigin, xoffset, re->origin );
...
velocity[2] * v[2][0]; xvelocity[1] = velocity[0] * v[0][1] + velocity[1] *
v[1][1] + velocity[2] * v[2][1]; xvelocity[2] = velocity[0] * v[0][2] +
velocity[1] * v[1][2] + velocity[2] * v[2][2]; VectorScale( xvelocity,
waterScale, le->pos.trDelta );
...
void CG_MissileHitWall(int weapon, int clientNum, vec3_t origin, vec3_t dir,
  ...
  alphaFade =
      (mark ==
       cgs.media.energyMarkShader); // plasma fades alpha, all others fade color
  if (weapon == WP_RAILGUN) {
    float *color;
    ...
    CG_ImpactMark(mark, origin, dir, random() * 360, color[0], color[1],
                  color[2], 1, alphaFade, radius, qfalse);
  } else {
    CG_ImpactMark(mark, origin, dir, random() * 360, 1, 1, 1, 1, alphaFade,
                  radius, qfalse);
  }
}
...
void CG_ShotgunPattern(vec3_t origin, vec3_t origin2, int seed,
                       int otherEntNum) {
  int i;
  float r, u;
  vec3_t end;
