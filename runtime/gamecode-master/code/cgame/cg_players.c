...
Read eyes definitions.  Maybe this should be done engine-side for mod
compatiblity? :S
======================
 */
...
==========================
 */
static qboolean CG_FindClientModelFile(char *filename, int length,
                                       clientInfo_t *ci, const char *teamName,
                                       const char *modelName,
                                       const char *skinName, const char *base,
                                       const char *ext) {
  ...
  while (1) {
    for (i = 0; i < 2; i++) {
      if (i == 0 && teamName && *teamName) {
        ...
        Com_sprintf(filename, length, "models/players/%s%s/%s%s_%s_%s.%s",
                    charactersFolder, modelName, teamName, base, skinName, team,
                    ext);
      } else {
        //								"models/players/characters/sergei/lower_lily_red.skin"
        Com_sprintf(filename, length, "models/players/%s%s/%s_%s_%s.%s",
                    charactersFolder, modelName, base, skinName, team, ext);
      }
      ...
      if (cgs.gametype >= GT_TEAM && cgs.ffa_gt != 1) {
        if (i == 0 && teamName && *teamName) {
          ...
          Com_sprintf(filename, length, "models/players/%s%s/%s_%s.%s",
                      charactersFolder, modelName, base, team, ext);
        }
      } else {
        if (i == 0 && teamName && *teamName) {
          //								"models/players/characters/sergei/stroggs/lower_lily.skin"
          Com_sprintf(filename, length, "models/players/%s%s/%s%s_%s.%s",
                      charactersFolder, modelName, teamName, base, skinName,
                      ext);
        } else {
          //								"models/players/characters/sergei/lower_lily.skin"
          Com_sprintf(filename, length, "models/players/%s%s/%s_%s.%s",
                      charactersFolder, modelName, base, skinName, ext);
        }
      }
      if (CG_FileExists(filename)) {
        return qtrue;
...
==========================
 */
static qboolean CG_FindClientHeadFile(char *filename, int length,
                                      clientInfo_t *ci, const char *teamName,
  ...
  while (1) {
    for (i = 0; i < 2; i++) {
      if (i == 0 && teamName && *teamName) {
        Com_sprintf(filename, length, "models/players/%s%s/%s/%s%s_%s.%s",
                    headsFolder, headModelName, headSkinName, teamName, base,
                    team, ext);
      } else {
        Com_sprintf(filename, length, "models/players/%s%s/%s/%s_%s.%s",
                    headsFolder, headModelName, headSkinName, base, team, ext);
      }
      if (CG_FileExists(filename)) {
        return qtrue;
      }
      if (cgs.gametype >= GT_TEAM && cgs.ffa_gt != 1) {
        if (i == 0 && teamName && *teamName) {
          Com_sprintf(filename, length, "models/players/%s%s/%s%s_%s.%s",
                      headsFolder, headModelName, teamName, base, team, ext);
        } else {
          Com_sprintf(filename, length, "models/players/%s%s/%s_%s.%s",
                      headsFolder, headModelName, base, team, ext);
        }
      } else {
        if (i == 0 && teamName && *teamName) {
          Com_sprintf(filename, length, "models/players/%s%s/%s%s_%s.%s",
                      headsFolder, headModelName, teamName, base, headSkinName,
                      ext);
        } else {
          Com_sprintf(filename, length, "models/players/%s%s/%s_%s.%s",
                      headsFolder, headModelName, base, headSkinName, ext);
        }
      }
      if (CG_FileExists(filename)) {
        return qtrue;
...
static qboolean CG_RegisterClientSkin(clientInfo_t *ci, const char *teamName,
  ...
  if (ci->onepiece) {
    Com_sprintf(filename, sizeof(filename), "models/playerfs/%s/%stris_%s.skin",
    ...
    if (CG_FindClientModelFile(filename, sizeof(filename), ci, teamName,
                               modelName, skinName, "lower", "skin")) {
      ci->legsSkin = trap_R_RegisterSkin(filename);
    }
    if (!ci->legsSkin) {
  ...
  if (CG_FindClientHeadFile(filename, sizeof(filename), ci, teamName,
...
CG_RegisterClientModelname(clientInfo_t *ci, const char *modelName,
  ...
  if (cg_enableFS.integer) {
    Com_sprintf(filename, sizeof(filename), "models/playerfs/%s/tris.mdr",
                modelName);
  ...
  if (FSloaded == 0) {
    Com_sprintf(filename, sizeof(filename), "models/players/%s/lower.mdr",
                modelName);
    ci->legsModel = trap_R_RegisterModel(filename);
    if (!ci->legsModel) {
      Com_sprintf(filename, sizeof(filename),
                  "models/players/characters/%s/lower.mdr", modelName);
      ci->legsModel = trap_R_RegisterModel(filename);
      if (!ci->legsModel) {
    ...
    Com_sprintf(filename, sizeof(filename), "models/players/%s/upper.mdr",
                modelName);
    ci->torsoModel = trap_R_RegisterModel(filename);
    if (!ci->torsoModel) {
      Com_sprintf(filename, sizeof(filename),
                  "models/players/characters/%s/upper.mdr", modelName);
      ci->torsoModel = trap_R_RegisterModel(filename);
      if (!ci->torsoModel) {
  ...
  if (headName[0] == '*') {
    Com_sprintf(filename, sizeof(filename), "models/players/heads/%s/%s.mdr",
                &headModelName[1], &headModelName[1]);
  } else {
    Com_sprintf(filename, sizeof(filename), "models/players/%s/head.mdr",
                headName);
  }
  ...
  if (!CG_RegisterClientSkin(ci, teamName, modelName, skinName, headName,
    ...
    if (teamName && *teamName) {
      ...
      if (ci->team == TEAM_BLUE) {
        Com_sprintf(newTeamName, sizeof(newTeamName), "%s/",
                    DEFAULT_BLUETEAM_NAME);
      } else {
        Com_sprintf(newTeamName, sizeof(newTeamName), "%s/",
                    DEFAULT_REDTEAM_NAME);
      }
      if (!CG_RegisterClientSkin(ci, newTeamName, modelName, skinName, headName,
                                 headSkinName)) {
        Com_Printf("Failed to load skin file: %s : %s : %s, %s : %s\n",
  ...
  if (ci->onepiece) {
    Com_sprintf(filename, sizeof(filename), "models/playerfs/%s/animation.cfg",
                modelName);
  ...
  // leilei - load eyes
  Com_sprintf(filename, sizeof(filename), "models/players/%s/eyes.cfg",
              modelName);
  if (!CG_ParseEyesFile(filename, ci)) {
    //	Com_Printf( "No eyes for %s\n", filename );
  }

  if (CG_FindClientHeadFile(filename, sizeof(filename), ci, teamName, headName,
                            headSkinName, "icon", "skin")) {
    ci->modelIcon = trap_R_RegisterShaderNoMip(filename);
  } else if (CG_FindClientHeadFile(filename, sizeof(filename), ci, teamName,
                                   headName, headSkinName, "icon", "tga")) {
    ci->modelIcon = trap_R_RegisterShaderNoMip(filename);
  }
...
static void CG_LoadClientInfo(int clientNum, clientInfo_t *ci) {
  ...
  if (!CG_RegisterClientModelname(ci, ci->modelName, ci->skinName,
    ...
    if (cgs.gametype >= GT_TEAM && cgs.ffa_gt != 1) {
      ...
      if (!CG_RegisterClientModelname(ci, DEFAULT_TEAM_MODEL, ci->skinName,
                                      DEFAULT_TEAM_HEAD, ci->skinName,
                                      teamname)) {
      ...
      if (!CG_RegisterClientModelname(ci, DEFAULT_MODEL, "default",
                                      DEFAULT_MODEL, "default", teamname)) {
  ...
  // sounds
  dir = ci->modelName;
  fallback = (cgs.gametype >= GT_TEAM && cgs.ffa_gt != 1) ? DEFAULT_TEAM_MODEL
                                                          : DEFAULT_MODEL;
...
void CG_NewClientInfo(int clientNum) {
  ...
  if (cg_forceModel.integer) {
    ...
    if (cgs.gametype >= GT_TEAM && cgs.ffa_gt != 1) {
      Q_strncpyz(newInfo.headModelName, DEFAULT_TEAM_MODEL,
...
static void CG_PlayerAngles(centity_t *cent, vec3_t legs[3], vec3_t torso[3],
  ...
  // torso
  if (cg_cameramode.integer == 1) {
    CG_SwingAngles(torsoAngles[YAW], 180, 90, cg_swingSpeed.value,
                   &cent->pe.torso.yawAngle, &cent->pe.torso.yawing);
    CG_SwingAngles(legsAngles[YAW], 180, 90, cg_swingSpeed.value,
  ...
  if (clientNum >= 0 && clientNum < MAX_CLIENTS) {
    ...
    if ((cent->currentState.legsAnim & ~ANIM_TOGGLEBIT) == LEGS_STRAFE_LEFT ||
...
===============
 */
static void CG_PlayerFlag(centity_t *cent, qhandle_t hSkin,
                          refEntity_t *torso) {
  clientInfo_t *ci;
  refEntity_t pole;
  refEntity_t flag;
  ...
  if (updateangles) {
    ...
    if (fabs(d) < 0.9) {
      ...
      CG_SwingAngles(angles[YAW], 25, 90, 0.15f, &cent->pe.flag.yawAngle,
                     &cent->pe.flag.yawing);
    }
  }
...
static void CG_PlayerTokens(centity_t *cent, int renderfx) {
  ...
  for (i = 0; i < trail->numpositions; i++) {
    ...
    angle = (((cg.time + 500 * MAX_SKULLTRAIL - 500 * i) / 16) & 255) *
            (M_PI * 2) / 255;
...
void CG_AddRefEntityWithPowerups(refEntity_t *ent, entityState_t *state,
  ...
  if (state->powerups & (1 << PW_INVIS)) {
    ...
    trap_R_AddRefEntityToScene(ent);
    if (!isMissile && (cgs.dmflags & DF_PLAYER_OVERLAY) &&
...
void CG_Player(centity_t *cent) {
  ...
  if (!ci->onepiece) {
    // onepiece uses internal model skins right now until skin loading is fixed
    // later
  ...
  if (ci->onepiece) {
    ...
    VectorCopy(cent->pe.eyepos,
               head.eyepos[0]); // Copy it to our refdef for the renderer
