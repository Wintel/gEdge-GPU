...
void CG_CheckPlayerstateEvents(playerState_t *ps, playerState_t *ops) {
  ...
  for (i = ps->eventSequence - MAX_PS_EVENTS; i < ps->eventSequence; i++) {
    ...
    if (i >= ops->eventSequence
        ...
        // event we already issued or something the server told us changed our
        // prediction causing a different event
...
void CG_CheckLocalSounds(playerState_t *ps, playerState_t *ops) {
  ...
  if (ps->persistant[PERS_PLAYEREVENTS] != ops->persistant[PERS_PLAYEREVENTS]) {
    ...
    } else if ((ps->persistant[PERS_PLAYEREVENTS] &
                PLAYEREVENT_GAUNTLETREWARD) !=
               (ops->persistant[PERS_PLAYEREVENTS] &
                PLAYEREVENT_GAUNTLETREWARD)) {
  ...
  if (cgs.gametype > GT_TEAM) {
    if ((ps->powerups[PW_REDFLAG] != ops->powerups[PW_REDFLAG] &&
         ps->powerups[PW_REDFLAG]) ||
        (ps->powerups[PW_BLUEFLAG] != ops->powerups[PW_BLUEFLAG] &&
         ps->powerups[PW_BLUEFLAG]) ||
        (ps->powerups[PW_NEUTRALFLAG] != ops->powerups[PW_NEUTRALFLAG] &&
        ...
        if (cgs.gametype < GT_TEAM || cgs.ffa_gt == 1) {
          if (ps->persistant[PERS_RANK] == 0) {
            CG_AddBufferedSound(cgs.media.takenLeadSound);
                     ...
                     cgs.gametype != GT_POSSESSION) {
            CG_AddBufferedSound(cgs.media.tiedLeadSound);
          } else if (ps->persistant[PERS_RANK] != RANK_TIED_FLAG &&
                     (ops->persistant[PERS_RANK] & ~RANK_TIED_FLAG) == 0) {
  ...
  if (cgs.timelimit > 0 && cgs.timelimit <= 1000) {
    ...
    } else if (!(cg.timelimitWarnings & 2) &&
               msec > (cgs.timelimit - 1) * 60 * 1000) {
      cg.timelimitWarnings |= 1 | 2;
      trap_S_StartLocalSound(cgs.media.oneMinuteSound, CHAN_ANNOUNCER);
    } else if (cgs.timelimit > 5 && !(cg.timelimitWarnings & 1) &&
  ...
  if (cgs.fraglimit > 0 && cgs.gametype < GT_CTF) {
    ...
    } else if (cgs.fraglimit > 2 && !(cg.fraglimitWarnings & 2) &&
               highScore == (cgs.fraglimit - 2)) {
      cg.fraglimitWarnings |= 1 | 2;
      CG_AddBufferedSound(cgs.media.twoFragSound);
    } else if (cgs.fraglimit > 3 && !(cg.fraglimitWarnings & 1) &&
