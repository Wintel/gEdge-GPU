  ...
  if (attacker == cg.snap->ps.clientNum) {
    ...
    if (!(cg_singlePlayerActive.integer && cg_cameraOrbit.integer)) {
      CG_CenterPrint(s, SCREEN_HEIGHT * 0.30,
                     (int)(BIGCHAR_WIDTH * cg_fragmsgsize.value));
    }
#else
    CG_CenterPrint(s, SCREEN_HEIGHT * 0.30,
                   (int)(BIGCHAR_WIDTH * cg_fragmsgsize.value));
...
static void CG_ItemPickup(int itemNum) {
  ...
  if (bg_itemlist[itemNum].giType == IT_WEAPON) {
    ...
    if (cg_autoswitch.integer == 1 &&
        bg_itemlist[itemNum].giTag != WP_MACHINEGUN) {
      cg.weaponSelectTime = cg.time;
      cg.weaponSelect = bg_itemlist[itemNum].giTag;
    }
    /* if new */
    if (cg_autoswitch.integer == 2 &&
        0 == (cg.snap->ps.stats[STAT_WEAPONS] &
              (1 << bg_itemlist[itemNum].giTag))) {
      cg.weaponSelectTime = cg.time;
      cg.weaponSelect = bg_itemlist[itemNum].giTag;
    }
    /* if better */
    if (cg_autoswitch.integer == 3 &&
        CG_WeaponHigher(cg.weaponSelect, bg_itemlist[itemNum].giTag)) {
      cg.weaponSelectTime = cg.time;
      cg.weaponSelect = bg_itemlist[itemNum].giTag;
    }
    /* if new and better */
    if (cg_autoswitch.integer == 4 &&
        0 == (cg.snap->ps.stats[STAT_WEAPONS] &
              (1 << bg_itemlist[itemNum].giTag)) &&
        CG_WeaponHigher(cg.weaponSelect, bg_itemlist[itemNum].giTag)) {
      cg.weaponSelectTime = cg.time;
...
void CG_EntityEvent(centity_t *cent, vec3_t position) {
  ...
  case EV_JUMP_PAD:
    ...
    {
      vec3_t up = {0, 0, 1};
      CG_SmokePuff(cent->lerpOrigin, up, 32, 1, 1, 1, 0.33f, 1000, cg.time, 0,
                   LEF_PUFF_DONT_SCALE, cgs.media.smokePuffShader);
    }
  ...
  case EV_TAUNT_FOLLOWME:
    ...
    CG_VoiceChatLocal(SAY_TEAM, qfalse, es->number, COLOR_CYAN,
                      VOICECHAT_FOLLOWME);
    break;
  ...
  case EV_PLAYER_TELEPORT_IN:
    ...
    if (cg_leiEnhancement.integer)
      trap_R_LFX_ParticleEffect(18, cent->lerpOrigin,
  ...
  case EV_ITEM_RESPAWN:
    ...
    if (cg_leiEnhancement.integer)
      trap_R_LFX_ParticleEffect(41, cent->lerpOrigin,
                                cent->currentState.angles);
    break;
  ...
  case EV_OBELISKEXPLODE:
    ...
    if (cg_leiEnhancement.integer)
      trap_R_LFX_ParticleEffect(17, cent->lerpOrigin,
                                cent->currentState.angles);
    else
  ...
  case EV_INVUL_IMPACT:
    ...
    if (cg_leiEnhancement.integer)
      trap_R_LFX_ParticleEffect(42, cent->lerpOrigin,
                                cent->currentState.angles);
    else
  ...
  case EV_RAILTRAIL:
    ...
    //  if the client is us, unlagged is on server-side, and we've got it
    //  client-side
                             ...
                             // diminishes
  {
    ...
    switch (es->eventParm) {
    case GTS_RED_CAPTURE: // CTF: red team captured the blue flag, 1FCTF: red
                          // team captured the neutral flag
      if (cg.snap->ps.persistant[PERS_TEAM] == TEAM_RED)
        CG_AddBufferedSound(cgs.media.captureYourTeamSound);
      else
        CG_AddBufferedSound(cgs.media.captureOpponentSound);
      break;
    case GTS_BLUE_CAPTURE: // CTF: blue team captured the red flag, 1FCTF: blue
                           // team captured the neutral flag
    ...
    case GTS_RED_TAKEN: // CTF: red team took blue flag, 1FCTF: blue team took
                        // the neutral flag
      // if this player picked up the flag then a sound is played in
      // CG_CheckLocalSounds
      ...
      break;
    case GTS_BLUE_TAKEN: // CTF: blue team took the red flag, 1FCTF red team
                         // took the neutral flag
      // if this player picked up the flag then a sound is played in
      // CG_CheckLocalSounds
  ...
  case EV_GIB_PLAYER:
    ...
    if (cg_leiEnhancement.integer)
      trap_R_LFX_ParticleEffect(16, cent->lerpOrigin,
                                cent->currentState.angles);
...
void CG_CheckEvents(centity_t *cent) {
  ...
  if (cent->currentState.eType > ET_EVENTS) {
    ...
    // if this is a player event set the entity number of the client entity
    // number
