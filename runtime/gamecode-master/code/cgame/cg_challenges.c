...
// First two static variables that may only be accessed throgh functions in this
// file The challengeTable is of constant size so there is room for more
// challenges being added in the furture
...
// This function initializes the challenge system. It has no effect if the
// system is already intialized.
...
void challenges_init(void) {
  ...
  // If the file not is big enough to contain all challenges then initialize
  // from zero
  if (fileLen < sizeof(challengeTable)) {
    trap_FS_FCloseFile(
        file); // Lets remeber to close the file before all returns
...
// This functions saves the challenges. It only needs to be called once for each
// game. It should be called from the shutdown function
...
void challenges_save(void) {
  ...
  int i;
  if (!challengesInitialized) {
    // The challenge system has not been initialized and therefore cannot be
    // saved CG_Printf("Challenge system not initialized.\n");
    return;
  }
  ...
  // Lets make the challenges system uninitialized since changes after this will
  // most likely not be saved anyway.
...
void addChallenge(int challenge) {
  ...
  // CG_Printf("Increased challenge #%u by one. Is now
