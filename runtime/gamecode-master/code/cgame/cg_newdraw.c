...
void CG_SetSelectedPlayerName(void) {
  if (cg_currentSelectedPlayer.integer >= 0 &&
    ...
    clientInfo_t *ci =
        cgs.clientinfo + sortedTeamPlayers[cg_currentSelectedPlayer.integer];
    if (ci) {
...
static void CG_DrawPlayerArmorIcon(rectDef_t *rect, qboolean draw2D) {
  ...
  if (draw2D || (!cg_draw3dIcons.integer &&
                 cg_drawIcons.integer)) { // bk001206 - parentheses
    ...
    CG_Draw3DModel(rect->x, rect->y, rect->w, rect->h, cgs.media.armorModel, 0,
                   origin, angles);
  }
}
...
static float healthColors[4][4] = {
    //		{ 0.2, 1.0, 0.2, 1.0 } , { 1.0, 0.2, 0.2, 1.0 }, {0.5, 0.5, 0.5,
    //1} };
...
static void CG_DrawRedScore(rectDef_t *rect, float scale, vec4_t color,
  ...
  value = CG_Text_Width(num, scale, 0);
  CG_Text_Paint(rect->x + rect->w - value, rect->y + rect->h, scale, color, num,
                0, 0, textStyle);
}
...
static void CG_DrawRedName(rectDef_t *rect, float scale, vec4_t color,
                           int textStyle) {
  CG_Text_Paint(rect->x, rect->y + rect->h, scale, color, cg_redTeamName.string,
                0, 0, textStyle);
}
...
static void CG_DrawBlueFlagName(rectDef_t *rect, float scale, vec4_t color,
                                int textStyle) {
  int i;
  for (i = 0; i < cgs.maxclients; i++) {
    if (cgs.clientinfo[i].infoValid && cgs.clientinfo[i].team == TEAM_RED &&
        cgs.clientinfo[i].powerups & (1 << PW_BLUEFLAG)) {
      CG_Text_Paint(rect->x, rect->y + rect->h, scale, color,
                    cgs.clientinfo[i].name, 0, 0, textStyle);
      return;
    }
  }
}
  ...
  if (shader) {
    ...
    if (item) {
      ...
      if (cgs.blueflag >= 0 && cgs.blueflag <= 2) {
        CG_DrawPic(rect->x, rect->y, rect->w, rect->h,
                   cgs.media.flagShaders[cgs.blueflag]);
      } else {
        CG_DrawPic(rect->x, rect->y, rect->w, rect->h,
...
static void CG_DrawRedFlagName(rectDef_t *rect, float scale, vec4_t color,
                               int textStyle) {
  int i;
  for (i = 0; i < cgs.maxclients; i++) {
    if (cgs.clientinfo[i].infoValid && cgs.clientinfo[i].team == TEAM_BLUE &&
        cgs.clientinfo[i].powerups & (1 << PW_REDFLAG)) {
      CG_Text_Paint(rect->x, rect->y + rect->h, scale, color,
                    cgs.clientinfo[i].name, 0, 0, textStyle);
      return;
    }
  ...
  if (shader) {
    ...
    if (item) {
      ...
      if (cgs.redflag >= 0 && cgs.redflag <= 2) {
        CG_DrawPic(rect->x, rect->y, rect->w, rect->h,
                   cgs.media.flagShaders[cgs.redflag]);
      } else {
        CG_DrawPic(rect->x, rect->y, rect->w, rect->h,
                   cgs.media.flagShaders[0]);
      }
...
static void CG_HarvesterSkulls(rectDef_t *rect, float scale, vec4_t color,
                               qboolean force2D, int textStyle) {
  char num[16];
  vec3_t origin, angles;
  qhandle_t handle;
...
static void CG_DrawAreaPowerUp(rectDef_t *rect, int align, float special,
  ...
  for (i = 0; i < active; i++) {
    ...
    if (item) {
      ...
      CG_DrawPic(r2.x, r2.y, r2.w * .75, r2.h,
                 trap_R_RegisterShader(item->icon));

      Com_sprintf(num, sizeof(num), "%i", sortedTime[i] / 1000);
      CG_Text_Paint(r2.x + (r2.w * .75) + 3, r2.y + r2.h, scale, color, num, 0,
                    0, 0);
      *inc += r2.w + special;
    }
  }
...
qboolean CG_OwnerDrawVisible(int flags) {
  ...
  if (flags & (CG_SHOW_BLUE_TEAM_HAS_REDFLAG | CG_SHOW_RED_TEAM_HAS_BLUEFLAG)) {
    if (flags & CG_SHOW_BLUE_TEAM_HAS_REDFLAG &&
        (cgs.redflag == FLAG_TAKEN || cgs.flagStatus == FLAG_TAKEN_RED)) {
      return qtrue;
    } else if (flags & CG_SHOW_RED_TEAM_HAS_BLUEFLAG &&
               (cgs.blueflag == FLAG_TAKEN ||
                cgs.flagStatus == FLAG_TAKEN_BLUE)) {
      return qtrue;
    }
    return qfalse;
  }
...
static void CG_DrawPlayerHasFlag(rectDef_t *rect, qboolean force2D) {
  ...
  if (cg.predictedPlayerState.powerups[PW_REDFLAG]) {
    CG_DrawFlagModel(rect->x + adj, rect->y + adj, rect->w - adj, rect->h - adj,
                     TEAM_RED, force2D);
...
static void CG_DrawCapFragLimit(rectDef_t *rect, float scale, vec4_t color,
                                qhandle_t shader, int textStyle) {
  int limit = (cgs.gametype >= GT_CTF && cgs.ffa_gt == 0) ? cgs.capturelimit
                                                          : cgs.fraglimit;
...
static void CG_Draw1stPlace(rectDef_t *rect, float scale, vec4_t color,
                            qhandle_t shader, int textStyle) {
  if (cgs.scores1 != SCORE_NOT_PRESENT) {
    CG_Text_Paint(rect->x, rect->y, scale, color, va("%2i", cgs.scores1), 0, 0,
                  textStyle);
  }
}
...
static void CG_Draw2ndPlace(rectDef_t *rect, float scale, vec4_t color,
                            qhandle_t shader, int textStyle) {
  if (cgs.scores2 != SCORE_NOT_PRESENT) {
    CG_Text_Paint(rect->x, rect->y, scale, color, va("%2i", cgs.scores2), 0, 0,
                  textStyle);
  }
}
...
void CG_DrawNewTeamInfo(rectDef_t *rect, float text_x, float text_y,
  ...
  if (cg.spectatorLen) {
    ...
    if (cg.time > cg.spectatorTime) {
      ...
      if (cg.spectatorPaintX <= rect->x + 2) {
        if (cg.spectatorOffset < cg.spectatorLen) {
          cg.spectatorPaintX +=
              CG_Text_Width(&cg.spectatorList[cg.spectatorOffset], scale, 1) -
              1;
          cg.spectatorOffset++;
        } else {
          cg.spectatorOffset = 0;
    ...
    CG_Text_Paint_Limit(&maxX, cg.spectatorPaintX, rect->y + rect->h - 3, scale,
                        color, &cg.spectatorList[cg.spectatorOffset], 0, 0);
    if (cg.spectatorPaintX2 >= 0) {
      float maxX2 = rect->x + rect->w - 2;
      CG_Text_Paint_Limit(&maxX2, cg.spectatorPaintX2, rect->y + rect->h - 3,
                          scale, color, cg.spectatorList, 0,
                          cg.spectatorOffset);
    }
...
void CG_DrawMedal(int ownerDraw, rectDef_t *rect, float scale, vec4_t color,
                  qhandle_t shader) {
  ...
  if (text) {
    ...
    CG_Text_Paint(rect->x + (rect->w - value) / 2, rect->y + rect->h + 10,
                  scale, color, text, 0, 0, 0);
  }
  trap_R_SetColor(NULL);
}
...
static void CG_DrawProgressBar(rectDef_t *rect, vec4_t color, float scale,
  ...
  if (align == ITEM_ALIGN_RIGHT) {
    ...
    CG_DrawPic(rect->x + rimWidth, rect->y + rect->h - rimWidth, leftWidth,
               rimWidth, cgs.media.whiteShader);
    CG_DrawPic(rect->x + rimWidth + leftWidth, rect->y, rimWidth + doneWidth,
               rect->h, cgs.media.whiteShader);
  } else {
    CG_DrawPic(rect->x, rect->y, rimWidth + doneWidth, rect->h,
               cgs.media.whiteShader);
    CG_DrawPic(rimWidth + rect->x + doneWidth, rect->y, leftWidth, rimWidth,
               cgs.media.whiteShader);
...
static void CG_DrawProgressLabel(rectDef_t *rect, float text_x, float text_y,
  ...
  if (fraction < 1.0f)
    CG_Text_Paint(rect->x + text_x + tx, rect->y + text_y, scale, white, s, 0,
                  0, ITEM_TEXTSTYLE_NORMAL);
  else
    CG_Text_Paint(rect->x + text_x + tx, rect->y + text_y, scale, color, s, 0,
                  0, ITEM_TEXTSTYLE_NEON);
}
...
static void CG_DrawMediaProgressLabel(rectDef_t *rect, float text_x,
                                      ...
                                      int align) {
  CG_DrawProgressLabel(rect, text_x, text_y, color, scale, align, "Overall",
                       cg.mediaFraction);
}
...
static void CG_DrawSoundProgressLabel(rectDef_t *rect, float text_x,
                                      float text_y, vec4_t color, float scale,
                                      int align) {
  CG_DrawProgressLabel(rect, text_x, text_y, color, scale, align, "Sound",
                       cg.soundFraction);
}
...
static void CG_DrawGraphicProgressLabel(rectDef_t *rect, float text_x,
                                        float text_y, vec4_t color, float scale,
                                        int align) {
  CG_DrawProgressLabel(rect, text_x, text_y, color, scale, align, "Graphics",
                       cg.graphicFraction);
}
...
static void CG_DrawHostname(rectDef_t *rect, float text_x, float text_y,
  ...
  CG_DrawLoadingString(rect, text_x, text_y, color, scale, align, textStyle,
                       buffer);
}
...
// leilei - additions that were missing from the loaing screen
static void CG_DrawTimeLimit(rectDef_t *rect, float text_x, float text_y,
                             vec4_t color, float scale, int align,
                             int textStyle) {
  const char *info;
  int value;
  ...
  if (value) {
    ...
    CG_DrawLoadingString(rect, text_x, text_y, color, scale, align, textStyle,
                         va("%01i:%02i", minutes, seconds));
  }
}

static void CG_DrawFragLimit(rectDef_t *rect, float text_x, float text_y,
                             vec4_t color, float scale, int align,
                             int textStyle) {
  const char *info;
  int value;
...
// leilei - additions that were missing from the loaing screen
static void CG_DrawCaptureLimit(rectDef_t *rect, float text_x, float text_y,
                                vec4_t color, float scale, int align,
                                int textStyle) {
  const char *info;
  int value;
...
void CG_KeyEvent(int key, qboolean down) {
  ...
  // if (key == trap_Key_GetKey("teamMenu") || !Display_CaptureItem(cgs.cursorX,
  // cgs.cursorY)) {
