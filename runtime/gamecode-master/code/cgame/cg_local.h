...
typedef struct centity_s {
  ...
  int newcamrunning; // leilei - determines if we should look in a direction for
                     // running
  vec3_t eyesOrigin;
  vec3_t eyesAngles;
  ...
  float startroll; // leilei - muzzleflashes - starting roll so it's not
                   // constantly flickering a roll every other frame.
  int muztime[5];
  int muzslot;
  float muzroll[5];
} centity_t;
...
typedef struct weaponInfo_s {
  ...
  qhandle_t handsModel; // the hands don't actually draw, they just position the
                        // weapon
  ...
  qhandle_t flashModel_type1;  // Quake style
                               // (gradient-textured oval sphere)
  qhandle_t flashModel_type2;  // u99 style
                               // (additive-blended animated sprite)
  qhandle_t flashModel_type2a; // 3rd person for U99 style	(sprite cloud)
  qhandle_t flashModel_type3;  // 64 style			(an alphablended
                               // sprite for 1st gen 3d hardware)
  qhandle_t flashModel_type4;  // Anime style			(whatever I darn
                               // well want)
  qhandle_t flashModel_type5;  // 2003 style			(4 intersecting
                               // planes that scale up and fade)
  qhandle_t flashModel_type5a; // 3rd person for 2003 style 	(cone that
                               // scales up and doesn't fade))

  // float			startroll; // starting roll so it's not
  // constantly flickering a roll every other frame.
} weaponInfo_t;
...
typedef enum {
  ...
  CHAT_PRINT,
  TEAMCHAT_PRINT
} q3print_t; // bk001201 - warning: useless keyword or type name in empty
             // declaration
...
void trap_R_LFX_ParticleEffect(
    int effect, const vec3_t origin,
    const vec3_t velocity); // leilei - particle effects. this allows to pick an
                            // effect, such as..
