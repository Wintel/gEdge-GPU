...
void CG_GoreMark(localEntity_t *le, trace_t *trace) {
  ...
  if (le->leMarkType == LEMT_BURN) {
    ...
    radius = 6 + (rand() & 16);
    CG_ImpactMark(cgs.media.lbldShader2, trace->endpos, trace->plane.normal,
                  random() * 360, 1, 1, 1, 1, qtrue, radius, qfalse);
  }
...
static void CG_AddMoveScaleFade(localEntity_t *le) {
  ...
  if (le->fadeInTime > le->startTime && cg.time < le->fadeInTime) {
    // fade / grow time
    c = 1.0 -
        (float)(le->fadeInTime - cg.time) / (le->fadeInTime - le->startTime);
  } else {
    // fade / grow time
    c = (le->endTime - cg.time) * le->lifeRate;
  }
...
void CG_AddKamikaze(localEntity_t *le) {
  ...
  if (t > KAMI_SHOCKWAVE_STARTTIME && t < KAMI_SHOCKWAVE_ENDTIME) {
    ...
    if (!(le->leFlags & LEF_SOUND1)) {
      //			trap_S_StartSound (re->origin, ENTITYNUM_WORLD,
      //CHAN_AUTO, cgs.media.kamikazeExplodeSound );
    ...
    c = (float)(t - KAMI_SHOCKWAVE_STARTTIME) /
        (float)(KAMI_SHOCKWAVE_ENDTIME - KAMI_SHOCKWAVE_STARTTIME);
    VectorScale(axis[0],
                c * KAMI_SHOCKWAVE_MAXRADIUS / KAMI_SHOCKWAVEMODEL_RADIUS,
                shockwave.axis[0]);
    VectorScale(axis[1],
                c * KAMI_SHOCKWAVE_MAXRADIUS / KAMI_SHOCKWAVEMODEL_RADIUS,
                shockwave.axis[1]);
  ...
  if (t > KAMI_EXPLODE_STARTTIME && t < KAMI_IMPLODE_ENDTIME) {
    ...
    if (t < KAMI_IMPLODE_STARTTIME) {
      ...
      if (!(le->leFlags & LEF_SOUND2)) {
        //				trap_S_StartSound (re->origin,
        //ENTITYNUM_WORLD, CHAN_AUTO, cgs.media.kamikazeImplodeSound );
    ...
    VectorScale(axis[0],
                c * KAMI_BOOMSPHERE_MAXRADIUS / KAMI_BOOMSPHEREMODEL_RADIUS,
                re->axis[0]);
    VectorScale(axis[1],
                c * KAMI_BOOMSPHERE_MAXRADIUS / KAMI_BOOMSPHEREMODEL_RADIUS,
                re->axis[1]);
  ...
  if (t > KAMI_SHOCKWAVE2_STARTTIME && t < KAMI_SHOCKWAVE2_ENDTIME) {
    ...
    if (le->angles.trBase[0] == 0 && le->angles.trBase[1] == 0 &&
    ...
    VectorScale(axis[0],
                c * KAMI_SHOCKWAVE2_MAXRADIUS / KAMI_SHOCKWAVEMODEL_RADIUS,
                shockwave.axis[0]);
    VectorScale(axis[1],
                c * KAMI_SHOCKWAVE2_MAXRADIUS / KAMI_SHOCKWAVEMODEL_RADIUS,
                shockwave.axis[1]);
...
void CG_AddScorePlum(localEntity_t *le) {
  ...
  for (i = 0; i < numdigits; i++) {
    VectorMA(origin, (float)(((float)numdigits / 2) - i) * NUMBER_SIZE, vec,
             re->origin);
