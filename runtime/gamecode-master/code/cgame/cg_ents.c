...
static void CG_Item(centity_t *cent) {
  ...
  if (item->giType == IT_WEAPON) {
    ...
    ent.nonNormalizedAxes = qtrue;
    trap_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
                           vec3_origin, cgs.media.weaponHoverSound);
  }
  ...
  */
  ...
  if (weapon->missileDlight) {
    trap_R_AddLightToScene(
        cent->lerpOrigin, weapon->missileDlight, weapon->missileDlightColor[0],
...
static void CG_Grapple(centity_t *cent) {
  ...
  if (weapon->missileSound) {
    trap_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
                           vec3_origin, weapon->missileSound);
  }
...
static void CG_CalcEntityLerpPositions(centity_t *cent) {
  ...
  //cent->lerpOrigin ); 	BG_EvaluateTrajectory( &cent->currentState.apos,
  //cg.time, cent->lerpAngles );
  ...
  if (timeshift != 0) {
    ...
    if (tr.fraction < 1.0f) {
      cent->lerpOrigin[0] =
          lastOrigin[0] + tr.fraction * (cent->lerpOrigin[0] - lastOrigin[0]);
      cent->lerpOrigin[1] =
          lastOrigin[1] + tr.fraction * (cent->lerpOrigin[1] - lastOrigin[1]);
      cent->lerpOrigin[2] =
          lastOrigin[2] + tr.fraction * (cent->lerpOrigin[2] - lastOrigin[2]);
    }
  }
  // unlagged - projectile nudge
  ...
  if (cent != &cg.predictedPlayerEntity) {
    CG_AdjustPositionForMover(cent->lerpOrigin,
                              ...
                              cent->lerpAngles, cent->lerpAngles);
  }
}
    ...
    if (cent->currentState.frame == 2) {
      ...
      if (t > h) {
        if (!cent->muzzleFlashTime) {
          trap_S_StartSound(cent->lerpOrigin, ENTITYNUM_NONE, CHAN_BODY,
