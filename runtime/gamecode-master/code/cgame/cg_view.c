...
static void CG_CalcVrect(void) {
  ...
  if (cg.snap->ps.pm_type == PM_INTERMISSION) {
    ...
    if (cg_viewsize.integer < 30) {
      ...
      trap_Cvar_Set(
          "cg_viewsize",
          "120"); // leilei - increased to 120 for retro sbar disabling
      size = 120;
    } else {
...
static void CG_OffsetThirdPersonView(void) {
  ...
  if (cg_deathcam.integer == 2 &&
      (cg.predictedPlayerState.stats[STAT_HEALTH] <= 0)) { // leilei - deathcam
  ...
  if (cg_cameramode.integer &&
      (cg.predictedPlayerState.stats[STAT_HEALTH] >
       0)) // leilei this mode is off to the player's right
  {
    ...
    // cg.refdefViewAngles[YAW] -= cg_thirdPersonAngle.value;	 // can't do
    // this right now.
    ...
    /*
       ...
       cg.predictedPlayerState.clientNum, MASK_SOLID );
                    ...
                    if ( trace.fraction != 1.0 ) {
                            ...
                            // try another trace to this position, because a
       tunnel may have the ceiling
                            // close enough that this is poking out

                            CG_Trace( &trace, cg.refdef.vieworg, mins, maxs,
       view, cg.predictedPlayerState.clientNum, MASK_SOLID ); VectorCopy(
       trace.endpos, view );
                    }
            }
    */
...
static void CG_OffsetFirstPersonView(void) {
  ...
  if (cg.damageTime && cgs.gametype != GT_ELIMINATION &&
...
// loadingscreen
  ...
  if (!cg.hyperspace) {
    CG_AddPacketEntities(); // adter calcViewValues, so predicted player state
                            // is correct
    CG_AddMarks();
    CG_AddLocalEntities();
  }
