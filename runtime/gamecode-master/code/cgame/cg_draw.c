    ...
    while (s && *s && count < len) {
      ...
      if (Q_IsColorString(s)) {
        ...
        if (style == ITEM_TEXTSTYLE_SHADOWED ||
          ...
          CG_Text_PaintChar(x + ofs, y - yadj + ofs, glyph->imageWidth,
                            glyph->imageHeight, useScale, glyph->s, glyph->t,
                            glyph->s2, glyph->t2, glyph->glyph);
          colorBlack[3] = 1.0;
          trap_R_SetColor(newColor);
        }
        CG_Text_PaintChar(x, y - yadj, glyph->imageWidth, glyph->imageHeight,
...
void CG_Draw3DModelEyes(float x, float y, float w, float h, qhandle_t model,
                        qhandle_t skin, vec3_t origin, vec3_t angles,
                        vec3_t eyep) {
  refdef_t refdef;
  refEntity_t ent;
...
void CG_DrawHead(float x, float y, float w, float h, int clientNum,
  ...
  if (cg_draw3dIcons.integer) {
    ...
    // headAngles );
    CG_Draw3DModelEyes(x, y, w, h, ci->headModel, ci->headSkin, origin,
                       headAngles, ci->eyepos);
...
static void CG_DrawStatusBarHead(float x) {
  ...
  frac =
      (cg.time - cg.headStartTime) / (float)(cg.headEndTime - cg.headStartTime);
...
static void CG_DrawStatusBar(void) {
  ...
  else // Sago: If we follow find the teamcolor of the guy we follow. It might
       // not be our own team!
    CG_DrawTeamBackground(0, 420, 640, 60, 0.33f,
  ...
  if (cent->currentState.weapon &&
    ...
    CG_Draw3DModel(CHAR_WIDTH * 3 + TEXT_ICON_SPACE, 432, ICON_SIZE, ICON_SIZE,
                   cg_weapons[cent->currentState.weapon].ammoModel, 0, origin,
                   angles);
  }
  ...
  if (cgs.gametype == GT_HARVESTER) {
    ...
    CG_Draw3DModel(470 + CHAR_WIDTH * 3 + TEXT_ICON_SPACE, 432, ICON_SIZE,
                   ICON_SIZE, handle, 0, origin, angles);
  }
  ...
  if (value > 0) {
    ...
    if (!cg_draw3dIcons.integer && cg_drawIcons.integer) {
      CG_DrawPic(370 + CHAR_WIDTH * 3 + TEXT_ICON_SPACE, 432, ICON_SIZE,
                 ICON_SIZE, cgs.media.armorIcon);
    }
  }
  ...
  if (cgs.gametype == GT_HARVESTER) {
    ...
    if (value > 0) {
      ...
      if (!cg_draw3dIcons.integer && cg_drawIcons.integer) {
        ...
        CG_DrawPic(470 + CHAR_WIDTH * 3 + TEXT_ICON_SPACE, 432, ICON_SIZE,
                   ICON_SIZE, handle);
      }
    }
  }
  ...
  if (cg.snap->ps.powerups[PW_NEUTRALFLAG]) {
    ...
    for (i = 0; i < MAX_GENTITIES; i++) {
      if (cg_entities[i].currentState.eType == ET_PLAYER &&
          cg_entities[i].currentState.powerups & (1 << PW_NEUTRALFLAG)) {
        if (cg_entities[i].lerpOrigin[0] || cg_entities[i].lerpOrigin[1] ||
...
static float CG_DrawEliminationTimer(float y) {
  ...
  if (cg.time > rst) // We are started
  {
    ...
    if (cg.warmup == 0) {
      ...
      // CG_Text_Paint(320 - w / 2, 125, scale, colorWhite, st, 0, 0,
      // ITEM_TEXTSTYLE_SHADOWEDMORE);
#else

      w = CG_DrawStrlen(st);
      CG_DrawStringExt(320 - w * cw / 2, 70, st, colorWhite, qfalse, qtrue, cw,
                       (int)(cw * 1.5), 0);
#endif
    }
    /*
...
static float CG_DrawTeamOverlay(float y, qboolean right, qboolean upper) {
  ...
  for (i = 0; i < count; i++) {
    ...
    if (ci->infoValid && ci->team == cg.snap->ps.persistant[PERS_TEAM]) {
      ...
      CG_DrawStringExt(xx, y, ci->name, hcolor, qfalse, qfalse, TINYCHAR_WIDTH,
                       TINYCHAR_HEIGHT, TEAM_OVERLAY_MAXNAME_WIDTH);
      ...
      xx = x + TINYCHAR_WIDTH * 3 + TINYCHAR_WIDTH * pwidth +
           TINYCHAR_WIDTH * lwidth;

      CG_DrawStringExt(xx, y, st, hcolor, qfalse, qfalse, TINYCHAR_WIDTH,
                       TINYCHAR_HEIGHT, 0);
...
static void CG_DrawFragMsgIcons(fragInfo_t fi, int i) {
  ...
  if (fi.attackerName[0] != '\0') {
    CG_Text_Paint(FRAGMSG_X, FRAGMSG_Y + (i * 16), 0.20, color, fi.attackerName,
  ...
  if (fi.attackerName[0] != '\0') {
    CG_DrawStringExt(FRAGMSG_X, FRAGMSG_Y + (i * TINYCHAR_HEIGHT),
                     fi.attackerName, hcolor, qfalse, qfalse, TINYCHAR_WIDTH,
                     TINYCHAR_HEIGHT, 0);
  ...
  CG_DrawStringExt(FRAGMSG_X + x_offset, FRAGMSG_Y + (i * TINYCHAR_HEIGHT),
                   fi.targetName, hcolor, qfalse, qfalse, TINYCHAR_WIDTH,
                   TINYCHAR_HEIGHT, 0);

  if (fi.teamFrag) {
    x_offset += CG_DrawStrlen(fi.targetName) * TINYCHAR_WIDTH + spacing;
    CG_DrawStringExt(FRAGMSG_X + x_offset, FRAGMSG_Y + (i * TINYCHAR_HEIGHT),
                     "(^1TEAMMATE^7)", hcolor, qfalse, qfalse, TINYCHAR_WIDTH,
                     TINYCHAR_HEIGHT, 0);
  }
...
static void CG_DrawFragMsgText(char *msg, int i) {
  ...
  CG_DrawStringExt(FRAGMSG_X, FRAGMSG_Y + (i * TINYCHAR_HEIGHT), msg, hcolor,
    ...
    if (cgs.gametype == GT_CTF || cgs.gametype == GT_CTF_ELIMINATION) {
      ...
      if (item) {
        ...
        if (cgs.blueflag >= 0 && cgs.blueflag <= 2) {
          CG_DrawPic(x, y1 - 4, w, BIGCHAR_HEIGHT + 8,
                     cgs.media.blueFlagShader[cgs.blueflag]);
        }
      }
    }
    ...
    if (cgs.gametype == GT_CTF || cgs.gametype == GT_CTF_ELIMINATION) {
      ...
      if (item) {
        ...
        if (cgs.redflag >= 0 && cgs.redflag <= 2) {
          CG_DrawPic(x, y1 - 4, w, BIGCHAR_HEIGHT + 8,
                     cgs.media.redFlagShader[cgs.redflag]);
        }
      }
    }
...
static float CG_DrawPowerups(float y) {
  ...
  float size;
  float f;
  static float colors[2][4] = {{0.2f, 1.0f, 0.2f, 1.0f},
                               {1.0f, 0.2f, 0.2f, 1.0f}};
...
static void CG_DrawReward(void) {
  ...
  /*
  count = cg.rewardCount[0]/10;				// number of big rewards
  to draw
  ...
  count = cg.rewardCount[0] - count*10;		// number of small rewards to
  draw
   */
...
static void CG_DrawDisconnect(void) {
  ...
  if (cmd.serverTime <= cg.snap->ps.commandTime ||
      cmd.serverTime >
          cg.time) { // special check for map_restart // bk 0102165 - FIXME
    return;
  }
...
static void drawCenterString(const char *line, float *color) {
  ...
  CG_DrawStringExt(x, y, line, color, qfalse, qtrue, cg.centerPrintCharWidth,
                   (int)(cg.centerPrintCharWidth * 1.5), 0);
}
#endif
...
static void CG_DrawCenterDDString(void) {
  ...
  if (statusA == TEAM_BLUE) {
    line = va("Blue scores in %i",
              (cgs.timetaken + 10 * 1000 - cg.time) / 1000 + 1);
    color = colorBlue;
  } else if (statusA == TEAM_RED) {
    line = va("Red scores in %i",
              (cgs.timetaken + 10 * 1000 - cg.time) / 1000 + 1);
    color = colorRed;
  } else {
    lastDDSec = -100;
...
static void CG_DrawCrosshair(void) {
  ...
  trap_R_DrawStretchPic(x + cg.refdef.x + 0.5 * (cg.refdef.width - w),
                        y + cg.refdef.y + 0.5 * (cg.refdef.height - h), w, h, 0,
                        0, 1, 1, hShader);
}
...
static void CG_DrawCrosshair3D(void) {
  ...
  // Use a different method rendering the crosshair so players don't see two of
  // them when focusing their eyes at distant objects with high stereo
  // separation We are going to trace to the next shootable object and place the
  // crosshair in front of it.
...
static void CG_DrawSpectator(void) {
  ...
  if (cgs.gametype == GT_TOURNAMENT) {
    ...
    CG_DrawBigString(320 - 39 * 8, 460,
                     "press ESC and use the JOIN menu to play", 1.0F);
  }
}
...
static qboolean CG_DrawScoreboard(void) {
  ...
  char *s;
  int w;
  if (cg.respawnTime && cg.snap->ps.persistant[PERS_TEAM] != TEAM_SPECTATOR &&
      (cgs.gametype < GT_ELIMINATION || cgs.gametype > GT_LMS)) {
    if (cg.respawnTime > cg.time) {
      s = va("Respawn in: %2.2f",
             ((double)cg.respawnTime - (double)cg.time) / 1000.0);
      w = CG_DrawStrlen(s) * SMALLCHAR_WIDTH;
      CG_DrawSmallStringColor(320 - w / 2, 400, s, colorYellow);
    } else {
      s = va("Click FIRE to respawn");
      w = CG_DrawStrlen(s) * SMALLCHAR_WIDTH;
      CG_DrawSmallStringColor(320 - w / 2, 400, "Click FIRE to respawn",
                              colorGreen);
    }
  }
...
qboolean CG_DrawAccboard(void) {
  ...
  for (counter = 0; counter < WP_NUM_WEAPONS; counter++) {
    if (cg_weapons[counter + 2].weaponIcon && counter != WP_PROX_LAUNCHER &&
        counter != WP_GRAPPLING_HOOK)
      i++;
  }

  CG_DrawTeamBackground(ACCBOARD_XPOS, ACCBOARD_YPOS, ACCBOARD_WIDTH,
                        ACCBOARD_HEIGHT * (i + 1), 0.33f, TEAM_BLUE);
  ...
  for (counter = 0; counter < WP_NUM_WEAPONS; counter++) {
    if (cg_weapons[counter + 2].weaponIcon && counter != WP_PROX_LAUNCHER &&
        counter != WP_GRAPPLING_HOOK) {
      CG_DrawPic(ACCBOARD_XPOS + 10, ACCBOARD_YPOS + 10 + i * ACCBOARD_HEIGHT,
                 ACCITEM_SIZE, ACCITEM_SIZE,
                 cg_weapons[counter + 2].weaponIcon);
      if (cg.accuracys[counter][0] > 0)
        CG_DrawSmallStringColor(
            ACCBOARD_XPOS + 10 + ACCITEM_SIZE + 10,
            ACCBOARD_YPOS + 10 + i * ACCBOARD_HEIGHT + ACCITEM_SIZE / 2 -
                SMALLCHAR_HEIGHT / 2,
            va("%i%s",
               (int)(((float)cg.accuracys[counter][1] * 100) /
                     ((float)(cg.accuracys[counter][0]))),
               "%"),
            colorWhite);
      else
        CG_DrawSmallStringColor(ACCBOARD_XPOS + 10 + ACCITEM_SIZE + 10,
                                ACCBOARD_YPOS + 10 + i * ACCBOARD_HEIGHT +
                                    ACCITEM_SIZE / 2 - SMALLCHAR_HEIGHT / 2,
                                "-%", colorWhite);
      i++;
    }
  }
...
void trap_Cmd_ExecuteText(int exec_when,
                          const char *text); // leilei - for unlocks only...
...
static void CG_DrawProxWarning(void) {
  ...
  CG_DrawBigStringColor(320 - w / 2, 64 + BIGCHAR_HEIGHT, s,
                        g_color_table[ColorIndex(COLOR_RED)]);
}
// #endif
...
static void CG_DrawWarmup(void) {
  ...
  if (cgs.gametype == GT_TOURNAMENT) {
    ...
    if (ci1 && ci2) {
      ...
      CG_DrawStringExt(320 - w * cw / 2, 20, s, colorWhite, qfalse, qtrue, cw,
                       (int)(cw * 1.5f), 0);
#endif
    }
  } else {
...
static void CG_Draw2D(stereoFrame_t stereoFrame) {
   ...
   */
  if (cg.snap->ps.persistant[PERS_TEAM] ==
      TEAM_SPECTATOR /*|| cg.snap->ps.pm_type == PM_SPECTATOR*/) {
    CG_DrawSpectator();
