...
static void CG_spWin_f(void) {
  ...
  // leilei - Unlock stuff!!! Trophies crap.
  {
    ...
    trap_Cvar_VariableStringBuffer(
        "ui_currentMap", berf,
        sizeof(berf)); // get map number instead for list consistency
    ...
    if (trophyearn) {
      ...
      trap_Cvar_Set(va("ui_sp_unlock_%s", berf),
                    va("%i", trophyearn)); // YA YUO DID IT!!!1
    }

    // leilei - get all the total trophies. Should really be done in the single
    // player ui scripts, but 		doing it here could make a nice verifier for
    // legitimacy :)
    {
      int tropees = 0;
      int tropgold = 0;
