...
void trap_CM_TransformedCapsuleTrace(trace_t *results, const vec3_t start,
                                     ...
                                     int brushmask, const vec3_t origin,
                                     const vec3_t angles) {
  syscall(CG_CM_TRANSFORMEDCAPSULETRACE, results, start, end, mins, maxs, model,
          brushmask, origin, angles);
}
...
void trap_R_AddAdditiveLightToScene(const vec3_t org, float intensity, float r,
                                    float g, float b) {
  syscall(CG_R_ADDADDITIVELIGHTTOSCENE, org, PASSFLOAT(intensity), PASSFLOAT(r),
          PASSFLOAT(g), PASSFLOAT(b));
}
...
int trap_R_LerpTag(orientation_t *tag, clipHandle_t mod, int startFrame,
  ...
  return syscall(CG_R_LERPTAG, tag, mod, startFrame, endFrame, PASSFLOAT(frac),
                 tagName);
}
...
// leilei - particles!
void trap_R_LFX_ParticleEffect(int effect, const vec3_t origin,
