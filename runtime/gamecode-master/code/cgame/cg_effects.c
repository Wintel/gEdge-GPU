...
localEntity_t *CG_SlowPuff(const vec3_t p, const vec3_t vel, float radius,
                           float r, float g, float b, float a, float duration,
                           int startTime, int fadeInTime, int leFlags,
                           qhandle_t hShader) {
  static int seed = 0x92;
...
void CG_ObeliskExplode(vec3_t org, int entityNum) {
  ...
  origin[2] += 64;
  le = CG_MakeExplosion(origin, vec3_origin, cgs.media.dishFlashModel,
                        cgs.media.rocketExplosionShader, 600, qtrue);
  le->light = 300;
  le->lightColor[0] = 1;
...
void CG_ScorePlum(int client, vec3_t org, int score) {
  ...
  // (int)org[2], (int)Distance(org, lastPos));
