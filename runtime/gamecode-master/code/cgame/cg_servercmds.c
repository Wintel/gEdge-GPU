...
static void CG_ParseAccuracy(void) {
  ...
  for (i = WP_MACHINEGUN; i < WP_NUM_WEAPONS; i++) {
    cg.accuracys[i - WP_MACHINEGUN][0] =
        atoi(CG_Argv((i - WP_MACHINEGUN) * 2 + 1));
    cg.accuracys[i - WP_MACHINEGUN][1] =
        atoi(CG_Argv((i - WP_MACHINEGUN) * 2 + 2));
#if DEBUG
...
=================
CG_ParseMappage
Sago: This parses values from the server rather directly. Some checks are
performed, but beware if you change it or new security holes are found
=================
*/
...
=================
*/
static void CG_ParseDDtimetaken(void) { cgs.timetaken = atoi(CG_Argv(1)); }
...
static void CG_ParseDomPointNames(void) {
  ...
  for (i = 0; i < cgs.domination_points_count; i++) {
    Q_strncpyz(cgs.domination_points_names[i],
               CG_Argv(2) + i * MAX_DOMINATION_POINTS_NAMES,
               MAX_DOMINATION_POINTS_NAMES - 1);
    for (j = MAX_DOMINATION_POINTS_NAMES - 1;
         cgs.domination_points_names[i][j] < '0' && j > 0; j--) {
...
void CG_ParseServerinfo(void) {
  ...
  cgs.nopickup = atoi(Info_ValueForKey(info, "g_rockets")) +
                 atoi(Info_ValueForKey(info, "g_instantgib")) +
                 atoi(Info_ValueForKey(info, "g_elimination"));
...
#ifdef MISSIONPACK
    if (cgs.gametype >= GT_CTF && cgs.gametype < GT_MAX_GAME_TYPE &&
        !cgs.ffa_gt) {
  ...
  while (1) {
    ...
    while (1) {
      ...
      voiceChats[voiceChatList->numVoiceChats]
          .sounds[voiceChats[voiceChatList->numVoiceChats].numSounds] = sound;
      token = COM_ParseExt(p, qtrue);
      if (!token || token[0] == 0) {
        return qtrue;
      }
      Com_sprintf(
          voiceChats[voiceChatList->numVoiceChats]
              .chats[voiceChats[voiceChatList->numVoiceChats].numSounds],
...
void CG_LoadVoiceChats(void) {
  ...
  CG_ParseVoiceChats("scripts/female1.voice", &voiceChatLists[0],
                     MAX_VOICECHATS);
  CG_ParseVoiceChats("scripts/female2.voice", &voiceChatLists[1],
                     MAX_VOICECHATS);
  CG_ParseVoiceChats("scripts/female3.voice", &voiceChatLists[2],
                     MAX_VOICECHATS);
...
=================
*/
int CG_GetVoiceChat(voiceChatList_t *voiceChatList, const char *id,
                    sfxHandle_t *snd, char **chat) {
  int i, rnd;
...
voiceChatList_t *CG_VoiceChatListForClient(int clientNum) {
  ...
  for (k = 0; k < 2; k++) {
    if (k == 0) {
      if (ci->headModelName[0] == '*') {
        Com_sprintf(headModelName, sizeof(headModelName), "%s/%s",
                    ci->headModelName + 1, ci->headSkinName);
      } else {
        Com_sprintf(headModelName, sizeof(headModelName), "%s/%s",
      ...
      if (ci->headModelName[0] == '*') {
        Com_sprintf(headModelName, sizeof(headModelName), "%s",
                    ci->headModelName + 1);
      } else {
        Com_sprintf(headModelName, sizeof(headModelName), "%s",
                    ci->headModelName);
    ...
    for (i = 0; i < MAX_HEADMODELS; i++) {
      if (!strlen(headModelVoiceChat[i].headmodel)) {
        ...
        if (voiceChatNum == -1)
          break;
        Com_sprintf(headModelVoiceChat[i].headmodel,
                    sizeof(headModelVoiceChat[i].headmodel), "%s",
                    headModelName);
  ...
  for (j = 0; j < MAX_HEADMODELS; j++) {
    if (!strlen(headModelVoiceChat[j].headmodel)) {
      Com_sprintf(headModelVoiceChat[j].headmodel,
                  sizeof(headModelVoiceChat[j].headmodel), "%s", headModelName);
...
void CG_AddBufferedVoiceChat(bufferedVoiceChat_t *vchat) {
  ...
  memcpy(&voiceChatBuffer[cg.voiceChatBufferIn], vchat,
         sizeof(bufferedVoiceChat_t));
...
void CG_VoiceChat(int mode) {
  ...
  if (cg_noTaunt.integer != 0) {
    if (strequals(cmd, VOICECHAT_KILLINSULT) ||
        strequals(cmd, VOICECHAT_TAUNT) ||
        strequals(cmd, VOICECHAT_DEATHINSULT) ||
        strequals(cmd, VOICECHAT_KILLGAUNTLET) ||
...
static void CG_ServerCommand(void) {
...
#ifdef MISSIONPACK
    cmd = CG_Argv(1); // yes, this is obviously a hack, but so is the way we
                      // hear about votes passing or failing
    if (Q_strequaln(cmd, "vote failed", 11) ||
        Q_strequaln(cmd, "team vote failed", 16)) {
      trap_S_StartLocalSound(cgs.media.voteFailed, CHAN_ANNOUNCER);
    } else if (Q_strequaln(cmd, "vote passed", 11) ||
               Q_strequaln(cmd, "team vote passed", 16)) {
  ...
  // challenge completed is determened by the server. A client should consider
  // this message valid:
  if (strequals(cmd, "ch")) {
    CG_ParseChallenge();
    return;
