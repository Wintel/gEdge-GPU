...
void UI_LoadArenas(void) {
  ...
  for (n = 0; n < ui_numArenas; n++) {
    ...
    // rfactory changes
    // Changed RD
    uiInfo.mapList[uiInfo.mapCount].botName =
        String_Alloc(Info_ValueForKey(ui_arenaInfos[n], "bots"));
    uiInfo.mapList[uiInfo.mapCount].special =
        String_Alloc(Info_ValueForKey(ui_arenaInfos[n], "special"));
    uiInfo.mapList[uiInfo.mapCount].fraglimit =
        atoi(Info_ValueForKey(ui_arenaInfos[n], "fraglimit"));
    // end changed RD
    ...
    if (*type) {
      // Changed RD
      trap_Cvar_VariableStringBuffer("ui_SpecialGame", specialgame,
                                     sizeof(specialgame));
      //			if (!specialgame[0])
      //			{
      if (strstr(type, "ffa")) {
