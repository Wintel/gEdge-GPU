...
static void UI_LegsSequencing(playerInfo_t *pi) {
  ...
  if (pi->legsAnimationTimer > 0) {
    if (currentAnim == LEGS_JUMP) {
      jumpHeight =
          JUMP_HEIGHT *
          sin(M_PI * (UI_TIMER_JUMP - pi->legsAnimationTimer) / UI_TIMER_JUMP);
    }
    return;
  }
...
static void UI_PlayerAnglesBust(playerInfo_t *pi, vec3_t legs[3],
  ...
  //
  {
    ...
    if (uiInfo.uiDC.realTime >= pi->headEndTime) {
      ...
      if (pi->headEndPitch > 8)
        pi->headEndPitch = 8;
      if ((pi->headEndYaw < 215 && pi->headEndYaw > 155) &&
          pi->headEndPitch < 8)
        randroll = 8;

      pi->headEndRoll = randroll * cos(crandom() * M_PI);
    }
    frac = (uiInfo.uiDC.realTime - pi->headStartTime) /
           (float)(pi->headEndTime - pi->headStartTime);
    frac = frac * frac * (3 - 2 * frac);
    idler[YAW] = pi->headStartYaw + (pi->headEndYaw - pi->headStartYaw) * frac;
    idler[PITCH] =
        pi->headStartPitch + (pi->headEndPitch - pi->headStartPitch) * frac;
    idler[ROLL] =
        pi->headStartRoll + (pi->headEndRoll - pi->headStartRoll) * frac;
  ...
  /*	if ( ( pi->legsAnim & ~ANIM_TOGGLEBIT ) != LEGS_IDLE
                  || ( pi->torsoAnim & ~ANIM_TOGGLEBIT ) != TORSO_STAND  ) {
                  // if not standing still, always point all in the same
     direction pi->torso.yawing = qtrue;	// always center
                  pi->torso.pitching = qtrue;	// always center
                  pi->legs.yawing = qtrue;	// always center
          }
  */
  ...
  // torso
  UI_SwingAngles(torsoAngles[YAW], 11, 30, 0.03f, &pi->torso.yawAngle,
                 &pi->torso.yawing);
  // UI_SwingAngles( legsAngles[YAW], 40, 90, 0.03f, &pi->legs.yawAngle,
  // &pi->legs.yawing );
  ...
  UI_SwingAngles(dest, 45, 60, 0.03f, &pi->torso.pitchAngle,
                 &pi->torso.pitching);
  torsoAngles[PITCH] = pi->torso.pitchAngle;
...
void UI_DrawPlayer(float x, float y, float w, float h, playerInfo_t *pi,
  ...
  if (dp_realtime <= pi->muzzleFlashTime) {
    ...
    if (pi->flashDlightColor[0] || pi->flashDlightColor[1] ||
      ...
      trap_R_AddLightToScene(flash.origin, 200 + (rand() & 31),
                             pi->flashDlightColor[0], pi->flashDlightColor[1],
                             pi->flashDlightColor[2]);
    }
  }
...
===============
*/
void UI_DrawPlayerII(float x, float y, float w, float h, playerInfo_t *pi,
                     int time) {
  refdef_t refdef;
  refEntity_t legs;
  refEntity_t torso;
...
// only their bust w/ head, focused on head, for less polygons showing in the
// menu when we select maps and things
#define FOCUS_DISTANCE 512
float focusDist;
float forwardScale, sideScale;
void UI_DrawPlayersBust(float x, float y, float w, float h, playerInfo_t *pi,
                        int time) {
  refdef_t refdef;
  refEntity_t legs;
  refEntity_t torso;
  ...
  */
  ...
  // what I don't understand about the accent light? THE RENDERER ONLY DRAWS ONE
  // OF THEM.
...
void UI_DrawPlayerOC(float x, float y, float w, float h, playerInfo_t *pi,
                     int time) {
  refdef_t refdef;
  refEntity_t legs;
  refEntity_t torso;
...
static qboolean UI_RegisterClientSkin(playerInfo_t *pi, const char *modelName,
  ...
  if (teamName && *teamName) {
    Com_sprintf(filename, sizeof(filename),
                "models/players/%s/%s/lower_%s.skin", modelName, teamName,
                skinName);
  } else {
  ...
  if (!pi->legsSkin) {
    if (teamName && *teamName) {
      Com_sprintf(filename, sizeof(filename),
                  "models/players/characters/%s/%s/lower_%s.skin", modelName,
                  teamName, skinName);
    } else {
      Com_sprintf(filename, sizeof(filename),
                  "models/players/characters/%s/lower_%s.skin", modelName,
                  skinName);
    }
    pi->legsSkin = trap_R_RegisterSkin(filename);
  }
  ...
  if (teamName && *teamName) {
    Com_sprintf(filename, sizeof(filename),
                "models/players/%s/%s/upper_%s.skin", modelName, teamName,
                skinName);
  } else {
    Com_sprintf(filename, sizeof(filename), "models/players/%s/upper_%s.skin",
                modelName, skinName);
  }
  ...
  if (!pi->torsoSkin) {
    if (teamName && *teamName) {
      Com_sprintf(filename, sizeof(filename),
                  "models/players/characters/%s/%s/upper_%s.skin", modelName,
                  teamName, skinName);
    } else {
      Com_sprintf(filename, sizeof(filename),
                  "models/players/characters/%s/upper_%s.skin", modelName,
                  skinName);
    }
    pi->torsoSkin = trap_R_RegisterSkin(filename);
  }

  if (UI_FindClientHeadFile(filename, sizeof(filename), teamName, headModelName,
                            headSkinName, "head", "skin")) {
...
qboolean UI_RegisterClientModelname(playerInfo_t *pi, const char *modelSkinName,
  ...
  if (!UI_RegisterClientSkin(pi, modelName, skinName, headModelName,
    ...
    if (!UI_RegisterClientSkin(pi, modelName, "default", headModelName,
                               "default", teamName)) {
