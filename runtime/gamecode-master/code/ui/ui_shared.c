...
void Window_Paint(Window *w, float fadeAmount, float fadeClamp,
  ...
  if (w->style == WINDOW_STYLE_FILLED) {
    ...
    if (w->flags & WINDOW_FORECOLORSET) {
      Fade(&w->flags, &w->foreColor[3], fadeClamp, &w->nextTime, fadeCycle,
           qtrue, fadeAmount);
...
void Item_SetScreenCoords(itemDef_t *item, float x, float y) {
  ...
  // %f\n%ix%i = newres\n",realVidWidth, realVidHeight, xscale, yscale, resbias,
  // adjustx, adjusty, newresx, newresy);
  {
    if (item->scralign)
  ...
  item->window.rect.x =
      x + (adjustx * item->scralignfactor) + item->window.rectClient.x;
  item->window.rect.y =
      y + (adjusty * item->scralignfactor) + item->window.rectClient.y;
...
void Menu_TransitionItemByName(itemDef_t *callitem, menuDef_t *menu,
                               const char *p, rectDef_t rectFrom,
                               rectDef_t rectTo, int time, float amt,
                               qboolean xflag) {
  // void Menu_TransitionItemByName(menuDef_t *menu, const char *p, rectDef_t
  // rectFrom, rectDef_t rectTo, int time, float amt) {
  itemDef_t *item;
  int i;
  int count = Menu_ItemsMatchingGroup(menu, p);
  for (i = 0; i < count; i++) {
...
void Script_Transition(itemDef_t *item, char **args) {
  ...
  if (String_Parse(args, &name)) {
    if (Rect_Parse(args, &rectFrom) && Rect_Parse(args, &rectTo) &&
      ...
      // Changed RD
      Menu_TransitionItemByName(item, item->parent, name, rectFrom, rectTo,
                                time, amt, qfalse);
      // end changed RD
    }
  }
...
qboolean Item_Checkbox_HandleKey(itemDef_t *item, int key) {
  ...
  if (Rect_ContainsPoint(&item->window.rect, DC->cursorx, DC->cursory) &&
    ...
    if (key == K_MOUSE1 || key == K_ENTER || key == K_JOY1 || key == K_MOUSE2 ||
        key == K_MOUSE3) {
...
qboolean Item_Combo_HandleKey(itemDef_t *item, int key) {
  ...
  if (comboPtr->combopen) {
    if (Rect_ContainsPoint(&comboPtr->ComboBox, DC->cursorx, DC->cursory) &&
        item->window.flags & WINDOW_HASFOCUS && item->cvar) {
      if (key == K_MOUSE1 || key == K_ENTER || key == K_JOY1 ||
        ...
        if (comboPtr->comboItem != -1) {
          ...
          if (!multiPtr->strDef) {
            DC->setCVar(
                item->cvar,
                va("%d", (int)multiPtr->cvarValue[comboPtr->comboItem]));
          } else {
  ...
  } else if (Rect_ContainsPoint(&Box, DC->cursorx, DC->cursory) &&
             item->window.flags & WINDOW_HASFOCUS && item->cvar) {
    if (key == K_MOUSE1 || key == K_ENTER || key == K_JOY1 || key == K_MOUSE2 ||
        key == K_MOUSE3) {
      multiDef_t *multiPtr = (multiDef_t *)item->multiData;
      if (multiPtr) {
        if (multiPtr->count > 0) {
          ...
          for (i = 0; i < multiPtr->count; i++) {
            ...
            if (t > w)
              w = t;
            t = DC->textHeight(multiPtr->cvarList[i], comboPtr->boxtextscale,
                               0);
            if (t > h)
              h = t;
          }
          ...
          comboPtr->ComboBox.y =
              item->window.rect.y + item->window.rect.h + comboPtr->offsety;
          comboPtr->ComboBox.w = w + 20;
          comboPtr->ComboBox.h = (h * multiPtr->count) +
                                 (multiPtr->count * BOXTEXTSTEP) +
                                 BOXTEXTOFFSET;
...
qboolean Item_TextField_HandleKey(itemDef_t *item, int key) {
  ...
  if (item->cvar) {
    ...
    if (key & K_CHAR_FLAG) {
      ...
      if (key == K_HOME) { // || ( tolower(key) == 'a' && trap_Key_IsDown(
                           // K_CTRL ) ) ) {
        item->cursorPos = 0;
      ...
      if (key ==
          K_END) { // ( tolower(key) == 'e' && trap_Key_IsDown( K_CTRL ) ) ) {
        item->cursorPos = len;
    ...
    if (key == K_ENTER || key == K_JOY1 || key == K_KP_ENTER ||
        key == K_ESCAPE || key == K_JOY4) {
      return qfalse;
    }
...
static void Scroll_ListBox_AutoFunc(void *p) {
  ...
  if (DC->realTime > si->nextScrollTime) {
    ...
    // this is done a bit sideways as the autoscroll "knows" that the item is a
    // listbox so it calls it directly
...
static void Scroll_ListBox_ThumbFunc(void *p) {
  ...
  if (si->item->window.flags & WINDOW_HORIZONTAL) {
    ...
    max = Item_ListBox_MaxScroll(si->item);
    //
    pos = (DC->cursory - r.y - DC->Assets.scrollbarsize / 2) * max /
          (r.h - DC->Assets.scrollbarsize);
    if (pos < 0) {
      pos = 0;
    } else if (pos > max) {
  ...
  if (item->window.flags & WINDOW_HASFOCUS && item->cvar &&
    ...
    if (key == K_MOUSE1 || key == K_ENTER || key == K_JOY1 || key == K_MOUSE2 ||
      ...
      if (editDef) {
        ...
        testRect.x -= value;
        testRect.w =
            (DC->Assets.sliderwidth + (float)DC->Assets.sliderthumbwidth / 2);
...
void Menus_HandleOOBClick(menuDef_t *menu, int key, qboolean down) {
  if (menu) {
    int i;
    // basically the behaviour we are looking for is if there are windows in the
    // stack.. see if the cursor is within any of them.. if not close them
    // otherwise activate them and pass the key on.. force a mouse move to
    // activate focus and script stuff
...
void Menu_HandleKey(menuDef_t *menu, int key, qboolean down) {
  ...
  if (g_editingField && down) {
    ...
    } else if (key == K_TAB || key == K_UPARROW || key == K_JOY29 ||
               key == K_JOY31 || key == K_DOWNARROW) {
      return;
    }
  }
            ...
            if (item->type == ITEM_TYPE_LISTBOX) {
              if (!(item->window.flags &
                    (WINDOW_LB_LEFTARROW | WINDOW_LB_RIGHTARROW |
                     WINDOW_LB_PGUP | WINDOW_LB_PGDN | WINDOW_LB_THUMB |
                     WINDOW_FOCUSDISABLE))) {
                Item_Action(item);
              }
            } else {
...
int Item_Text_AutoWrapped_Lines(itemDef_t *item) {
  ...
  while (p) {
    ...
    // TA: forceably split lines that are too long (where normal splitage has
    // failed)
...
static int checkCache(const char *text, rectDef_t *rect, int width,
                      int height) {
  int i;
  ...
  for (i = 0; i < MAX_AUTOWRAP_CACHE; i++) {
    ...
    if (rect->x != awc[i].rect.x || rect->y != awc[i].rect.y ||
...
void Item_TextField_Paint(itemDef_t *item) {
  ...
  if (item->window.flags & WINDOW_HASFOCUS && g_editingField) {
    ...
    DC->drawText(item->textRect.x + item->textRect.w + offset, item->textRect.y,
                 item->textscale, newColor, buff + editPtr->paintOffset, 0,
                 editPtr->maxPaintChars, item->textStyle);
  }
}
...
void Item_Checkbox_Paint(itemDef_t *item) {
  ...
  DC->drawHandlePic(x, y, DC->Assets.checkboxwidth, DC->Assets.checkboxheight,
                    (value != 0) ? DC->Assets.checkbox_sel
                                 : DC->Assets.checkbox);
...
void Item_Combo_Paint(itemDef_t *item) {
  ...
  DC->drawHandlePic(x, y, DC->Assets.combowidth, DC->Assets.comboheight,
                    (comboPtr->combopen) ? DC->Assets.combo_sel
                                         : DC->Assets.combo);
  ...
  if (item->cvar) {
    ...
    if (multiPtr) {
      ...
      DC->drawText(x, y, item->textscale, item->window.foreColor, textPtr, 0, 0,
                   item->textStyle);
    }
  }
}
  ...
  if (multiPtr) {
    ...
    for (i = 0; i < multiPtr->count; i++) {
      ...
      if (i == comboPtr->comboItem) {
        DC->drawText(x, y, comboPtr->boxtextscale, newColor,
                     multiPtr->cvarList[i], 0, 0, item->textStyle);
      } else {
        DC->drawText(x, y, comboPtr->boxtextscale, comboPtr->boxtextcolor,
                     multiPtr->cvarList[i], 0, 0, item->textStyle);
      }
      y += BOXTEXTSTEP;
    }
...
void Controls_GetConfig(void) {
  ...
  // Controls_GetCvarValue( "cg_autoswitch" ) ); s_controls.sensitivity.curvalue
  // = UI_ClampCvar( 2, 30, Controls_GetCvarValue( "sensitivity" ) );
  // s_controls.joyenable.curvalue    = UI_ClampCvar( 0, 1,
  // Controls_GetCvarValue( "in_joystick" ) ); s_controls.joythreshold.curvalue
  // = UI_ClampCvar( 0.05, 0.75, Controls_GetCvarValue( "joy_threshold" ) );
  // s_controls.freelook.curvalue     = UI_ClampCvar( 0, 1,
  // Controls_GetCvarValue( "cl_freelook" ) );
}
...
void Controls_SetDefaults(void) {
  ...
  // "sensitivity" ); s_controls.joyenable.curvalue    =
  // Controls_GetCvarDefault( "in_joystick" ); s_controls.joythreshold.curvalue
  // = Controls_GetCvarDefault( "joy_threshold" ); s_controls.freelook.curvalue
...
void Item_Slider_Paint(itemDef_t *item) {
  ...
  /*	if (item->window.flags & WINDOW_HASFOCUS) {
                  ...
                  lowLight[2] = 0.8 * parent->focusColor[2];
                  lowLight[3] = 0.8 * parent->focusColor[3];
                  LerpColor(parent->focusColor,lowLight,newColor,0.5+0.5*sin(DC->realTime
     / PULSE_DIVISOR)); } else { memcpy(&newColor, &item->window.foreColor,
     sizeof(vec4_t));
          } */
  ...
  DC->drawHandlePic(x, y, DC->Assets.sliderwidth, DC->Assets.sliderheight,
                    DC->Assets.sliderBar);
  ...
  if (item->window.flags & WINDOW_HASFOCUS) {
    DC->drawHandlePic(x - (DC->Assets.sliderthumbwidth / 2), y,
                      DC->Assets.sliderthumbwidth, DC->Assets.sliderthumbheight,
                      DC->Assets.sliderThumb_sel);
  } else {
    DC->drawHandlePic(x - (DC->Assets.sliderthumbwidth / 2), y,
                      DC->Assets.sliderthumbwidth, DC->Assets.sliderthumbheight,
...
void Item_Bind_Paint(itemDef_t *item) {
  ...
  if (item->text) {
    ...
    DC->drawText(x, y, item->textscale, newColor, g_nameBind1, 0, maxChars,
                 item->textStyle);
  }
}
  ...
  if (id != -1) {
    if (key == -1) {
      if (g_bindings[id].bind1 != -1 &&
          (item->bindtype == BIND_BOTH || item->bindtype == BIND_PRIMARY)) {
        DC->setBinding(g_bindings[id].bind1, "");
        g_bindings[id].bind1 = -1;
      }
      if (g_bindings[id].bind2 != -1 &&
          (item->bindtype == BIND_BOTH || item->bindtype == BIND_ALTERNATE)) {
        DC->setBinding(g_bindings[id].bind2, "");
        g_bindings[id].bind2 = -1;
      }
    } else if (g_bindings[id].bind1 == -1 && (item->bindtype == BIND_BOTH ||
                                              item->bindtype == BIND_PRIMARY)) {
      g_bindings[id].bind1 = key;
    } else if (g_bindings[id].bind2 == -1 &&
               (item->bindtype == BIND_ALTERNATE)) {
      g_bindings[id].bind2 = key;
    } else if (g_bindings[id].bind1 != key && g_bindings[id].bind2 == -1 &&
               item->bindtype == BIND_BOTH) {
      g_bindings[id].bind2 = key;
    } else {
      if (item->bindtype == BIND_BOTH) {
        DC->setBinding(g_bindings[id].bind1, "");
...
void Item_Model_Paint(itemDef_t *item) {
  ...
  if (modelPtr->backlerp > 1) {
    ...
    if ((modelPtr->frame - modelPtr->startframe) > modelPtr->numframes)
      modelPtr->frame =
          modelPtr->startframe +
          modelPtr->frame % modelPtr->numframes; // todo: ignoring loopframes

    modelPtr->oldframe += (backLerpWhole);
    if ((modelPtr->oldframe - modelPtr->startframe) > modelPtr->numframes)
      modelPtr->oldframe =
          modelPtr->startframe +
          modelPtr->oldframe % modelPtr->numframes; // todo: ignoring loopframes
...
void Item_ListBox_Paint(itemDef_t *item) {
  ...
  Fade(&item->window.flags, &item->fadeColor[3], parent->fadeClamp,
       &item->window.nextTime, parent->fadeCycle, qtrue, parent->fadeAmount);
  Fade(&item->window.flags, &item->window.foreColor[3], parent->fadeClamp,
       &item->window.nextTime, parent->fadeCycle, qtrue, parent->fadeAmount);
  Fade(&item->window.flags, &item->window.borderColor[3], parent->fadeClamp,
       &item->window.nextTime, parent->fadeCycle, qtrue, parent->fadeAmount);
  DC->setColor(item->fadeColor);
  // end changed RD

  // the listbox is horizontal or vertical and has a fixed size scroll bar going
  // either direction elements are enumerated from the DC and either text or
  // image handles are acquired from the DC as well textscale is used to size
  // the text, textalignx and textaligny are used to size image elements there
  // is no clipping available so only the last completely visible item is
  // painted
  ...
  if (item->window.flags & WINDOW_HORIZONTAL) {
    ...
    // bar
    x = item->window.rect.x + 1;
    y = item->window.rect.y + item->window.rect.h - DC->Assets.scrollbarsize -
        1;
    DC->drawHandlePic(x, y, DC->Assets.scrollbarsize, DC->Assets.scrollbarsize,
                      DC->Assets.scrollBarArrowLeft);
    x += DC->Assets.scrollbarsize - 1;
    size = item->window.rect.w - (DC->Assets.scrollbarsize * 2);
    DC->drawHandlePic(x, y, size + 1, DC->Assets.scrollbarsize,
                      DC->Assets.scrollBarHorz);
    x += size - 1;
    DC->drawHandlePic(x, y, DC->Assets.scrollbarsize, DC->Assets.scrollbarsize,
    ...
    DC->drawHandlePic(thumb, y, DC->Assets.scrollbarsize,
                      DC->Assets.scrollbarsize, DC->Assets.scrollBarThumb);
    //
    ...
    x = item->window.rect.x + item->window.rect.w - DC->Assets.scrollbarsize -
        1;
    y = item->window.rect.y + 1;
    DC->drawHandlePic(x, y, DC->Assets.scrollbarsize, DC->Assets.scrollbarsize,
    ...
    DC->drawHandlePic(x, y, DC->Assets.scrollbarsize, size + 1,
                      DC->Assets.scrollBarVert);
    y += size - 1;
    DC->drawHandlePic(x, y, DC->Assets.scrollbarsize, DC->Assets.scrollbarsize,
    ...
    DC->drawHandlePic(x, thumb, DC->Assets.scrollbarsize,
                      DC->Assets.scrollbarsize, DC->Assets.scrollBarThumb);
    ...
    if (listPtr->elementStyle == LISTBOX_IMAGE) {
      ...
      for (i = listPtr->startPos; i < count; i++) {
        ...
        if (i == item->cursorPos) {
          DC->fillRect(x + 2, y + 2,
                       item->window.rect.w - DC->Assets.scrollbarsize - 4,
...
void Item_OwnerDraw_Paint(itemDef_t *item) {
  ...
  if (DC->ownerDrawItem) {
    ...
    if (item->text) {
      Item_Text_Paint(item);
      if (item->text[0]) {
        // +8 is an offset kludge to properly align owner draw items that have
        // text combined with them
...
void Item_Paint(itemDef_t *item) {
  ...
  // DC->drawRect(item->window.rect.x, item->window.rect.y, item->window.rect.w,
  // item->window.rect.h, 1, red);
    ...
    for (i = 0; i < menu->itemCount; i++) {
      if (menu->items[i]->special == feeder) {
        Item_ListBox_HandleKey(menu->items[i], (down) ? K_DOWNARROW : K_UPARROW,
                               qtrue, qtrue);
        Item_ListBox_HandleKey(menu->items[i], (down) ? K_JOY31 : K_JOY29,
                               qtrue, qtrue); // leilei - gamepad support
...
void Menu_Paint(menuDef_t *menu, qboolean forcePaint) {
  ...
  if (menu->fullScreen) {
    ...
    // FIXME: make sure we have a default shader if fullscreen is set with no
    // background
...
qboolean ItemParse_asset_model(itemDef_t *item, int handle) {
  ...
  modelPtr->angle =
      0; // leilei - don't do this because it makes menu changing inconsistent
...
keywordHash_t itemParseKeywords[] = {
    ...
    {"scralignfactor", ItemParse_scralignfactor,
     NULL}, // leilei - scroll align adaptation factor
...
qboolean MenuParse_fullscreen(itemDef_t *item, int handle) {
  ...
  if (!PC_Int_Parse(handle,
                    (int *)&menu->fullScreen)) { // bk001206 - cast qboolean
    return qfalse;
  }
  return qtrue;
}
    ...
    if (Rect_ContainsPoint(&menu->window.rect, x, y)) {
      ...
      for (i = 0; i < menu->itemCount; i++) {
        ...
        if (menu->items[i]->window.flags & WINDOW_DECORATION ||
            menu->items[i]->window.flags & WINDOW_FOCUSDISABLE) {
          continue;
        }
