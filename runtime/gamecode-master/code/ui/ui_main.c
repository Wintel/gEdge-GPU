...
static const char *skillLevels[] = {"I Can Win", "Bring It On",
                                    "Hurt Me Plenty", "Hardcore", "Nightmare"};
 ...
 */
static const char *netSources[] = {"Local", "", "Internet", "Favorites"};
...
static const char *teamArenaGameTypes[] = {
    "FFA",   "TOURNAMENT", "SP",        "TEAM DM",     "CTF",
    "1FCTF", "OVERLOAD",   "HARVESTER", "ELIMINATION", "CTFELIMINATION",
    "LMS",   "DOUBLE D",   "DOMINATION"};

static int const numTeamArenaGameTypes =
    sizeof(teamArenaGameTypes) / sizeof(const char *);

static const int numServerFilters =
    sizeof(serverFilters) / sizeof(serverFilter_t);

static char *netnames[] = {"???", "UDP", NULL};

#ifndef MISSIONPACK // bk001206
static char quake3worldMessage[] =
    "Visit www.openarena.ws - News, Community, Events, Files";
#endif
...
void AssetCache(void) {
  ...
  // Changed RD
  uiInfo.uiDC.Assets.scrollBarHorz =
      trap_R_RegisterShaderNoMip(ASSET_SCROLLBAR);
  uiInfo.uiDC.Assets.scrollBarVert =
      trap_R_RegisterShaderNoMip(ASSET_SCROLLBAR);
  // end changed RD
  ...
  uiInfo.newHighScoreSound =
      trap_S_RegisterSound("sound/feedback/voc_newhighscore.wav", qfalse);
  // Changed RD
  ...
  if (text) {
    ...
    while (s && *s && count < len) {
      if (Q_IsColorString(s)) {
        ...
        glyph = &font->glyphs[(
            int)*s]; // TTimo: FIXME: getting nasty warnings without the cast,
    ...
    while (s && *s && count < len) {
      ...
      if (Q_IsColorString(s)) {
        ...
        if (style == ITEM_TEXTSTYLE_SHADOWED ||
          ...
          Text_PaintChar(x - 3, y - yadj - 3, glyph->imageWidth + 6,
                         glyph->imageHeight + 6, useScale, glyph->s, glyph->t,
                         glyph->s2, glyph->t2, glyph->glyph);

          trap_R_SetColor(outer);
          Text_PaintChar(x - 1, y - yadj - 1, glyph->imageWidth + 2,
                         glyph->imageHeight + 2, useScale, glyph->s, glyph->t,
                         glyph->s2, glyph->t2, glyph->glyph);

          trap_R_SetColor(inner);
          Text_PaintChar(x - 0.5, y - yadj - 0.5, glyph->imageWidth + 1,
                         glyph->imageHeight + 1, useScale, glyph->s, glyph->t,
                         glyph->s2, glyph->t2, glyph->glyph);
      ...
      if (Q_IsColorString(s)) {
        ...
        if (count == cursorPos &&
          ...
          Text_PaintChar(x, y - yadj, glyph2->imageWidth, glyph2->imageHeight,
                         useScale, glyph2->s, glyph2->t, glyph2->s2, glyph2->t2,
                         glyph2->glyph);
        }
...
void _UI_Refresh(int realtime) {
  ...
  if (Menu_Count() > 0 && !trap_Cvar_VariableValue("ui_loading")) {
    // end changed RD
    UI_DrawHandlePic(uiInfo.uiDC.cursorx - 16, uiInfo.uiDC.cursory - 16, 32, 32,
...
void UI_Load(void) {
  ...
  if (!ui_introPlayed.integer) { // load a menu with little overhead with
                                 // information about legal notices etc
    ...
    if (uiInfo.teamList[i].cinematic >= -2) {
      if (uiInfo.teamList[i].cinematic == -1) {
        uiInfo.teamList[i].cinematic =
            trap_CIN_PlayCinematic(va("%s.roq", uiInfo.teamList[i].imageName),
                                   0, 0, 0, 0, (CIN_loop | CIN_silent));
      }
      if (uiInfo.teamList[i].cinematic >= 0) {
        trap_CIN_RunCinematic(uiInfo.teamList[i].cinematic);
        trap_CIN_SetExtents(uiInfo.teamList[i].cinematic, rect->x, rect->y,
                            rect->w, rect->h);
...
static void UI_DrawTeamMember(rectDef_t *rect, float scale, vec4_t color,
  ...
  // 2..NumCharacters - Bot
  int value =
      trap_Cvar_VariableValue(va(blue ? "ui_blueteam%i" : "ui_redteam%i", num));
  const char *text;
  if (value <= 0) {
    text = "Closed";
  } else if (value == 1) {
...
static void UI_DrawPlayerModel2(rectDef_t *rect) {
  ...
  if (updateModel) {
    ...
    UI_PlayerInfo_SetInfo(&uiInfo.info, trap_Cvar_VariableValue("ui_LowerAnim"),
                          trap_Cvar_VariableValue("ui_UpperAnim"), viewangles,
                          moveangles, trap_Cvar_VariableValue("ui_Weapon"),
                          qfalse);
    // end changed RD
    updateModel = qfalse;
  }

  UI_DrawPlayerII(rect->x, rect->y, rect->w, rect->h, &info,
                  uiInfo.uiDC.realTime / 2);
}
...
static void UI_DrawOC(rectDef_t *rect) {
  ...
  UI_DrawPlayerOC(rect->x, rect->y, rect->w, rect->h, &info,
                  uiInfo.uiDC.realTime / 2);
}
...
static void UI_DrawNetSource(rectDef_t *rect, float scale, vec4_t color,
  ...
  // Changed RD
  Text_Paint(rect->x, rect->y, scale, color,
             va("%s", netSources[ui_netSource.integer]), 0, 0, textStyle);
  // end changed RD
  // Text_Paint(rect->x, rect->y, scale, color, va("Source: %s",
...
static void UI_DrawOpponent2(rectDef_t *rect) {
  ...
  UI_DrawPlayerII(rect->x, rect->y, rect->w, rect->h, &info2,
                  uiInfo.uiDC.realTime / 2);
}
...
static void UI_DrawPlayerPortrait(rectDef_t *rect) {
  ...
  if (randomtaunter < 1) {
    ...
    viewangles[PITCH] = 0;
    viewangles[ROLL] = 0;
    UI_PlayerInfo_SetInfo(&info, LEGS_IDLE, TORSO_GESTURE, viewangles,
                          vec3_origin, WP_MACHINEGUN, qfalse);
  }

  if (updateModel) {
    memset(&info, 0, sizeof(playerInfo_t));
    // viewangles[YAW]   = 180 - 10 + sin(uiInfo.uiDC.realTime * 55 /
    // PULSE_DIVISOR); // (sin(uiInfo.uiDC.realTime  * 75)  * 75);
    viewangles[YAW] = 180;
    viewangles[PITCH] = 0;
    viewangles[ROLL] = 0;
    VectorClear(moveangles);
  ...
  UI_DrawPlayersBust(rect->x, rect->y, rect->w, rect->h, &info,
                     uiInfo.uiDC.realTime / 2);
}
...
static void UI_DrawAllMapsSelection(rectDef_t *rect, float scale, vec4_t color,
  ...
  int map = (net) ? ui_currentNetMap.integer : ui_currentMap.integer;
  if (map >= 0 && map < uiInfo.mapCount) {
    Text_Paint(rect->x, rect->y, scale, color, uiInfo.mapList[map].mapName, 0,
               0, textStyle);
  }
}
...
static void UI_DrawMenuMapName(rectDef_t *rect, float scale, vec4_t color,
                               int textStyle) {
  Text_Paint(rect->x, rect->y, scale, color, uiInfo.mapList[2].mapLoadName, 0,
             0, textStyle);
  ...
  if (uiInfo.serverStatus.refreshActive) {
    ...
    Text_Paint(rect->x, rect->y, scale, newColor,
               va("Getting info for %d servers (ESC to cancel)",
                  trap_LAN_GetServerCount(ui_netSource.integer)),
               0, 0, textStyle);
    // Changed RD
...
static void UI_DrawServerMOTD(rectDef_t *rect, float scale, vec4_t color) {
  if (uiInfo.serverStatus.motdLen) {
    ...
    Text_Paint_Limit(
        ...
        color, &uiInfo.serverStatus.motd[uiInfo.serverStatus.motdOffset], 0, 0);
    if (uiInfo.serverStatus.motdPaintX2 >= 0) {
      float maxX2 = rect->x + rect->w - 2;
      Text_Paint_Limit(
          &maxX2, uiInfo.serverStatus.motdPaintX2, rect->y + rect->h - 3, scale,
          color, uiInfo.serverStatus.motd, 0, uiInfo.serverStatus.motdOffset);
    }
...
static void UI_DrawKeyBindStatus(rectDef_t *rect, float scale, vec4_t color,
                                 int textStyle, int ownerDrawFlags) {
  if (Display_KeyBindPending()) {
    Text_Paint(rect->x, rect->y, scale, color,
    ...
    if (!(ownerDrawFlags & UI_BIND2CLICK)) {
      Text_Paint(rect->x, rect->y, scale, color,
                 "Press ENTER or CLICK to change, Press BACKSPACE to clear", 0,
                 0, textStyle);
    }
  }
}
...
static void UI_DrawGLInfo(rectDef_t *rect, float scale, vec4_t color,
  ...
  // in TA this was not directly crashing, but displaying a nasty broken shader
  // right in the middle brought down the string size to 1024, there's not much
  // that can be shown on the screen anyway
  ...
  while (i < numLines) {
    ...
    if (i < numLines) {
      Text_Paint(rect->x + rect->w / 2, y, scale, color, lines[i++], 0, 20,
                 textStyle);
    }
    y += 10;
...
static void UI_OwnerDraw(float x, float y, float w, float h, float text_x,
  ...
  case UI_BLUETEAM4:
  case UI_BLUETEAM5:
    UI_DrawTeamMember(&rect, scale, color, qtrue, ownerDraw - UI_BLUETEAM1 + 1,
                      textStyle);
    break;
  case UI_REDTEAM1:
  case UI_REDTEAM2:
  ...
  case UI_REDTEAM4:
  case UI_REDTEAM5:
    UI_DrawTeamMember(&rect, scale, color, qfalse, ownerDraw - UI_REDTEAM1 + 1,
                      textStyle);
    break;
  case UI_NETSOURCE:
  ...
  while (flags) {
    ...
    if (flags & UI_SHOW_FFA) {
      if (trap_Cvar_VariableValue("g_gametype") != GT_FFA ||
          trap_Cvar_VariableValue("g_gametype") != GT_LMS) {
        vis = qfalse;
      }
      flags &= ~UI_SHOW_FFA;
    }
    ...
    if (flags & UI_SHOW_LEADER) {
      // these need to show when this client can give orders to a player or a
      // group
      if (!uiInfo.teamLeader) {
        vis = qfalse;
      } else {
    ...
    if (flags & UI_SHOW_NOTLEADER) {
      // these need to show when this client is assigning their own status or
    ...
    if (flags & UI_SHOW_ANYTEAMGAME) {
      if (uiInfo.gameTypes[ui_gameType.integer].gtEnum <= GT_TEAM ||
          uiInfo.gameTypes[ui_gameType.integer].gtEnum != GT_LMS) {
        vis = qfalse;
      }
    ...
    if (key == K_MOUSE2) {
      do {
        ...
        if (ui_gameType.integer < 0) {
          ...
          uiInfo.maskGameTypes[uiInfo.gameTypes[ui_gameType.integer].gtEnum]);
    } else {
      do {
        ui_gameType.integer++;
    ...
    if (key == K_MOUSE2) {
      do {
        ...
        if (ui_netGameType.integer < 0) {
          ...
          uiInfo
              .maskGameTypes[uiInfo.gameTypes[ui_netGameType.integer].gtEnum]);
    } else {
      do {
        ui_netGameType.integer++;
...
static void UI_StartSkirmish(qboolean next, char *name) {
  ...
  if (g == GT_TOURNAMENT) {
    ...
    Com_sprintf(buff, sizeof(buff),
                "wait ; addbot %s %f "
                ", %i \n",
                uiInfo.mapList[ui_currentMap.integer].opponentName, skill,
                delay);
    trap_Cmd_ExecuteText(EXEC_APPEND, buff);
  } else if (g == GT_FFA) { // leilei - parse the opponentname as a list of bots
                            // instead like q3_ui's arena parsing
    char *p;
    char *bot;
    const char *botInfo;
    ...
    Q_strncpyz(bots, uiInfo.mapList[ui_currentMap.integer].opponentName,
               sizeof(bots));
    ...
    for (i = 0; i < uiInfo.mapList[ui_currentMap.integer].teamMembers - 1;
         i++) {
      Com_sprintf(buff, sizeof(buff), "addbot %s %f %s %i %s\n",
                  UI_AIFromName(uiInfo.teamList[k].teamMembers[i]), skill,
                  (g == GT_FFA) ? "" : "Red", delay,
...
static void UI_RunMenuScript(char **args) {
  ...
  if (String_Parse(args, &name)) {
    if (Q_strequal(name, "StartServer")) {
      ...
      trap_Cvar_SetValue(
          "g_gametype",
          Com_Clamp(0, 13, uiInfo.gameTypes[ui_netGameType.integer].gtEnum));
      trap_Cvar_Set("g_redTeam", UI_Cvar_VariableString("ui_teamName"));
      trap_Cvar_Set("g_blueTeam", UI_Cvar_VariableString("ui_opponentName"));
      ...
      if (ui_netGameType.integer >= 0 &&
          ui_netGameType.integer < uiInfo.numGameTypes) {
        trap_Cmd_ExecuteText(
            EXEC_APPEND, va("callvote g_gametype %i\n",
      ...
      if (String_Parse(args, &orders)) {
        ...
        if (selectedPlayer < uiInfo.myTeamCount) {
          ...
          for (i = 0; i < uiInfo.myTeamCount; i++) {
            if (Q_strequal(UI_Cvar_VariableString("name"),
                           uiInfo.teamNames[i])) {
              continue;
            }
            strcpy(buff, orders);
...
qboolean UI_hasSkinForBase(const char *base, const char *team) {
  ...
  Com_sprintf(test, sizeof(test), "models/players/%s/%s/lower_default.skin",
              base, team);
  ...
  Com_sprintf(test, sizeof(test),
              "models/players/characters/%s/%s/lower_default.skin", base, team);
    ...
    for (j = 0; j < TEAM_MEMBERS; j++) {
      if (uiInfo.teamList[tIndex].teamMembers[j] != NULL) {
        if (uiInfo.characterList[i].reference &
            (1 << tIndex)) { // &&
                             // Q_strequal(uiInfo.teamList[tIndex].teamMembers[j],
                             // uiInfo.characterList[i].name)) {
  ...
  for (j = 0; j < TEAM_MEMBERS; j++) {
    for (k = 0; k < uiInfo.aliasCount; k++) {
      if (uiInfo.aliasList[k].name != NULL) {
        if (Q_strequal(uiInfo.teamList[tIndex].teamMembers[j],
                       uiInfo.aliasList[k].name)) {
          for (i = 0; i < uiInfo.characterCount; i++) {
            if (uiInfo.characterList[i].headImage != -1 &&
                uiInfo.characterList[i].reference & (1 << tIndex) &&
                Q_strequal(uiInfo.aliasList[k].ai,
                           uiInfo.characterList[i].name)) {
  ...
  for (i = 0; i < count; i++) {
    ...
    if (ping > 0 || ui_netSource.integer == AS_FAVORITES) {
      ...
      // Changed RD
        ...
        if (Q_stricmp(Info_ValueForKey(info, "game"),
                      UI_Cvar_VariableString("ui_CustomServer")) != 0) {
          trap_LAN_MarkServerVisible(ui_netSource.integer, i, qfalse);
          continue;
        }
      }
...
static void UI_BuildFindPlayerList(qboolean force) {
  ...
  for (i = 0; i < MAX_SERVERSTATUSREQUESTS; i++) {
      ...
      if (UI_GetServerStatusInfo(uiInfo.pendingServerStatus.server[i].adrstr,
        ...
        for (j = 0; j < info.numLines; j++) {
          ...
          if (stristr(name, uiInfo.findPlayerName)) {
            // add to found server list if we have space (always leave space for
            // a line with the number found)
    ...
    if (!uiInfo.pendingServerStatus.server[i].valid ||
      ...
      // reuse pending slot
      uiInfo.pendingServerStatus.server[i].valid = qfalse;
      // if we didn't try to get the status of all servers in the main browser
      // yet
    ...
    if (index >= 0 && index < uiInfo.serverStatus.numDisplayServers) {
      ...
      case SORT_CLIENTS:
        Com_sprintf(clientBuff, sizeof(clientBuff), "%s (%s)",
                    trap_Cvar_VariableValue("ui_humansonly")
                        ? Info_ValueForKey(info, "g_humanplayers")
  ...
  if (feederID == FEEDER_HEADS) {
    ...
    if (feederID == FEEDER_MAPS) {
      ...
      uiInfo.mapList[ui_currentMap.integer].cinematic = trap_CIN_PlayCinematic(
          va("%s.roq", uiInfo.mapList[ui_currentMap.integer].mapLoadName), 0, 0,
...
static qboolean Team_Parse(char **p) {
  ...
  while (1) {
    ...
    if (token[0] == '{') {
      ...
      uiInfo.teamList[uiInfo.teamCount].teamIcon = trap_R_RegisterShaderNoMip(
          uiInfo.teamList[uiInfo.teamCount].imageName);
      uiInfo.teamList[uiInfo.teamCount].teamIcon_Metal =
          trap_R_RegisterShaderNoMip(
              va("%s_metal", uiInfo.teamList[uiInfo.teamCount].imageName));
      uiInfo.teamList[uiInfo.teamCount].teamIcon_Name =
          trap_R_RegisterShaderNoMip(
              va("%s_name", uiInfo.teamList[uiInfo.teamCount].imageName));
      ...
      for (i = 0; i < TEAM_MEMBERS; i++) {
        ...
        if (!String_Parse(p,
                          &uiInfo.teamList[uiInfo.teamCount].teamMembers[i])) {
          return qfalse;
        }
      }
    ...
    if (token[0] == '{') {
      ...
      if (tempStr && Q_strequal(tempStr, "female")) {
        uiInfo.characterList[uiInfo.characterCount].base =
            String_Alloc(va("kyonshi"));
      } else if (tempStr && Q_strequal(tempStr, "male")) {
        uiInfo.characterList[uiInfo.characterCount].base =
            String_Alloc(va("sergei"));
      } else {
        uiInfo.characterList[uiInfo.characterCount].base =
...
static qboolean Alias_Parse(char **p) {
  ...
  while (1) {
    ...
    if (token[0] == '{') {
      ...
      // a - all purpose, d - defense, o - offense
    ...
    if (token[0] == '{') {
      ...
      while (1) {
        ...
        if (token[0] >= '0' && token[0] <= '9') {
          ...
          if (!Int_Parse(p, &uiInfo.mapList[uiInfo.mapCount]
                                 .timeToBeat[token[0] - 0x30])) {
            return qfalse;
          }
        } else {
          break;
      ...
      // mapList[mapCount].imageName = String_Alloc(va("levelshots/%s",
      // mapList[mapCount].mapLoadName)); if (uiInfo.mapCount == 0) {
      //  only load the first cinematic, selection loads the others
      //   uiInfo.mapList[uiInfo.mapCount].cinematic =
      //   trap_CIN_PlayCinematic(va("%s.roq",uiInfo.mapList[uiInfo.mapCount].mapLoadName),
      //   qfalse, qfalse, qtrue, 0, 0, 0, 0);
      // }
      uiInfo.mapList[uiInfo.mapCount].cinematic = -1;
      // uiInfo.mapList[uiInfo.mapCount].levelShot =
      // trap_R_RegisterShaderNoMip(va("levelshots/%s_small",
      ...
      uiInfo.mapList[uiInfo.mapCount].levelShot = trap_R_RegisterShaderNoMip(
...
static void UI_BuildQ3Model_List(void) {
  ...
  numdirs = trap_FS_GetFileList("models/players", "/", dirlist,
                                32768); // upped from 2048
  dirptr = dirlist;
  for (i = 0; i < numdirs && uiInfo.q3HeadCount < MAX_PLAYERMODELS;
    ...
    for (j = 0; j < numfiles && uiInfo.q3HeadCount < MAX_PLAYERMODELS;
      ...
      if (Q_stricmpn(skinname, "icon_", 5) == 0 &&
          !(Q_stricmp(skinname, "icon_blue") == 0 ||
            Q_stricmp(skinname, "icon_red") == 0))
      // if (Q_stricmpn(skinname, "icon_", 5) == 0)	// leilei - show red
      // blue anyway

      {
        ...
        if (!dirty) {
          Com_sprintf(uiInfo.q3HeadNames[uiInfo.q3HeadCount],
                      sizeof(uiInfo.q3HeadNames[uiInfo.q3HeadCount]), "%s",
                      scratch);
          uiInfo.q3HeadIcons[uiInfo.q3HeadCount++] = trap_R_RegisterShaderNoMip(
              va("models/players/%s/%s", dirptr, skinname));
        }
      }
    }
    ...
    for (j = 0; j < numfiles && uiInfo.q3HeadCount2 < MAX_PLAYERMODELS;
         j++, fileptr += filelen + 1) {
      filelen = strlen(fileptr);
...

      {
        ...
        if (!dirty) {
          Com_sprintf(uiInfo.q3HeadNames2[uiInfo.q3HeadCount2],
                      sizeof(uiInfo.q3HeadNames2[uiInfo.q3HeadCount2]), "%s",
                      scratch);
          uiInfo.q3HeadIcons[uiInfo.q3HeadCount2++] =
              trap_R_RegisterShaderNoMip(
                  va("models/players/%s/%s", dirptr, skinname));
        }
      }
...
void _UI_KeyEvent(int key, qboolean down) {
  ...
  if (Menu_Count() > 0) {
    menuDef_t *menu = Menu_GetFocused();
    if (menu) {
      if ((key == K_ESCAPE || key == K_JOY3) && down &&
    ...
    if (width > xmax) {
      ...
      *s2 = '\0';
      ...
      if (c_bcp == '\0') {
        ...
        s2++;
        if (*s2 !=
            '\0') // if we are printing an overflowing line we have s2 == s3
...
static void UI_DisplayDownloadInfo(const char *downloadName, float centerPoint,
  ...
  if (downloadCount < 4096 || !downloadTime) {
    ...
    if (downloadSize && xferRate) {
      ...
      Text_PaintCenter(leftWidth, yStart + 216, scale, colorWhite, dlTimeBuf,
                       0);
      Text_PaintCenter(leftWidth, yStart + 160, scale, colorWhite,
                       va("(%s of %s copied)", dlSizeBuf, totalSizeBuf), 0);
    } else {
    ...
    if (xferRate) {
      Text_PaintCenter(leftWidth, yStart + 272, scale, colorWhite,
...
void UI_DrawConnectScreen(qboolean overlay) {
  ...
  if (Q_strequal(cstate.servername, "localhost")) {
    Text_PaintCenter(centerPoint, yStart + 48, scale, colorWhite,
                     va("Starting up..."), ITEM_TEXTSTYLE_SHADOWEDMORE);
  } else {
  ...
  if (ui_netSource.integer == AS_GLOBAL) {
    ...
    if (strlen(ptr)) {
      trap_Cmd_ExecuteText(EXEC_NOW,
                           va("globalservers 0 %s full empty\n", ptr));
    } else {
      trap_Cmd_ExecuteText(EXEC_NOW,
                           va("globalservers 0 %d full empty\n",
                              (int)trap_Cvar_VariableValue("protocol")));
    }
  }
}
