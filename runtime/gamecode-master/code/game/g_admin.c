...
g_admin_cmd_t g_admin_cmds[] = {
    ...
    {"lock", G_admin_lock, "K", "lock a team to prevent anyone from joining it",
     "[^3a|h^7]"},
    // KK-OAX
    {"map", G_admin_map, "M", "load a map", "[^3mapname^7]"},

    {"mute", G_admin_mute, "m", "mute a player", "[^3name|slot#^7]"},
    ...
    // KK-OAX
    {"orient", G_admin_orient, "d", "orient a player after a !disorient",
     "[^3name|slot#^7]"},
    ...
    {"unban", G_admin_unban, "b",
     "unbans a player specified by the slot as seen in showbans", "[^3ban#^7]"},

    {"unlock", G_admin_unlock, "K", "unlock a locked team", "[^3a|h^7]"},

    {"unmute", G_admin_mute, "m", "unmute a muted player", "[^3name|slot#^7]"},

    // KK-OAX
    {"warn", G_admin_warn, "w", "warn a player", "[^3name|slot#^7] [reason]"}
...
qboolean G_admin_name_check(gentity_t *ent, char *name, char *err, int len) {
  ...
  for (i = 0; i < MAX_ADMIN_ADMINS && g_admin_admins[i]; i++) {
    ...
    if (Q_strequal(name2, testName) &&
        !Q_strequal(ent->client->pers.guid, g_admin_admins[i]->guid)) {
      Com_sprintf(err, len,
...
// KK-OAX Moved the Read/Write int/String functions to g_fileops.c for
// portability across GAME
...
static void admin_default_levels(void) {
  ...
  Q_strncpyz(g_admin_levels[4]->flags, "iahCpPwrkmfKncN?MVdBbeDS51",
             sizeof(l->flags));
  ...
  *flags = '\0';
  ...
  if (victim && !Q_strequal(cmd, "attempted")) {
    Com_sprintf(string, sizeof(string),
                ...
                sec, (admin) ? admin->s.clientNum : -1,
                (admin) ? admin->client->pers.guid
                        : "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX",
                (admin) ? admin->client->pers.netname : "console", flags, cmd,
                victim->client->pers.guid, victim->client->pers.netname,
                G_SayConcatArgs(2 + skiparg));
  } else {
    Com_sprintf(
        string, sizeof(string), "%3i:%i%i: %i: %s: %s: %s: %s: \"%s\"\n", min,
        tens, sec, (admin) ? admin->s.clientNum : -1,
        (admin) ? admin->client->pers.guid : "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX",
        (admin) ? admin->client->pers.netname : "console", flags, cmd,
...
static int admin_listadmins(gentity_t *ent, int start, char *search) {
      ...
      if (g_admin_levels[j]->level == l) {
        ...
        for (colorlen = k = 0; g_admin_levels[j]->name[k]; k++)
          if (Q_IsColorString(&g_admin_levels[j]->name[k]))
            colorlen += 2;
        Com_sprintf(lname, sizeof(lname), "%*s", admin_level_maxname + colorlen,
  ...
  for (i = start;
       i < MAX_ADMIN_ADMINS && g_admin_admins[i] && drawn < MAX_ADMIN_LISTITEMS;
       i++) {
    if (search[0]) {
    ...
    ADMBP(va("%4i %4i %s^7 (*%s) %s^7\n", (i + MAX_CLIENTS),
             g_admin_admins[i]->level, lname, guid_stub,
...
void G_admin_namelog_update(gclient_t *client, qboolean disconnect) {
  ...
  for (i = 0; i < MAX_ADMIN_NAMELOGS && g_admin_namelog[i]; i++) {
    ...
    if (Q_strequal(client->pers.ip, g_admin_namelog[i]->ip) &&
      ...
      for (j = 0; j < MAX_ADMIN_NAMELOG_NAMES && g_admin_namelog[i]->name[j][0];
           j++) {
...
static qboolean admin_create_ban(gentity_t *ent, char *netname, char *guid,
  ...
  Com_sprintf(b->made, sizeof(b->made), "%02i/%02i/%02i %02i:%02i:%02i",
              qt.tm_mon + 1, qt.tm_mday, qt.tm_year % 100, qt.tm_hour,
              qt.tm_min, qt.tm_sec);
...
// KK-OAX Copied create_ban to get Time Stuff Right (Didn't feel like writing
// code to parse it)
static qboolean admin_create_warning(gentity_t *ent, char *netname, char *guid,
                                     char *ip, int seconds, char *warning) {
  g_admin_warning_t *w = NULL;
  qtime_t qt;
  int t;
  int i;
...
qboolean G_admin_kick(gentity_t *ent, int skiparg) {
  ...
  trap_DropClient(pids[0],
                  va("has been kicked%s^7. reason: %s",
...
qboolean G_admin_ban(gentity_t *ent, int skiparg) {
  ...
  for (i = 0; !exactmatch && i < MAX_ADMIN_NAMELOGS && g_admin_namelog[i];
       i++) {
    // skip players in the namelog who have already been banned
    if (g_admin_namelog[i]->banned)
      continue;
  ...
  else if (strlen(g_admin_namelog[logmatch]->guid) == 0 ||
           strlen(g_admin_namelog[logmatch]->ip))
    ADMP("^3!ban: ^7WARNING bot or without GUID or IP cannot write to ban "
         "file\n");
  else
    admin_writeconfig();
  ...
  if (length < 0)
    ...
    if (length) {
      ...
      if (g_adminMaxBan.integer &&
        ...
        ADMP(va("^3!adjustban: ^7ban length is limited to %s for your admin "
                "level\n",
                g_adminMaxBan.string));
...
qboolean G_admin_listplayers(gentity_t *ent, int skiparg) {
  ...
  for (i = 0; i < level.maxclients; i++) {
    ...
    ADMBP(va("%2i %s%s^7 %-2i %s^7 (*%s) ^1%1s^7 %s^7 %s%s^7%s\n", i, c, t, l,
             lname, guid_stub, muted, p->pers.netname, (*n) ? "(a.k.a. " : "",
             n, (*n) ? ")" : ""));
  }
  ADMBP_end();
  return qtrue;
...
qboolean G_admin_showbans(gentity_t *ent, int skiparg) {
  ...
  if (name_match[0] || ip_match) {
    ADMBP(va("^3!showbans:^7 found %d matching bans by %s.  ", count,
             (ip_match) ? "IP" : "name"));
  } else {
  ...
  if (i <= max)
    ADMBP(va("  run !showbans %d%s%s to see more", i + 1,
             (filter[0]) ? " " : "", (filter[0]) ? filter : ""));
  ADMBP("\n");
  ADMBP_end();
  return qtrue;
}
...
qboolean G_admin_rename(gentity_t *ent, int skiparg) {
  ...
  // KK-OAX Since NameChanges are not going to be implemented just yet...let's
  // ignore this.
  ...
  AP(va("print \"^3!rename: ^7%s^7 has been renamed to %s^7 by %s\n\"", oldname,
        newname, (ent) ? ent->client->pers.netname : "console"));
  return qtrue;
}
...
qboolean G_admin_disorient(gentity_t *ent, int skiparg) {
  ...
  vic = &g_entities[pids[0]];
  if (!admin_higher(ent, vic)) {
    ADMP("^/disorient: ^7sorry, but your intended victim has a higher admin "
         "level than you do");
    return qfalse;
  }
...
qboolean G_admin_slap(gentity_t *ent, int skiparg) {
  ...
  vic = &g_entities[pids[0]];
  if (!admin_higher(ent, vic)) {
    ADMP("^/slap: ^7sorry, but your intended victim has a higher admin level "
         "than you do");
    return qfalse;
  }
...
int G_admin_warn_check(gentity_t *ent) {
  ...
  // Just to make sure...Don't want to crash...Will Figure something better out
  // later
  if (!*guid)
    return 0;
  ...
  for (i = 0; i < MAX_ADMIN_WARNINGS && g_admin_warnings[i]; i++) {
    ...
    if (strstr(ip, g_admin_warnings[i]->ip) ||
        strstr(guid, g_admin_warnings[i]->guid)) {
      numWarnings++;
    }
  }
...
qboolean G_admin_warn(gentity_t *ent, int skiparg) {
  ...
  admin_create_warning(ent, vic->client->pers.netname, vic->client->pers.guid,
                       vic->client->pers.ip, seconds, reason);
  ...
  if (g_maxWarnings.integer) {
    ...
    if (totalWarnings >= g_maxWarnings.integer) {
      ...
      admin_create_ban(ent, vic->client->pers.netname, vic->client->pers.guid,
                       vic->client->pers.ip, seconds, "Too Many Warnings");
      ...
      trap_DropClient(pids[0], va("has been kicked%s^7. reason: %s",
                                  "Auto-Admin System", "Too Many Warnings"));
      return qtrue;
    } else {
...
void G_admin_buffer_begin() { g_bfb[0] = '\0'; }
