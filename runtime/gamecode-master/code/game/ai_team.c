...
==================
*/
int BotSortTeamMatesByRelativeTravelTime2ddA(bot_state_t *bs, int *teammates,
                                             int maxteammates) {
...
==================
*/
int BotSortTeamMatesByTaskPreference(bot_state_t *bs, int *teammates,
                                     int numteammates) {
...
void BotCTFOrders_BothFlagsNotAtBase(bot_state_t *bs) {
  ...
  case 3: {
    // tell the one closest to the base not carrying the flag to accompany the
    // flag carrier
    if (teammates[0] != bs->flagcarrier)
      other = teammates[0];
    else
    ...
    if (bot_nochat.integer < 3)
      BotSayTeamOrder(bs, other);
    // tell the one furthest from the the base not carrying the flag to get the
    // enemy flag
...
void BotCTFOrders_FlagNotAtBase(bot_state_t *bs) {
  ...
  numteammates =
      BotSortTeamMatesByBaseTravelTime(bs, teammates, sizeof(teammates));
  BotSortTeamMatesByTaskPreference(bs, teammates, numteammates);
  // In oneway ctf we must all move out of the base (only one strategi, maybe we
  // can also send some to the enemy base  to meet the flag carier?) We must be
  // defending
...
void BotDDorders_Standard(bot_state_t *bs) {
  ...
  numteammates = BotSortTeamMatesByRelativeTravelTime2ddA(bs, teammates,
                                                          sizeof(teammates));
  ...
  default: {
    ...
    for (i = numteammates / 2 + 1; i < numteammates; i++) { // Rest takes point
                                                            // B
      ClientName(teammates[i], name, sizeof(name));
      BotAI_BotInitialChat(bs, "cmd_takeb", name, NULL);
      BotSayTeamOrder(bs, teammates[i]);
...
==================
*/
void BotDDorders(bot_state_t *bs) { BotDDorders_Standard(bs); }
...
Bot1FCTFOrders_TeamHasFlag
  ...
  X% towards neutral flag, Y% go towards enemy base and accompany flag carrier
if visible
==================
*/
void Bot1FCTFOrders_TeamHasFlag(bot_state_t *bs) {
  ...
  if (!(bs->ctfstrategy & CTFS_AGRESSIVE)) {
    ...
    case 3: {
      // tell the one closest to the base not carrying the flag to defend the
      // base
      if (teammates[0] != bs->flagcarrier)
        other = teammates[0];
      else
      ...
      BotSayTeamOrder(bs, other);
      BotSayVoiceTeamOrder(bs, other, VOICECHAT_DEFEND);
      // tell the one furthest from the base not carrying the flag to accompany
      // the flag carrier
      if (teammates[2] != bs->flagcarrier)
        other = teammates[2];
      else
...
void BotTeamAI(bot_state_t *bs) {
  ...
  case GT_CTF_ELIMINATION: {
    ...
    if (bs->numteammates != numteammates || bs->flagstatuschanged ||
        bs->forceorders || lastRoundNumber != level.roundNumber) {
      bs->teamgiveorders_time = FloatTime();
      bs->numteammates = numteammates;
  ...
  case GT_DOUBLE_D: {
    // if the number of team mates changed or the domination point status
    // changed or someone wants to know what to do
    if (bs->numteammates != numteammates || bs->flagstatuschanged ||
        bs->forceorders) {
      bs->teamgiveorders_time = FloatTime();
      bs->numteammates = numteammates;
