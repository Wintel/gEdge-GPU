  ...
  if (bs->lastgoal_ltgtype) {
    ...
    if (gametype == GT_CTF || gametype == GT_CTF_ELIMINATION) {
      if (bs->ltgtype == LTG_GETFLAG) {
        ...
        tb = BotTeamFlag(bs);
        eb = BotEnemyFlag(bs);
        tt = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
                                               tb->areanum, TFL_DEFAULT);
        et = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
                                               eb->areanum, TFL_DEFAULT);
        // if the travel time towards the enemy base is larger than towards our
        // base
        if (et > tt) {
...
void BotCTFSeekGoals(bot_state_t *bs) {
  ...
  if (flagstatus == 1) {
    ...
    if (bs->owndecision_time < FloatTime()) {
      ...
      if (bs->ltgtype != LTG_GETFLAG && bs->ltgtype != LTG_RETURNFLAG &&
          bs->ltgtype != LTG_TEAMHELP && bs->ltgtype != LTG_TEAMACCOMPANY &&
    ...
    //
    if (bs->owndecision_time < FloatTime()) {
      // if not trying to return the flag and not following the team flag
      // carrier
  ...
  if (bs->ltgtype == LTG_TEAMHELP || bs->ltgtype == LTG_TEAMACCOMPANY ||
      ...
      bs->ltgtype == LTG_RUSHBASE || bs->ltgtype == LTG_RETURNFLAG ||
      bs->ltgtype == LTG_CAMPORDER || bs->ltgtype == LTG_PATROL ||
      bs->ltgtype == LTG_GETITEM || bs->ltgtype == LTG_MAKELOVE_UNDER ||
...
void Bot1FCTFSeekGoals(bot_state_t *bs) {
  ...
  if (bs->neutralflagstatus == 1) {
    if (bs->owndecision_time < FloatTime()) {
      ...
      if (bs->ltgtype == LTG_TEAMHELP || bs->ltgtype == LTG_TEAMACCOMPANY ||
          bs->ltgtype == LTG_DEFENDKEYAREA || bs->ltgtype == LTG_GETFLAG ||
          bs->ltgtype == LTG_RUSHBASE || bs->ltgtype == LTG_CAMPORDER ||
          bs->ltgtype == LTG_PATROL || bs->ltgtype == LTG_ATTACKENEMYBASE ||
          bs->ltgtype == LTG_GETITEM || bs->ltgtype == LTG_MAKELOVE_UNDER ||
          bs->ltgtype == LTG_MAKELOVE_ONTOP) {
        return;
      }
  ...
  if (bs->ltgtype == LTG_TEAMHELP || bs->ltgtype == LTG_TEAMACCOMPANY ||
      ...
      bs->ltgtype == LTG_RUSHBASE || bs->ltgtype == LTG_RETURNFLAG ||
      bs->ltgtype == LTG_CAMPORDER || bs->ltgtype == LTG_PATROL ||
      bs->ltgtype == LTG_ATTACKENEMYBASE || bs->ltgtype == LTG_GETITEM ||
      bs->ltgtype == LTG_MAKELOVE_UNDER || bs->ltgtype == LTG_MAKELOVE_ONTOP) {
    return;
  }
  //
  if (BotSetLastOrderedTask(bs))
...
void BotHarvesterSeekGoals(bot_state_t *bs) {
  ...
  if (bs->ltgtype == LTG_TEAMHELP || bs->ltgtype == LTG_TEAMACCOMPANY ||
      bs->ltgtype == LTG_DEFENDKEYAREA || bs->ltgtype == LTG_GETFLAG ||
      bs->ltgtype == LTG_CAMPORDER || bs->ltgtype == LTG_PATROL ||
      bs->ltgtype == LTG_ATTACKENEMYBASE || bs->ltgtype == LTG_HARVEST ||
      bs->ltgtype == LTG_GETITEM || bs->ltgtype == LTG_MAKELOVE_UNDER ||
      bs->ltgtype == LTG_MAKELOVE_ONTOP) {
    return;
  }
...
void BotCheckItemPickup(bot_state_t *bs, int *oldinventory) {
  ...
  if (!oldinventory[INVENTORY_KAMIKAZE] &&
      bs->inventory[INVENTORY_KAMIKAZE] >= 1) {
    offence = qtrue;
  }
  if (!oldinventory[INVENTORY_INVULNERABILITY] &&
      bs->inventory[INVENTORY_INVULNERABILITY] >= 1) {
    offence = qtrue;
  }
  ...
  if (!bs->inventory[INVENTORY_KAMIKAZE] &&
    ...
    if (!oldinventory[INVENTORY_DOUBLER] &&
        bs->inventory[INVENTORY_DOUBLER] >= 1) {
      offence = qfalse;
    }
    if (!oldinventory[INVENTORY_AMMOREGEN] &&
        bs->inventory[INVENTORY_AMMOREGEN] >= 1) {
      offence = qfalse;
    }
  }
        ...
        if (BotTeamLeader(bs)) {
          ...
          if ((bs->ltgtype != LTG_GETFLAG) &&
              ...
              (((gametype != GT_CTF) && (gametype != GT_CTF_ELIMINATION)) ||
               ((bs->redflagstatus == 0) && (bs->blueflagstatus == 0))) &&
...
void BotUpdateInventory(bot_state_t *bs) {
  ...
  bs->inventory[INVENTORY_GRENADELAUNCHER] =
      (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_GRENADE_LAUNCHER)) != 0;
  bs->inventory[INVENTORY_ROCKETLAUNCHER] =
      (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_ROCKET_LAUNCHER)) != 0;
  ...
  bs->inventory[INVENTORY_TELEPORTER] =
      bs->cur_ps.stats[STAT_HOLDABLE_ITEM] == MODELINDEX_TELEPORTER;
  bs->inventory[INVENTORY_MEDKIT] =
      bs->cur_ps.stats[STAT_HOLDABLE_ITEM] == MODELINDEX_MEDKIT;
  bs->inventory[INVENTORY_KAMIKAZE] =
      bs->cur_ps.stats[STAT_HOLDABLE_ITEM] == MODELINDEX_KAMIKAZE;
  bs->inventory[INVENTORY_PORTAL] =
      bs->cur_ps.stats[STAT_HOLDABLE_ITEM] == MODELINDEX_PORTAL;
  bs->inventory[INVENTORY_INVULNERABILITY] =
      bs->cur_ps.stats[STAT_HOLDABLE_ITEM] == MODELINDEX_INVULNERABILITY;
  bs->inventory[INVENTORY_QUAD] = bs->cur_ps.powerups[PW_QUAD] != 0;
  bs->inventory[INVENTORY_ENVIRONMENTSUIT] =
      bs->cur_ps.powerups[PW_BATTLESUIT] != 0;
...
==================
 */
int TeamPlayIsOn(void) { return (gametype >= GT_TEAM && g_ffa_gt != 1); }
...
int BotCanAndWantsToRocketJump(bot_state_t *bs) {
  ...
  if (bs->inventory[INVENTORY_ROCKETS] < 3)
    return qfalse;
  // Sago: Special rule - always happy to rocket jump in elimination, eCTF end
  // LMS if
  ...
  rocketjumper = trap_Characteristic_BFloat(bs->character,
...
int BotWantsToCamp(bot_state_t *bs) {
  ...
  if (BotAggression(bs) < 50)
    return qfalse;
  // the bot should have at least have the rocket launcher, the railgun or the
  // bfg10k with some ammo
  if ((bs->inventory[INVENTORY_ROCKETLAUNCHER] <= 0 ||
  ...
  for (cs = trap_BotGetNextCampSpotGoal(0, &goal); cs;
    ...
    traveltime = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
                                                   goal.areanum, TFL_DEFAULT);
...
void BotRoamGoal(bot_state_t *bs, vec3_t goal) {
  ...
  for (i = 0; i < 10; i++) {
    ...
    if (len > 200) {
      ...
      BotAI_Trace(&trace, bestorg, NULL, NULL, belowbestorg, bs->entitynum,
                  MASK_SOLID);
      //
      if (!trace.startsolid) {
        trace.endpos[2]++;
...
bot_moveresult_t BotAttackMove(bot_state_t *bs, int tfl) {
  ...
  //
  attack_skill = trap_Characteristic_BFloat(bs->character,
...
int BotSameTeam(bot_state_t *bs, int entnum) {
  ...
  if (gametype >= GT_TEAM && g_ffa_gt != 1) {
    /*Sago: I don't know why they decided to check the configstring instead of
     the real value. For some reason bots sometimes gets a wrong config string
     when chaning gametypes. Now we check the real value: */
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (trace.fraction >= 1 || trace.ent == hitent) {
      ...
      if (infog && otherinfog) {
        ...
        // if the entity and the viewer are not in fog assume there's no fog in
        // between
        squaredfogdist = 0;
      }
...
int BotFindEnemy(bot_state_t *bs, int curenemy) {
  ...
  for (i = 0; i < maxclients && i < MAX_CLIENTS; i++) {
    ...
    if (curenemy < 0 && (healthdecrease || EntityIsShooting(&entinfo)))
      ...
      f = 90 + 90 -
          (90 -
           (squaredist > Square(810) ? Square(810) : squaredist) / (810 * 9));
...
void BotAimAtEnemy(bot_state_t *bs) {
  ...
  aim_accuracy = trap_Characteristic_BFloat(bs->character,
                                            CHARACTERISTIC_AIM_ACCURACY, 0, 1);
  //
  if (aim_skill > 0.95) {
    // don't aim too early
    reactiontime = 0.5 * trap_Characteristic_BFloat(
  ...
  if (wi.number == WP_MACHINEGUN) {
    aim_accuracy = trap_Characteristic_BFloat(
        bs->character, CHARACTERISTIC_AIM_ACCURACY_MACHINEGUN, 0, 1);
  } else if (wi.number == WP_SHOTGUN) {
    aim_accuracy = trap_Characteristic_BFloat(
        bs->character, CHARACTERISTIC_AIM_ACCURACY_SHOTGUN, 0, 1);
  } else if (wi.number == WP_GRENADE_LAUNCHER) {
    aim_accuracy = trap_Characteristic_BFloat(
        bs->character, CHARACTERISTIC_AIM_ACCURACY_GRENADELAUNCHER, 0, 1);
    aim_skill = trap_Characteristic_BFloat(
        bs->character, CHARACTERISTIC_AIM_SKILL_GRENADELAUNCHER, 0, 1);
  } else if (wi.number == WP_ROCKET_LAUNCHER) {
    aim_accuracy = trap_Characteristic_BFloat(
        bs->character, CHARACTERISTIC_AIM_ACCURACY_ROCKETLAUNCHER, 0, 1);
    aim_skill = trap_Characteristic_BFloat(
        bs->character, CHARACTERISTIC_AIM_SKILL_ROCKETLAUNCHER, 0, 1);
  } else if (wi.number == WP_LIGHTNING) {
    aim_accuracy = trap_Characteristic_BFloat(
        bs->character, CHARACTERISTIC_AIM_ACCURACY_LIGHTNING, 0, 1);
  } else if (wi.number == WP_RAILGUN) {
    aim_accuracy = trap_Characteristic_BFloat(
        bs->character, CHARACTERISTIC_AIM_ACCURACY_RAILGUN, 0, 1);
  } else if (wi.number == WP_PLASMAGUN) {
    aim_accuracy = trap_Characteristic_BFloat(
        bs->character, CHARACTERISTIC_AIM_ACCURACY_PLASMAGUN, 0, 1);
    aim_skill = trap_Characteristic_BFloat(
  ...
  if (enemyvisible) {
    ...
    if (wi.speed) {
      ...
      VectorSubtract(entinfo.origin, bs->enemyorigin, dir);
      // if the enemy is NOT pretty far away and strafing just small steps left
      // and right
      if (!(dist > 100 && VectorLengthSquared(dir) < Square(32))) {
        ...
        if (aim_skill > 0.8 &&
          ...
          trap_AAS_PredictClientMovement(
              &move, bs->enemy, origin, PRESENCE_CROUCH, qfalse, dir, cmdmove,
              0, dist * 10 / wi.speed, 0.1f, 0, 0, qfalse);
          VectorCopy(move.endpos, bestorigin);
          // BotAI_Print(PRT_MESSAGE, "%1.1f predicted speed = %f, frames =
    ...
    if (aim_skill > 0.6 && wi.proj.damagetype & DAMAGETYPE_RADIAL) {
      ...
      if (entinfo.origin[2] < bs->origin[2] + 16) {
        ...
        BotAI_Trace(&trace, entinfo.origin, NULL, NULL, end, entinfo.number,
                    MASK_SHOT);
        //
        ...
        BotAI_Trace(&trace, start, NULL, NULL, groundtarget, bs->entitynum,
                    MASK_SHOT);
    ...
    if (aim_skill > 0.5) {
      ...
      if (wi.number == WP_BFG || wi.number == WP_ROCKET_LAUNCHER ||
  ...
  //
  if (wi.number == WP_MACHINEGUN || wi.number == WP_SHOTGUN ||
      wi.number == WP_LIGHTNING || wi.number == WP_RAILGUN) {
    // distance towards the enemy
    dist = VectorLength(dir);
    if (dist > 150)
      dist = 150;
...
void BotCheckAttack(bot_state_t *bs) {
  ...
  //
  reactiontime = trap_Characteristic_BFloat(bs->character,
  ...
  if (bs->firethrottlewait_time > FloatTime())
    return;
  firethrottle = trap_Characteristic_BFloat(bs->character,
  ...
  if (health) {
    ...
    if (bsptrace.fraction >= 1.0 || bsptrace.ent == entitynum) {
      ...
      activategoal->goal.entitynum =
          entitynum; // NOTE: this is the entity number of the shootable button
      ...
      // create a goal from where the button is visible and shoot at the button
      // from there add bounding box size to the dist
      ...
      if (i < 0) {
        // FIXME: trace forward and maybe in other directions to find a valid
        // area
      }
      if (i >= 0) {
        //
...
int BotGetActivateGoal(bot_state_t *bs, int entitynum,
  ...
  if (strequals(classname, "func_door")) {
    ...
    if (*model) {
      ...
      if (modelindex) {
        ...
        //
        numareas =
            trap_AAS_BBoxAreas(absmins, absmaxs, areas, MAX_ACTIVATEAREAS * 2);
    ...
    if (strequals(classname, "func_button")) {
      ...
      if (trap_AAS_AreaReachability(bs->areanum)) {
        ...
        t = trap_AAS_AreaTravelTimeToGoalArea(
            bs->areanum, bs->origin, activategoal->goal.areanum, bs->tfl);
      ...
      continue;
    } // the actual button or trigger might be linked through a target_relay or
      // target_delay
    else if (strequals(classname, "target_relay") ||
             strequals(classname, "target_delay")) {
      if (trap_AAS_ValueForBSPEpairKey(ent, "targetname", targetname[i + 1],
                                       sizeof(targetname[0]))) {
        i++;
  ...
  BotAI_Print(
      PRT_ERROR,
      "BotGetActivateGoal: no valid activator for entity with target \"%s\"\n",
      targetname[0]);
#endif
  return 0;
}
...
void BotAIBlocked(bot_state_t *bs, bot_moveresult_t *moveresult, int activate) {
  ...
  if (activate && entinfo.modelindex > 0 &&
      entinfo.modelindex <= max_bspmodelindex) {
    // find the bsp entity which should be activated in order to get the
    // blocking entity out of the way
  ...
  start[2] += 18;
  //
  // bsptrace = AAS_Trace(start, mins, maxs, end, bs->entitynum,
  // MASK_PLAYERSOLID); if (bsptrace.fraction >= 1) movetype = MOVE_CROUCH;
  // get the sideward vector
  CrossProduct(hordir, up, sideward);
  //
...
int BotAIPredictObstacles(bot_state_t *bs, bot_goal_t *goal) {
  ...
  trap_AAS_PredictRoute(&route, bs->areanum, bs->origin, goal->areanum, bs->tfl,
                        100, 1000, RSE_USETRAVELTYPE | RSE_ENTERCONTENTS,
    ...
    if (!BotMatchMessage(bs, m.message)) {
      ...
      if (m.type == CMS_CHAT && !bot_nochat.integer) {
        ...
        if (bot_testrchat.integer) {
          ...
          if (trap_BotReplyChat(bs->cs, message, context, CONTEXT_REPLY, NULL,
                                NULL, NULL, NULL, NULL, NULL, botname,
                                netname)) {
            BotAI_Print(PRT_MESSAGE, "------------------------\n");
          } else {
            BotAI_Print(PRT_MESSAGE, "**** no valid reply ****\n");
          }
        } // if at a valid chat position and not chatting already and not in
          // teamplay
    ...
    if (gametype == GT_CTF || gametype == GT_CTF_ELIMINATION) {
      ...
      case GTS_RED_TAKEN:
        bs->neutralflagstatus =
            BotTeam(bs) == TEAM_RED
                ? 2
                : 1; // FIXME: check Team_TakeFlagSound in g_team.c
        bs->flagstatuschanged = qtrue;
        break;
      case GTS_BLUE_TAKEN:
        bs->neutralflagstatus =
            BotTeam(bs) == TEAM_BLUE
                ? 2
                : 1; // FIXME: check Team_TakeFlagSound in g_team.c
        bs->flagstatuschanged = qtrue;
        break;
...
bot_goal_t *BotAlternateRoute(bot_state_t *bs, bot_goal_t *goal) {
  ...
  if (bs->altroutegoal.areanum) {
    ...
    t = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
                                          bs->altroutegoal.areanum, bs->tfl);
    if (t && t < 20) {
  ...
  if (gametype == GT_CTF || gametype == GT_CTF_ELIMINATION) {
    ...
    if (ctf_neutralflag.areanum) {
      ...
      red_numaltroutegoals = trap_AAS_AlternativeRouteGoals(
          ctf_neutralflag.origin, ctf_neutralflag.areanum, ctf_redflag.origin,
          ctf_redflag.areanum, TFL_DEFAULT, red_altroutegoals,
          MAX_ALTROUTEGOALS,
          ALTROUTEGOAL_CLUSTERPORTALS | ALTROUTEGOAL_VIEWPORTALS);
      blue_numaltroutegoals = trap_AAS_AlternativeRouteGoals(
          ctf_neutralflag.origin, ctf_neutralflag.areanum, ctf_blueflag.origin,
          ctf_blueflag.areanum, TFL_DEFAULT, blue_altroutegoals,
          MAX_ALTROUTEGOALS,
          ALTROUTEGOAL_CLUSTERPORTALS | ALTROUTEGOAL_VIEWPORTALS);
    }
  } else if (gametype == GT_1FCTF) {
    ...
    red_numaltroutegoals = trap_AAS_AlternativeRouteGoals(
        ctf_neutralflag.origin, ctf_neutralflag.areanum, ctf_redflag.origin,
        ctf_redflag.areanum, TFL_DEFAULT, red_altroutegoals, MAX_ALTROUTEGOALS,
        ALTROUTEGOAL_CLUSTERPORTALS | ALTROUTEGOAL_VIEWPORTALS);
    blue_numaltroutegoals = trap_AAS_AlternativeRouteGoals(
        ctf_neutralflag.origin, ctf_neutralflag.areanum, ctf_blueflag.origin,
        ctf_blueflag.areanum, TFL_DEFAULT, blue_altroutegoals,
    ...
    //
    red_numaltroutegoals = trap_AAS_AlternativeRouteGoals(
        neutralobelisk.origin, neutralobelisk.areanum, redobelisk.origin,
        redobelisk.areanum, TFL_DEFAULT, red_altroutegoals, MAX_ALTROUTEGOALS,
        ALTROUTEGOAL_CLUSTERPORTALS | ALTROUTEGOAL_VIEWPORTALS);
    blue_numaltroutegoals = trap_AAS_AlternativeRouteGoals(
        neutralobelisk.origin, neutralobelisk.areanum, blueobelisk.origin,
        blueobelisk.areanum, TFL_DEFAULT, blue_altroutegoals, MAX_ALTROUTEGOALS,
        ALTROUTEGOAL_CLUSTERPORTALS | ALTROUTEGOAL_VIEWPORTALS);
  } else if (gametype == GT_HARVESTER) {
...
void BotSetupDeathmatchAI(void) {
  ...
  if (gametype == GT_CTF || gametype == GT_CTF_ELIMINATION) {
    ...
    ent =
        untrap_BotGetLevelItemGoal(-1, "Domination point", &dom_points_bot[0]);
    if (ent < 0)
      BotAI_Print(PRT_WARNING,
                  "Domination without a single domination point\n");
    else
    ...
    for (i = 1; i < level.domination_points_count; i++) {
      ...
      ent = untrap_BotGetLevelItemGoal(ent, "Domination point",
                                       &dom_points_bot[i]);
      if (ent < 0)
