...
// Domination how many seconds between awarded a point (multiplied by two if
// more than 3 points)
...
typedef struct {
  ...
  qboolean cannotWin; // Set to true if the players joins a leading team or the
...
struct gclient_s {
  ...
  // New vote system. The votes are saved in the client info, so we know who
  // voted on what and can cancel votes on leave. 0=not voted, 1=voted yes,
  // -1=voted no
  int vote;
...
typedef struct {
  ...
  qboolean humansEliminated; // True if we are in an elimination mode and all
                             // humans have been eliminated

  // Added for Double Domination
  // Points get status: TEAM_FREE for not taking, TEAM_RED/TEAM_BLUE for taken
  // and TEAM_NONE for not spawned yet
  ...
  int pointStatusDom[MAX_DOMINATION_POINTS]; // Holds the owner of all the
                                             // points
  int dom_scoreGiven; // Number of times we have provided scores
  int domination_points_count;
  char domination_points_names[MAX_DOMINATION_POINTS]
                              [MAX_DOMINATION_POINTS_NAMES];

  // Added to keep track of challenges (can only be completed against
  // humanplayers)
  qboolean hadBots; // There was bots in the level
  int teamSize;     // The highest number of players on the least populated team
                // when there was most players
  ...
  int lastActiveTime; ///< Updated as long as there are at least one human
 ...
 * @return number of spawnpoints for each type of spawning (initial spawn,
 * respawn, red, blue)
 */
 ...
 * This function can be given a filter. If the callback function returns 0 then
 * the entity is not valid for selection
...
extern vmCvar_t
    g_lms_mode; // How do we score: 0 = One Survivor get a point, 1 = same but
                // without overtime, 2 = one point for each player killed
                // (+overtime), 3 = same without overtime

extern vmCvar_t g_elimination_ctf_oneway; // Only attack in one direction
                                          // (level.eliminationSides+level.roundNumber)%2
                                          // == 0 red attacks

extern vmCvar_t g_awardpushing; // The server can decide if players are awarded
                                // for pushing people in lave etc.
...
extern vmCvar_t
    g_spreeDiv; // Interval of a "streak" that form the spree triggers
