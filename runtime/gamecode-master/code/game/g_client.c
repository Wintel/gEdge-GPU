...
gentity_t *SelectSpawnPoint(vec3_t avoidPoint, vec3_t origin, vec3_t angles,
  ...
  if (!spot) {
    if (g_cheats.integer) {
      // If in developer mode: Crash to ensure that the developer discovers the
      // problem.
      G_Error("Couldn't find a spawn point.\nAny map must contain at least one "
              "\"info_player_deathmatch\" with no spawn restrictions");
    }
    spot = &g_entities[0];
  }
...
void CopyToBodyQue(gentity_t *ent) {
  ...
  if ((contents & CONTENTS_NODROP) &&
      !(ent->s.eFlags &
        EF_KAMIKAZE)) { // the check for kamikaze is a workaround for ctf4ish
    return;
  }
  ...
  for (i = 0;
       (level.bodyQue[level.bodyQueIndex]->s.eFlags & EF_KAMIKAZE) && (i < 10);
       i++) {
    level.bodyQueIndex = (level.bodyQueIndex + 1) % BODY_QUEUE_SIZE;
  }
  ...
  if (g_gametype.integer == GT_LMS) {
    ...
    } else // We have used all our lives
    {
      if (ent->client->isEliminated != qtrue) {
        ...
        if ((g_lms_mode.integer == 2 || g_lms_mode.integer == 3) &&
...
void TeamCvarSet(void) {
  ...
  // Note: We need to force update of the cvar or SendYourTeamMessage will send
  // the old cvar value!
  if (redChanged) {
...
team_t TeamLivingCount(int ignoreClientNum, int team) {
  ...
  for (i = 0; i < level.maxclients; i++) {
    ...
    if (level.clients[i].sess.sessionTeam == team &&
        (level.clients[i].ps.stats[STAT_HEALTH] > 0 || isLMS) &&
...
int TeamLivingHumanCount(int ignoreClientNum) {
  ...
  for (i = 0; i < level.maxclients; i++) {
    ...
    // crash if g_gametype.integer is used here, why?
    if ((level.clients[i].ps.stats[STAT_HEALTH] > 0 || isLMS) &&
        !(level.clients[i].isEliminated)) {
      count++;
    }
  }
...
void LMSpoint(void) {
  ...
  for (i = 0; i < level.maxclients; i++) {
    ...
    G_LogPrintf("PlayerScore: %i %i: %s now has %d points\n", i,
                client->client->ps.persistant[PERS_SCORE],
...
team_t PickTeam(int ignoreClientNum) {
  ...
  // KK-OAX Both Teams locked...forget about it, print an error message, keep as
  // spec
...
void ClientUserinfoChanged(int clientNum) {
  ...
  if (!strequals(oldname, client->pers.netname)) {
    if (client->pers.nameChangeTime &&
      ...
      trap_SendServerCommand(ent - g_entities,
                             va("print \"Name change spam protection "
                                "(g_minNameChangePeriod = %d)\n\"",
      ...
      revertName = qtrue;
    } else if (!G_admin_name_check(ent, client->pers.netname, err,
                                   sizeof(err))) {
      trap_SendServerCommand(ent - g_entities, va("print \"%s\n\"", err));
      revertName = qtrue;
    }

    // Never revert a bots name... just to bad if it hapens... but the bot will
    // always be expendeble :-)
  ...
  if (ent->r.svFlags & SVF_BOT) {
    s = va("n\\%s\\t\\%i\\model\\%s\\hmodel\\%s\\c1\\%s\\c2\\%s\\hc\\%i\\w\\%"
           "i\\l\\%i\\skill\\%s\\tt\\%d\\tl\\%d",
           client->pers.netname, team, model, headModel, c1, c2,
           client->pers.maxHealth, client->sess.wins, client->sess.losses,
           Info_ValueForKey(userinfo, "skill"), teamTask, teamLeader);
  } else {
    s = va("n\\%s\\t\\%i\\model\\%s\\hmodel\\%s\\g_redteam\\%s\\g_blueteam\\%"
           "s\\c1\\%s\\c2\\%s\\hc\\%i\\w\\%i\\l\\%i\\tt\\%d\\tl\\%d",
           client->pers.netname, client->sess.sessionTeam, model, headModel,
           redTeam, blueTeam, c1, c2, client->pers.maxHealth, client->sess.wins,
...
char *ClientConnect(int clientNum, qboolean firstTime, qboolean isBot) {
  ...
  if (!isBot && !strequals(value, "localhost")) {
    ...
    for (i = 0;
         i < sizeof(client->pers.guid) - 1 && isxdigit(client->pers.guid[i]);
         i++) {
    };
  ...
  if (g_delagHitscan.integer) {
    trap_SendServerCommand(clientNum,
                           "print \"Full lag compensation is ON!\n\"");
  } else {
    trap_SendServerCommand(clientNum,
                           "print \"Full lag compensation is OFF!\n\"");
  }
...
void ClientBegin(int clientNum) {
  ...
  if (g_gametype.integer == GT_LMS) {
    client->isEliminated =
        qtrue; // So player does not give a point in gamemode 2 and 3
    // trap_SendServerCommand( -1, va("print \"%s" S_COLOR_WHITE " will start
    // dead\n\"", client->pers.netname) );
  }
...
void ClientSpawn(gentity_t *ent) {
  ...
  // In Elimination the player should not spawn if he have already spawned in
  // the round (but not for spectators)
  //  N_G: You've obviously wanted something ELSE
  // Sago: Yes, the !level.intermissiontime is currently redundant but it might
  // still be the bast place to make the test, CheckElimination in g_main makes
  // sure the next if will fail and the rest of the things this block does might
  // not affect if in Intermission (I'll just test that)
  if (((g_gametype.integer == GT_ELIMINATION ||
        g_gametype.integer == GT_CTF_ELIMINATION ||
    ...
    // Sago: I beleive the TeamCount is to make sure people can join even if the
    // game can't start
    if ((level.roundNumber == level.roundNumberStarted) ||
        ((level.time <
          level.roundStartTime - g_elimination_activewarmup.integer * 1000) &&
  ...
  if (g_gametype.integer != GT_ELIMINATION &&
    ...
    ent->health = client->ps.stats[STAT_ARMOR] =
        g_elimination_startArmor
            .integer; // client->ps.stats[STAT_MAX_HEALTH]*2;
    ent->health = client->ps.stats[STAT_HEALTH] =
        g_elimination_startHealth
            .integer; // client->ps.stats[STAT_MAX_HEALTH]*2;
  }
  ...
  if (g_gametype.integer == GT_ELIMINATION ||
      g_gametype.integer == GT_CTF_ELIMINATION || g_gametype.integer == GT_LMS)
...
void ClientDisconnect(int clientNum) {
  ...
  for (i = 0; i < level.maxclients; i++) {
    if ((level.clients[i].sess.sessionTeam == TEAM_SPECTATOR ||
         level.clients[i].ps.pm_type == PM_SPECTATOR) &&
