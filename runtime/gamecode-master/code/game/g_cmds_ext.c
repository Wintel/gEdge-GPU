...
void G_MatchOnePlayer(int *plist, int num, char *err, int len) {
  ...
  if (num == 0) {
    ...
    Q_strcat(err, len,
             "more than one player name matches. Be more specific or use the "
             "slot #:\n");
    for (i = 0; i < num; i++) {
...
int G_FloodLimited(gentity_t *ent) {
  ...
  trap_SendServerCommand(ent - g_entities,
                         va("print \"You are flooding: please wait %d second%s "
                            "before trying again\n",
...
static void QDECL G_AdminMessage(const char *prefix, const char *fmt, ...) {
  ...
  Com_sprintf(outstring, sizeof(outstring), "%s " S_COLOR_MAGENTA "%s", prefix,
              string);
...
void Cmd_AdminMessage_f(gentity_t *ent) {
  ...
  if (!ent) {
    ...
    if (!g_publicAdminMessages.integer) {
      ...
      ADMP("Your message has been sent to any available admins and to the "
           "server logs.\n");
    }
  } else {
