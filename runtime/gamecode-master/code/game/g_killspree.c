...
Since this is cvar dependent, it has to be placed in G_InitGame after cvars are
registered.
=================
*/
qboolean G_ReadAltKillSettings(gentity_t *ent, int skiparg) {
  ...
  */
  if (g_spreeDiv.integer >= 2) {
    ...
    spreeDivisor = 5;
    G_Printf("Error: cvar g_spreeDiv must not be set to 0 or 1, reverting to "
             "default settings!\n");
  ...
  *(cnf + length) = '\0';
  ...
  while (1) {
    ...
    if (Q_strequal(t, "[kspree]")) {
      ...
      if (Q_strequal(t, "level")) {
        ...
        // Let's take the spreeLevel and multiply it by the spreeDivisor to give
        // us our count
  ...
  BG_Free(cnf2);
  G_Printf("Sprees/Kills: loaded %d killing sprees, %d death sprees, and %d "
           "multikills.\n",
           ksc, dsc, mc);
  // Mark the Upper Bounds of the Arrays (Since they start numbering at 0, We
  // subtract 1 )
...
static char *fillPlaceHolder(char *stringToSearch, char *placeHolder,
                             char *replaceWith) {
  ...
  Q_snprintf(output + (p - stringToSearch), output - stringToSearch, "%s%s",
             replaceWith, p + strlen(placeHolder));
...
static char *CreateMessage(gentity_t *ent, char *message, char *spreeNumber) {
  ...
  Q_strncpyz(output, fillPlaceHolder(output, "[k]", spreeNumber),
             sizeof(output));
  return output;
}
...
void G_RunStreakLogic(gentity_t *attacker, gentity_t *victim) {
  ...
  // Make sure they are a client and that the attacker and victim are not the
  // same client We don't want suicide to count towards a killstreak...
  if ((attacker) && (attacker->client) && (attacker != victim)) {

    // Check the gametype--If FFA enabled, everybody's on the same team.
    if (g_gametype.integer >= GT_TEAM && g_ffa_gt != 1) {
      // If they are on the same team we don't want to count it towards a
      // killing spree.
...
void G_CheckForSpree(gentity_t *ent, int streak2Test, qboolean checkKillSpree) {
  int i;
  char *returnedString;
  // If somebody want's to award killing sprees above 99 kills he/she can mod
  // this his/herself!!! :)
  char streakcount[3];
  char *sound;
  int position;
  ...
  // Probably Not Needed, but to protect Server Ops from Crashing their Stuff
  // MidMatch
  ...
  if (!checkKillSpree) {
    ...
    if (divisionHolder > level.dSpreeUBound) {
      ...
      if (!deathSprees[level.dSpreeUBound])
        return;
      returnedString = CreateMessage(
          ent, deathSprees[level.dSpreeUBound]->spreeMsg, streakcount);
      ...
      for (i = 0; deathSprees[i]; i++) {
        ...
        if (deathSprees[i]->streakCount == streak2Test) {
          ...
          Q_snprintf(streakcount, sizeof(streakcount), "%i",
                     deathSprees[i]->streakCount);
          // Let's grab the message to show, fill up the placeholders and
          // concatenate it.
          returnedString =
              CreateMessage(ent, deathSprees[i]->spreeMsg, streakcount);
    ...
    if (divisionHolder > level.kSpreeUBound) {
      ...
      if (!killSprees[level.kSpreeUBound])
        return;
      returnedString = CreateMessage(
          ent, killSprees[level.kSpreeUBound]->spreeMsg, streakcount);
      ...
      /* Doesn't do anything at the moment. cp does not work while kill message
      is displayed
       * if( position == CENTER_PRINT ) {
      ...
      for (i = 0; killSprees[i]; i++) {
        if (killSprees[i]->streakCount == streak2Test) {
          Q_snprintf(streakcount, sizeof(streakcount), "%i",
                     killSprees[i]->streakCount);
          returnedString =
              CreateMessage(ent, killSprees[i]->spreeMsg, streakcount);
...
void G_checkForMultiKill(gentity_t *ent) {
  ...
  if (multiKillCount > multiKills[level.mKillUBound]->kills) {
    ...
    if (!multiKills[level.mKillUBound])
      return; // If null
    returnedString = CreateMessage(ent, multiKills[level.mKillUBound]->killMsg,
                                   multiKillString);
    ...
    for (i = 0; multiKills[i]; i++) {
      ...
      if (multiKills[i]->kills == multiKillCount) {
        Q_snprintf(multiKillString, sizeof(multiKillString), "%i",
                   multiKills[i]->kills);
        // Build the Message
        returnedString =
            CreateMessage(ent, multiKills[i]->killMsg, multiKillString);
        // Grab the sound
        ...
        G_Sound(ent, 0, soundIndex);
        /* Print the String
        Since we don't want to clutter screens (the player is already going to
        get the excellent icon) we won't give them an option to centerprint.
        */
        AP(va("chat \"%s\"", returnedString));
        break;
      }
