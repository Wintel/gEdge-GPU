...
void BotReportStatus(bot_state_t *bs) {
  ...
  if (gametype == GT_CTF || gametype == GT_CTF_ELIMINATION) {
    ...
    if (BotHarvesterCarryingCubes(bs)) {
      if (BotTeam(bs) == TEAM_RED)
        Com_sprintf(flagstatus, sizeof(flagstatus), S_COLOR_RED "%2d",
                    bs->inventory[INVENTORY_REDCUBE]);
      else
        Com_sprintf(flagstatus, sizeof(flagstatus), S_COLOR_BLUE "%2d",
...
void BotSetInfoConfigString(bot_state_t *bs) {
  ...
  if (gametype == GT_CTF || gametype == GT_CTF_ELIMINATION) {
    ...
    if (BotHarvesterCarryingCubes(bs)) {
      if (BotTeam(bs) == TEAM_RED)
        Com_sprintf(carrying, sizeof(carrying), "%2d",
                    bs->inventory[INVENTORY_REDCUBE]);
      else
        Com_sprintf(carrying, sizeof(carrying), "%2d",
                    bs->inventory[INVENTORY_BLUECUBE]);
...
void BotInterbreedBots(void) {
  ...
  if (trap_GeneticParentsAndChildSelection(MAX_CLIENTS, ranks, &parent1,
    ...
    trap_BotInterbreedGoalFuzzyLogic(
        botstates[parent1]->gs, botstates[parent2]->gs, botstates[child]->gs);
...
void BotChangeViewAngles(bot_state_t *bs, float thinktime) {
  ...
  //
  if (bs->enemy >= 0) {
    factor = trap_Characteristic_BFloat(bs->character,
                                        CHARACTERISTIC_VIEW_FACTOR, 0.01f, 1);
  ...
  for (i = 0; i < 2; i++) {
    ...
    // bs->ideal_viewangles[1], bs->ideal_viewangles[2]);` bs->viewangles[i] =
...
==============
*/
void BotInputToUserCommand(bot_input_t *bi, usercmd_t *ucmd,
...
int BotAI(int client, float thinktime) {
  ...
  while (trap_BotGetServerCommand(client, buf, sizeof(buf))) {
    ...
    *args++ = '\0';
    ...
    if (Q_strequal(buf, "cp ")) {       /*CenterPrintf*/
    } else if (Q_strequal(buf, "cs")) { /*ConfigStringModified*/
...
#endif
    else if (Q_strequal(buf, "scores")) {            /*FIXME: parse scores?*/
    } else if (Q_strequal(buf, "clientLevelShot")) { /*ignore*/
    }
  }
  // add the delta angles to the bot's current view angles
  for (j = 0; j < 3; j++) {
...
int BotAISetupClient(int client, struct bot_settings_s *settings,
  ...
  // load the chat file
  trap_Characteristic_String(bs->character, CHARACTERISTIC_CHAT_FILE, filename,
                             MAX_PATH);
...
void ProximityMine_Trigger(gentity_t *trigger, gentity_t *other,
                           trace_t *trace);
  ...
  if (botlib_residual >= thinktime) {
    ...
    for (i = 0; i < MAX_GENTITIES; i++) {
      ...
      if (!(g_gametype.integer == GT_ELIMINATION ||
            g_gametype.integer == GT_LMS || g_instantgib.integer ||
            g_rockets.integer || g_elimination_allgametypes.integer ||
            g_gametype.integer == GT_CTF_ELIMINATION) &&
          ent->r.svFlags & SVF_NOCLIENT) {
        trap_BotLibUpdateEntity(i, NULL);
        continue;
