...
void G_SetClientSound(gentity_t *ent) {
  if (ent->s.eFlags & EF_TICKING) {
    ent->client->ps.loopSound =
        G_SoundIndex("sound/weapons/proxmine/wstbtick.wav");
...
void G_TouchTriggers(gentity_t *ent) {
  ...
  // ELIMINATION LMS
  //  dead clients don't activate triggers! The reason our pm_spectators can't
  //  do anything
  ...
  for (i = 0; i < num; i++) {
    ...
    if ((ent->client->sess.sessionTeam == TEAM_SPECTATOR) ||
      ...
      if (hit->s.eType != ET_TELEPORT_TRIGGER &&
          ...
          // We need to stop eliminated players from opening doors somewhere
          // else /Sago007 20070814
...
void SpectatorThink(gentity_t *ent, usercmd_t *ucmd) {
  ...
  if ((client->buttons & BUTTON_USE_HOLDABLE) &&
...
static void ClientTimerActions(gentity_t *ent, int msec) {
  ...
  while (client->timeResidual >= 1000) {
    ...
    if (maxHealth) {
      ...
      if ((g_gametype.integer == GT_ELIMINATION ||
          ...
          level.humansEliminated) {
        ent->damage = 5;
        G_Damage(ent, NULL, NULL, NULL, NULL, ent->damage, DAMAGE_NO_KNOCKBACK,
                 MOD_UNKNOWN);
      }
      // Start killing players in LMS, if we are in overtime
               ...
               (level.time >= level.roundStartTime +
                                  1000 * g_elimination_roundtime.integer)) {
        ent->damage = 5;
        G_Damage(ent, NULL, NULL, NULL, NULL, ent->damage, DAMAGE_NO_ARMOR,
                 MOD_UNKNOWN);
    ...
    if (g_gametype.integer == GT_POSSESSION && ent->health > 0 &&
  ...
  if (bg_itemlist[client->ps.stats[STAT_PERSISTANT_POWERUP]].giTag ==
      PW_AMMOREGEN) {
    int w, max, inc, t, i;
    int weapList[] = {WP_MACHINEGUN,      WP_SHOTGUN,   WP_GRENADE_LAUNCHER,
                      WP_ROCKET_LAUNCHER, WP_LIGHTNING, WP_RAILGUN,
                      WP_PLASMAGUN,       WP_BFG,       WP_NAILGUN,
...
void ClientThink_real(gentity_t *ent) {
  ...
  /*	client->pers.cmdqueue[client->pers.cmdhead] = client->pers.cmd;
          ...
          if ( client->pers.latentCmds ) {
                  ...
                  int cmdindex = client->pers.cmdhead - client->pers.latentCmds
     - 1; while ( cmdindex < 0 ) { cmdindex += MAX_LATENT_CMDS;
                  }
  ...
  /*if ( client->pers.latentSnaps ) {
          ...
          client->pers.realPing += client->pers.latentSnaps * (1000 /
  sv_fps.integer);
          // adjust the attack time so backward reconciliation will work
          client->attackTime -= client->pers.latentSnaps * (1000 /
  sv_fps.integer);
  }*/
  // unlagged - lag simulation #1
  ...
  if (client->ps.weapon == WP_GRAPPLING_HOOK && client->hook &&
  ...
  if (level.intermissionQueued != 0 && g_singlePlayer.integer) {
    if (level.time - level.intermissionQueued >= 1000) {
      ...
      if (level.time - level.intermissionQueued >= 2000 &&
          level.time - level.intermissionQueued <= 2500) {
  ...
  /*
     ...
     &ent->s, ent->client->ps.commandTime, qtrue );
          }
          else {
  */
  ...
  if (g_awardpushing.integer < 2 && pm.waterlevel <= 1 &&
