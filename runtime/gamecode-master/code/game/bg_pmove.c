...
extern int enableQ;
extern vmCvar_t cg_enableQ; // leilei - map changes player/weapons scale (for q1
                            // adaptations)
/*
===============
PM_AddEvent
...
static void PM_TorsoAnimation(void) {
  if (pm->ps->weaponstate == WEAPON_READY) {
    if (pm->ps->velocity[2] < 0 && !pml.groundPlane) {
      ...
      } else if (pm->ps->powerups[PW_REDFLAG] ||
                 pm->ps->powerups[PW_BLUEFLAG] ||
                 pm->ps->powerups[PW_NEUTRALFLAG]) {
        PM_ContinueTorsoAnim(TORSO_FALL2);
      } else {
      ...
      if (pm->ps->weapon == WP_GAUNTLET) {
        ...
        PM_ContinueTorsoAnim(TORSO_FALL);
               ...
               (pm->cmd.rightmove && pml.groundPlane && pm->xyspeed > 200 &&
      ...
      } else if (pm->cmd.rightmove && !pm->cmd.forwardmove && pml.groundPlane &&
                 !(pm->cmd.buttons & BUTTON_WALKING)) {
