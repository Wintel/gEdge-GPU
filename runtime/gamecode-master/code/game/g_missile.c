...
================
*/
static void ProximityMine_Die(gentity_t *ent, gentity_t *inflictor,
...
void ProximityMine_Trigger(gentity_t *trigger, gentity_t *other,
  ...
  // ok, now check for ability to damage so we don't get triggered thru walls,
  // closed doors, etc...
...
static void ProximityMine_Activate(gentity_t *ent) {
  ...
  if (g_gametype.integer == GT_TEAM || g_gametype.integer == GT_DOMINATION ||
...
static void ProximityMine_ExplodeOnPlayer(gentity_t *mine) {
  ...
  if (player->client->invulnerabilityTime > level.time) {
    G_Damage(player, mine->parent, mine->parent, vec3_origin, mine->s.origin,
             1000, DAMAGE_NO_KNOCKBACK, MOD_JUICED);
  ...
  if (other->takedamage) {
    if (ent->s.weapon != WP_PROX_LAUNCHER) {
      if (other->client && other->client->invulnerabilityTime > level.time) {
        ...
        if (G_InvulnerabilityEffect(other, forward, ent->s.pos.trBase,
...
void G_RunMissile(gentity_t *ent) {
  ...
  if (ent->s.weapon == WP_PROX_LAUNCHER && !ent->count) {
    ...
    trap_Trace(&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs,
               ent->r.currentOrigin, ENTITYNUM_NONE, ent->clipmask);
...
gentity_t *fire_grapple(gentity_t *self, vec3_t start, vec3_t dir) {
  ...
  // unlagged - grapple
  // hook->s.pos.trTime = level.time - MISSILE_PRESTEP_TIME;		// move
  // a bit on the very first frame
  // unlagged - grapple
