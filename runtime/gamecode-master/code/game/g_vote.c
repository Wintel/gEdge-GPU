...
==================
allowedVote
 *Note: Keep this in sync with allowedVote in ui_votemenu.c (except for
cg_voteNames and g_voteNames)
==================
 */
#define MAX_VOTENAME_LENGTH                                                    \
  14 // currently the longest string is "/map_restart/\0" (14 chars)
int allowedVote(const char *commandStr) {
  ...
  // Now constructing a string that starts and ends with '/' like:
  // "/clientkick/"
  tempStr[0] = '/';
...
==================
 */
#define MAX_GAMETYPENAME_LENGTH                                                \
  5 // currently the longest string is "/12/\0" (5 chars)
int allowedGametype(const char *gametypeStr) {
  char tempStr[MAX_GAMETYPENAME_LENGTH];
  int length;
  char voteGametypes[MAX_CVAR_VALUE_STRING];
  trap_Cvar_VariableStringBuffer("g_voteGametypes", voteGametypes,
                                 sizeof(voteGametypes));
    ...
    if (g_dmflags.integer & DF_LIGHT_VOTING) {
      ...
      if (level.voteYes > level.voteNo * 2) {
        trap_SendServerCommand(
            -1, "print \"Vote passed. At least 2 of 3 voted yes\n\"");
        level.voteExecuteTime = level.time + 3000;
      } else {
        // Let pass if there is more yes than no and at least 2 yes votes and at
        // least 30% yes of all on the server
        if (level.voteYes > level.voteNo && level.voteYes >= 2 &&
            (level.voteYes * 10) > (level.numVotingClients * 3)) {
          trap_SendServerCommand(-1,
                                 "print \"Vote passed. More yes than no.\n\"");
          level.voteExecuteTime = level.time + 3000;
        } else
