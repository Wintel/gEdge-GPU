...
void AddScore(gentity_t *ent, vec3_t origin, int score) {
  ...
  // show score plum
  if (level.numNonSpectatorClients < 3 && score < 0 &&
    ...
    if (g_gametype.integer == GT_TEAM) {
      ...
      G_LogPrintf("TeamScore: %i %i: Team %d now has %d points\n", team,
                  level.teamScores[team], team, level.teamScores[team]);
    }
  }
  G_LogPrintf("PlayerScore: %i %i: %s now has %d points\n", ent->s.number,
              ent->client->ps.persistant[PERS_SCORE], ent->client->pers.netname,
...
static const char *modNames[] = {
    "MOD_UNKNOWN",       "MOD_SHOTGUN",        "MOD_GAUNTLET",
    "MOD_MACHINEGUN",    "MOD_GRENADE",        "MOD_GRENADE_SPLASH",
    "MOD_ROCKET",        "MOD_ROCKET_SPLASH",  "MOD_PLASMA",
    "MOD_PLASMA_SPLASH", "MOD_RAILGUN",        "MOD_LIGHTNING",
    "MOD_BFG",           "MOD_BFG_SPLASH",     "MOD_WATER",
    "MOD_SLIME",         "MOD_LAVA",           "MOD_CRUSH",
    "MOD_TELEFRAG",      "MOD_FALLING",        "MOD_SUICIDE",
    "MOD_TARGET_LASER",  "MOD_TRIGGER_HURT",   "MOD_NAIL",
    "MOD_CHAINGUN",      "MOD_PROXIMITY_MINE", "MOD_KAMIKAZE",
...
void player_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
  ...
  // Sago: Hmmm... generic? Can I transmit anything I like? Like if it is a team
  // kill? Let's try
  ent->s.generic1 = OnSameTeam(self, attacker);
  if (!((g_gametype.integer == GT_ELIMINATION ||
         g_gametype.integer == GT_CTF_ELIMINATION) &&
        level.time < level.roundStartTime)) {
    ent->r.svFlags = SVF_BROADCAST; // send to everyone (if not an elimination
                                    // gametype during active warmup)
  } else {
    ent->r.svFlags = SVF_NOCLIENT;
  }
  ...
  if (attacker && attacker->client) {
    ...
    if (attacker == self || OnSameTeam(self, attacker)) {
      ...
      if (meansOfDeath == MOD_GAUNTLET) {
        ...
        G_LogPrintf("Award: %i %i: %s gained the %s award!\n",
                    attacker->client->ps.clientNum, 0,
                    attacker->client->pers.netname, "GAUNTLET");
      ...
      if (level.time - attacker->client->lastKillTime < CARNAGE_REWARD_TIME) {
        ...
        G_LogPrintf("Award: %i %i: %s gained the %s award!\n",
                    attacker->client->ps.clientNum, 1,
                    attacker->client->pers.netname, "EXCELLENT");
        ...
        // Must be 1 so the correct number of kills are displayed to the
        // clients.
    ...
    if (g_gametype.integer != GT_LMS && g_gametype.integer != GT_POSSESSION &&
        !((g_gametype.integer == GT_ELIMINATION ||
           g_gametype.integer == GT_CTF_ELIMINATION) &&
          level.time < level.roundStartTime)) {
      if (self->client->ps.persistant[PERS_SCORE] > 0 ||
          level.numNonSpectatorClients < 3) {
        // Cannot get negative scores by suicide
  ...
  if (meansOfDeath == MOD_SUICIDE) {
    if (self->client->ps
            .powerups[PW_NEUTRALFLAG]) { // only happens in One Flag CTF
      Team_ReturnFlag(TEAM_FREE);
      self->client->ps.powerups[PW_NEUTRALFLAG] = 0;
    } else if (self->client->ps
                   .powerups[PW_REDFLAG]) { // only happens in standard CTF
      Team_ReturnFlag(TEAM_RED);
      self->client->ps.powerups[PW_REDFLAG] = 0;
    } else if (self->client->ps
                   .powerups[PW_BLUEFLAG]) { // only happens in standard CTF
  ...
  if (g_respawntime.integer > 0) {
    for (i = 0; self->client->respawnTime > i * g_respawntime.integer * 1000;
         i++)
      ;
...
================
*/
int RaySphereIntersections(vec3_t origin, float radius, vec3_t point,
                           vec3_t dir, vec3_t intersections[2]) {
  float b, c, d, t;

  //	| origin - (point + t * dir) | = radius
  //	a = dir[0]^2 + dir[1]^2 + dir[2]^2;
  //	b = 2 * (dir[0] * (point[0] - origin[0]) + dir[1] * (point[1] -
  //origin[1]) + dir[2] * (point[2] - origin[2])); 	c = (point[0] - origin[0])^2
  //+ (point[1] - origin[1])^2 + (point[2] - origin[2])^2 - radius^2;

  // normalize dir so a = 1
  VectorNormalize(dir);
  b = 2 * (dir[0] * (point[0] - origin[0]) + dir[1] * (point[1] - origin[1]) +
...
static int catchup_damage(int damage, int attacker_points, int target_points) {
  ...
  newdamage = damage - ((attacker_points - target_points - 5) *
                        (g_catchup.integer * damage)) /
                           100;
...
void G_Damage(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
  ...
  if ((attacker == &g_entities[ENTITYNUM_WORLD] || attacker == targ ||
  ...
  if (attacker->client && attacker != targ) {
    ...
    if (bg_itemlist[attacker->client->ps.stats[STAT_PERSISTANT_POWERUP]]
            .giTag == PW_GUARD) {
      max /= 2;
    }
  ...
  if (knockback && targ->client) {
    ...
    if (!g_awardpushing.integer || targ == attacker ||
  ...
  if (!(dflags & DAMAGE_NO_PROTECTION)) {
    ...
    if (mod != MOD_JUICED && mod != MOD_CRUSH && targ != attacker &&
        !(dflags & DAMAGE_NO_TEAM_PROTECTION) && OnSameTeam(targ, attacker)) {
      if ((!g_friendlyFire.integer && g_gametype.integer != GT_ELIMINATION &&
           g_gametype.integer != GT_CTF_ELIMINATION) ||
    ...
    if (targ->client && targ->client->spawnprotected) {
      if (level.time > targ->client->respawnTime + g_spawnprotect.integer) {
        ...
        if ((mod > MOD_UNKNOWN && mod < MOD_WATER) || mod == MOD_TELEFRAG ||
  ...
  // add to the attacker's hit counter (if the target isn't a general entity
  // like a prox mine)
  ...
  if (targ && targ->client && attacker->client) {
    damage = catchup_damage(damage, attacker->client->ps.persistant[PERS_SCORE],
                            targ->client->ps.persistant[PERS_SCORE]);
  }
  ...
  // If vampire is enabled, gain health but not from self or teammate, cannot
  // steal more than targ has
  if (g_vampire.value > 0.0 && (targ != attacker) && take > 0 &&
      !(OnSameTeam(targ, attacker)) && attacker->health > 0 &&
      targ->health > 0) {
    if (take < targ->health) {
...
qboolean G_RadiusDamage(vec3_t origin, gentity_t *attacker, float damage,
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    if (CanDamage(ent, origin)) {
      ...
      dir[2] += 24;
      G_Damage(ent, NULL, attacker, dir, origin, (int)points, DAMAGE_RADIUS,
               mod);
    }
  }
...
void monster_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
                 int damage, int meansOfDeath) {
  int contents;
  int killer = 0;
