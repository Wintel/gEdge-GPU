...
// Some pointers for Double Domination so we don't need GFind (I think it might
...
Adds bonus point to a player if he is close to the point and on the team that
scores
================
*/
...
void Team_FragBonuses(gentity_t *targ, gentity_t *inflictor,
  ...
  if (tokens) {
    ...
    G_LogPrintf(
        ...
        attacker->client->ps.clientNum, team, 1, targ->client->ps.clientNum,
        tokens, attacker->client->pers.netname, targ->client->pers.netname,
        TeamName(team), tokens);
  ...
  if (targ->client->pers.teamState.lasthurtcarrier &&
    ...
    G_LogPrintf("Award: %i %i: %s gained the %s award!\n",
                attacker->client->ps.clientNum, 3,
                attacker->client->pers.netname, "DEFENCE");
    if (!level.hadBots)
  ...
  // We place the Double Domination bonus test here! This appears to be the best
  // place to place them.
  if (g_gametype.integer == GT_DOUBLE_D) {
    if (attacker->client->sess.sessionTeam ==
        level.pointStatusA) { // Attack must defend point A
      ...
      if (((VectorLength(v1) < CTF_TARGET_PROTECT_RADIUS &&
        ...
        if (attacker->client->sess.sessionTeam == level.pointStatusB &&
            level.time - level.timeTaken > (10 - DD_CLOSE) * 1000)
          AddScore(attacker, targ->r.currentOrigin,
                   DD_POINT_DEFENCE_CLOSE_BONUS);
        else
    ...
    if (attacker->client->sess.sessionTeam ==
        level.pointStatusB) { // Attack must defend point B
      ...
      if (((VectorLength(v1) < CTF_TARGET_PROTECT_RADIUS &&
        ...
        // We defended point B
        // Was we dominating and maybe close to score?
        if (attacker->client->sess.sessionTeam == level.pointStatusA &&
            level.time - level.timeTaken > (10 - DD_CLOSE) * 1000)
          AddScore(attacker, targ->r.currentOrigin,
                   DD_POINT_DEFENCE_CLOSE_BONUS);
        else
    ...
    return; // In double Domination we shall not go on, or we would test for
            // team bases that we don't use
  }
  ...
  if (((VectorLength(v1) < CTF_TARGET_PROTECT_RADIUS &&
      ...
      (g_gametype.integer != GT_CTF_ELIMINATION ||
       ...
       ((level.eliminationSides + level.roundNumber) % 2 == 0 &&
        attacker->client->sess.sessionTeam == TEAM_BLUE) ||
       ((level.eliminationSides + level.roundNumber) % 2 == 1 &&
...
void Team_Dom_SpawnPoints(void) {
  ...
  while ((flag = G_Find(flag, FOFS(classname), c)) != NULL) {
    ...
    if (flag->message) {
      Q_strncpyz(level.domination_points_names[i], flag->message,
                 MAX_DOMINATION_POINTS_NAMES - 1);
      PrintMsg(NULL, "Domination point \'%s\' found\n",
               level.domination_points_names[i]);
    } else {
      Q_strncpyz(level.domination_points_names[i], va("Point %i", i),
                 MAX_DOMINATION_POINTS_NAMES - 1);
...
int getDomPointNumber(gentity_t *point) {
  int i;
  for (i = 1; i < MAX_DOMINATION_POINTS && i < level.domination_points_count;
       i++) {
...
void Team_Dom_TakePoint(gentity_t *point, int team, int clientnumber) {
  ...
  G_LogPrintf("DOM: %i %i %i %i: %s takes point %s!\n", clientnumber, i, 0,
              team, TeamName(team), level.domination_points_names[i]);
...
static void Team_ResetFlags(void) {
  if (g_gametype.integer == GT_CTF ||
    ...
    Team_ResetFlag(TEAM_BLUE);
             ...
             g_gametype.integer == GT_POSSESSION) {
...
void Team_ReturnFlagSound(gentity_t *ent, int team) {
  ...
  if ((level.time <= level.roundStartTime &&
       level.time >
           level.roundStartTime - 1000 * g_elimination_activewarmup.integer) &&
      g_gametype.integer == GT_CTF_ELIMINATION) {
    return;
  }
...
int Team_TouchDoubleDominationPoint(gentity_t *ent, gentity_t *other,
                                    int team) {
  ...
  if (team == TEAM_RED) { // We have touched point A
    ...
    G_LogPrintf("DD: %i %i %i: %s took point A for %s!\n", cl->ps.clientNum,
                clientTeam, 0, cl->pers.netname, TeamName(clientTeam));
    // Give personal score
  ...
  if (team == TEAM_BLUE) { // We have touched point B
    ...
    G_LogPrintf("DD: %i %i %i: %s took point B for %s!\n", cl->ps.clientNum,
                clientTeam, 1, cl->pers.netname, TeamName(clientTeam));
    // Give personal score
    score = DD_POINT_CAPTURE; // Base score for capture
    if (otherDominating) {
      score += DD_POINT_CAPTURE_BREAK;
...
int Team_TouchOurFlag(gentity_t *ent, gentity_t *other, int team) {
    ...
    if (ent->flags & FL_DROPPED_ITEM) {
      ...
      if (g_gametype.integer == GT_CTF) {
        G_LogPrintf("CTF: %i %i %i: %s returned the %s flag!\n",
                    cl->ps.clientNum, team, 2, cl->pers.netname,
                    TeamName(team));
      } else if (g_gametype.integer == GT_CTF_ELIMINATION) {
  ...
  G_LogPrintf("Award: %i %i: %s gained the %s award!\n",
              other->client->ps.clientNum, 4, other->client->pers.netname,
              "CAPTURE");
  ...
  for (i = 0; i < g_maxclients.integer; i++) {
    ...
    if (player->client->sess.sessionTeam != cl->sess.sessionTeam) {
      ...
      if (player->client->pers.teamState.lastreturnedflag +
        ...
        G_LogPrintf("Award: %i %i: %s gained the %s award!\n",
                    player->client->ps.clientNum, 5,
                    player->client->pers.netname, "ASSIST");
        // add the sprite over the player's head
...
int Team_TouchEnemyFlag(gentity_t *ent, gentity_t *other, int team) {
  ...
  if (g_gametype.integer == GT_1FCTF) {
    ...
    if (g_gametype.integer == GT_CTF) {
      G_LogPrintf("CTF: %i %i %i: %s got the %s flag!\n", cl->ps.clientNum,
                  team, 0, cl->pers.netname, TeamName(team));
    } else if (g_gametype.integer == GT_CTF_ELIMINATION) {
      G_LogPrintf("CTF_ELIMINATION: %i %i %i %i: %s got the %s flag!\n",
                  level.roundNumber, cl->ps.clientNum, team, 0,
                  cl->pers.netname, TeamName(team));
    }
  ...
  return -1; // Do not respawn this automatically, but do delete it if it was
             // FL_DROPPED
}
...
gentity_t *SelectRandomEntityFilter(const char *classname,
                                    qboolean (*filter)(const gentity_t *)) {
  gentity_t *spot;
  int count;
  int selection;
...
============
*/
gentity_t *SelectDoubleDominationSpawnPoint(team_t team, vec3_t origin,
                                            vec3_t angles) {
  gentity_t *spot;
...
============
*/
gentity_t *SelectDominationSpawnPoint(team_t team, vec3_t origin,
                                      vec3_t angles) {
  gentity_t *spot;
...
void TeamplayInfoMessage(gentity_t *ent) {
  ...
  if (ent->client->sess.sessionTeam == TEAM_SPECTATOR) {
    ...
    team =
        g_entities[ent->client->sess.spectatorClient].client->sess.sessionTeam;
  } else {
  ...
  for (i = 0, cnt = 0;
       i < g_maxclients.integer && i < MAX_CLIENTS && cnt < TEAM_MAXOVERLAY;
       i++) {
  ...
  for (i = 0, cnt = 0;
       i < g_maxclients.integer && i < MAX_GENTITIES && cnt < TEAM_MAXOVERLAY;
       i++) {
    player = g_entities + i;
    if (player->inuse && player->client->sess.sessionTeam == team) {
      ...
      Com_sprintf(entry, sizeof(entry), " %i %i %i %i %i %i",
                  ...
                  i, player->client->pers.teamState.location, h, a, w,
                  player->s.powerups);
      j = strlen(entry);
...
The message-key gives the point a name that is showed while playing (MAX length:
19 chars)
*/
...
static void ObeliskRegen(gentity_t *self) {
  ...
  self->activator->s.modelindex2 =
      self->health * 0xff / g_obeliskHealth.integer;
...
static void ObeliskDie(gentity_t *self, gentity_t *inflictor,
  ...
  G_LogPrintf("Award: %i %i: %s gained the %s award!\n",
              attacker->client->ps.clientNum, 4, attacker->client->pers.netname,
              "CAPTURE");
  G_LogPrintf("OBELISK: %i %i %i %i: %s destroyed the enemy obelisk.\n",
              attacker->client->ps.clientNum,
              attacker->client->sess.sessionTeam, 3, 0,
              attacker->client->pers.netname);
  if (TeamCount(-1, TEAM_RED) && TeamCount(-1, TEAM_BLUE) && !level.hadBots) {
...
static void ObeliskTouch(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  G_LogPrintf("HARVESTER: %i %i %i %i %i: %s brought in %i skull%s for %s\n",
              other->client->ps.clientNum, other->client->sess.sessionTeam, 0,
              -1, tokens, other->client->pers.netname, tokens,
              tokens > 1 ? "s" : "", TeamName(other->client->sess.sessionTeam));
...
static void ObeliskPain(gentity_t *self, gentity_t *attacker, int damage) {
  ...
  G_LogPrintf(
      ...
      attacker->client->ps.clientNum, attacker->client->sess.sessionTeam, 1,
      actualDamage, attacker->client->pers.netname, actualDamage);
}
...
================
*/
qboolean CheckObeliskAttack(const gentity_t *obelisk,
  ...
  if ((obelisk->spawnflags == TEAM_RED &&
       teamgame.redObeliskAttackedTime <
           level.time - OVERLOAD_ATTACK_BASE_SOUND_TIME) ||
      (obelisk->spawnflags == TEAM_BLUE &&
       teamgame.blueObeliskAttackedTime <
           level.time - OVERLOAD_ATTACK_BASE_SOUND_TIME)) {
 ...
 *Currently there is only one type so type is ignored. You can add total
randomizaion or waighted randomization later.
 *
...
void ShuffleTeams(void) {
  ...
  for (i = 0; i < level.numConnectedClients; i++) {
    if (g_entities[&level.clients[level.sortedClients[i]] - level.clients]
            .r.svFlags &
        SVF_BOT)
    ...
    if (level.clients[level.sortedClients[i]].sess.sessionTeam == TEAM_RED ||
      ...
      // For every second client we chenge team. But we do it a little of to
      // make it slightly more fair
      ...
      // Set the team
      // We do not run all the logic because we shall run map_restart in a
      // moment.
