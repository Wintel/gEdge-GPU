...
void trap_Printf(const char *fmt) { syscall(G_PRINT, fmt); }
...
int trap_AAS_PredictRoute(void /*struct aas_predictroute_s*/ *route,
                          ...
                          int stopareanum) {
  return syscall(BOTLIB_AAS_PREDICT_ROUTE, route, areanum, origin, goalareanum,
                 travelflags, maxareas, maxtime, stopevent, stopcontents,
                 stoptfl, stopareanum);
}
...
int trap_BotMoveInDirection(int movestate, vec3_t dir, float speed, int type) {
  return syscall(BOTLIB_AI_MOVE_IN_DIRECTION, movestate, dir, PASSFLOAT(speed),
                 type);
}
...
int trap_GeneticParentsAndChildSelection(int numranks, float *ranks,
                                         ...
                                         int *child) {
  return syscall(BOTLIB_AI_GENETIC_PARENTS_AND_CHILD_SELECTION, numranks, ranks,
