...
int BotValidChatPosition(bot_state_t *bs) {
  ...
  if (bs->inventory[INVENTORY_QUAD] ||
      ...
      bs->inventory[INVENTORY_HASTE] || bs->inventory[INVENTORY_INVISIBILITY] ||
      bs->inventory[INVENTORY_REGEN] || bs->inventory[INVENTORY_FLIGHT])
    return qfalse;
...
int BotChat_Death(bot_state_t *bs) {
  ...
  if (bs->lastchat_time > FloatTime() - TIME_BETWEENCHATTING)
    return qfalse;
  rnd = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_DEATH, 0,
                                   1);
  // don't chat in tournament mode
  if (gametype == GT_TOURNAMENT)
    return qfalse;
  ...
  if (TeamPlayIsOn() && BotSameTeam(bs, bs->lastkilledby)) {
    ...
    if (bs->botdeathtype == MOD_WATER)
      ...
      BotAI_BotInitialChat(bs, "death_cratered", BotRandomOpponentName(bs),
                           NULL);
    ...
    else if (bs->botdeathtype == MOD_KAMIKAZE &&
      ...
      if ((bs->botdeathtype == MOD_GAUNTLET ||
           bs->botdeathtype == MOD_RAILGUN || bs->botdeathtype == MOD_BFG ||
      ...
      else if (random() < trap_Characteristic_BFloat(bs->character,
...
int BotChat_Kill(bot_state_t *bs) {
  ...
  if (bs->lastchat_time > FloatTime() - TIME_BETWEENCHATTING)
    return qfalse;
  rnd =
      trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_KILL, 0, 1);
  // don't chat in tournament mode
  if (gametype == GT_TOURNAMENT)
    return qfalse;
  // if fast chat is off
  ...
  if (TeamPlayIsOn() && BotSameTeam(bs, bs->lastkilledplayer)) {
    ...
    // choose between insult and praise
    else if (random() < trap_Characteristic_BFloat(
...
int BotChat_HitTalking(bot_state_t *bs) {
  ...
  ClientName(g_entities[bs->client].client->lasthurt_client, name,
             sizeof(name));
  weap =
      BotWeaponNameForMeansOfDeath(g_entities[bs->client].client->lasthurt_mod);
  //
...
int BotChat_HitNoKill(bot_state_t *bs) {
  ...
  if (BotNumActivePlayers() <= 1)
    return qfalse;
  rnd = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_HITNOKILL,
                                   0, 1);
  // don't chat in teamplay
  if (TeamPlayIsOn())
    return qfalse;
  ...
  weap =
      BotWeaponNameForMeansOfDeath(g_entities[bs->enemy].client->lasthurt_mod);
  //
...
int BotChat_Random(bot_state_t *bs) {
  ...
  if (bs->ltgtype == LTG_TEAMHELP || bs->ltgtype == LTG_TEAMACCOMPANY ||
    ...
    return qfalse;
  //
  rnd = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_RANDOM, 0,
                                   1);
