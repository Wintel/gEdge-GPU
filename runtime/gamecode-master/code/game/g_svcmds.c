...
void ClientKick_f(void) {
  ...
  // Now clientkick has been moved into game, but we still need to find the
  // idnum the server expects....
  // FIXME: To fix this, we need a relieble way to generate difference between
  // the server's client number and the game's client numbers
