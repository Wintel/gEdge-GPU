...
void FinishSpawningItem(gentity_t *ent) {
  ...
  if (g_gametype.integer != GT_ELIMINATION &&
      ...
      !g_elimination_allgametypes.integer && !g_rockets.integer)
    if (ent->item->giType == IT_POWERUP) {
      float respawn;
...
void G_SpawnItem(gentity_t *ent, gitem_t *item) {
  ...
  if (g_gametype.integer == GT_DOUBLE_D &&
      (strequals(ent->classname, "team_CTF_redflag") ||
       strequals(ent->classname, "team_CTF_blueflag") ||
       strequals(ent->classname, "team_CTF_neutralflag") ||
       item->giType == IT_PERSISTANT_POWERUP)) {
    ent->s.eFlags |= EF_NODRAW; // Don't draw the flag models/persistant
                                // powerups
  }
  ...
  if (g_gametype.integer == GT_POSSESSION &&
      (strequals(ent->classname, "team_CTF_redflag") ||
       strequals(ent->classname, "team_CTF_blueflag"))) {
    ent->s.eFlags |=
        EF_NODRAW; // Don't draw the flag colored flags in possession
  }

  if (strequals(ent->classname, "domination_point")) {
    ent->s.eFlags |=
        EF_NODRAW; // Don't draw domination_point. It is just a pointer to where
                   // the Domination points should be placed
  }
