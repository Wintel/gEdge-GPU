...
void Bullet_Fire(gentity_t *ent, float spread, int damage) {
  ...
  */
  ...
  for (i = 0; i < 10; i++) {
    ...
    if (traceEnt->takedamage) {
      if (traceEnt->client &&
          traceEnt->client->invulnerabilityTime > level.time) {
        if (G_InvulnerabilityEffect(traceEnt, forward, tr.endpos, impactpoint,
                                    bouncedir)) {
...
void weapon_railgun_fire(gentity_t *ent) {
  ...
  do {
    ...
    if (traceEnt->takedamage) {
      if (traceEnt->client &&
          traceEnt->client->invulnerabilityTime > level.time) {
        if (G_InvulnerabilityEffect(traceEnt, forward, trace.endpos,
                                    impactpoint, bouncedir)) {
          G_BounceProjectile(muzzle, impactpoint, bouncedir, end);
          // snap the endpos to integers to save net bandwidth, but nudged
          // towards the line
  ...
  // snap the endpos to integers to save net bandwidth, but nudged towards the
  // line
  SnapVectorTowards(trace.endpos, muzzle);
...
void Weapon_LightningFire(gentity_t *ent) {
  ...
  for (i = 0; i < 10; i++) {
    ...
    // Sago: I'm not sure this should recieve backward reconciliation. It is not
    // a real instant hit weapon, it can normally be dogded unlagged - backward
    // reconciliation #2
    //  backward-reconcile the other clients
    G_DoTimeShiftFor(ent);
    // unlagged - backward reconciliation #2
    ...
    // if not the first trace (the lightning bounced of an invulnerability
    // sphere)
    if (i) {
...
void FireWeapon(gentity_t *ent) {
  // Make people drop out of follow mode (this should be moved, so people can
  // change betwean players.)
  ...
  // track shots taken for accuracy tracking.  Grapple is not a weapon and
  // gauntet is just not tracked
...
===============
*/
static void KamikazeRadiusDamage(vec3_t origin, gentity_t *attacker,
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    G_Damage(ent, NULL, attacker, dir, origin, damage,
             DAMAGE_RADIUS | DAMAGE_NO_TEAM_PROTECTION, MOD_KAMIKAZE);
...
===============
*/
static void KamikazeShockWave(vec3_t origin, gentity_t *attacker, float damage,
                              float push, float radius) {
  float dist;
  gentity_t *ent;
  int entityList[MAX_GENTITIES];
  int numListedEntities;
...
static void KamikazeDamage(gentity_t *self) {
  ...
  for (i = 0; i < MAX_CLIENTS; i++) {
    ...
    ent->client->ps.delta_angles[0] +=
        ANGLE2SHORT(newangles[0] - self->movedir[0]);
    ent->client->ps.delta_angles[1] +=
        ANGLE2SHORT(newangles[1] - self->movedir[1]);
    ent->client->ps.delta_angles[2] +=
        ANGLE2SHORT(newangles[2] - self->movedir[2]);
  }
