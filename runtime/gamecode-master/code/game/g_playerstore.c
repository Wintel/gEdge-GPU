...
typedef struct {
  ...
  int age; // int that grows each time a new player is stored. The lowest number
           // is always replaced. Reset to 0 then retrieved.
  int persistant[MAX_PERSISTANT]; // This is the only information we need to
                                  // save
  int timePlayed;
...
/**
 * Logs all accuracy values for a given client. This should be called just
 * before the game ends and then a client leaves.
 * @param clientNum
 */
void LogAcc(int clientNum) {
  ...
  for (i = 0; i < WP_NUM_WEAPONS; ++i) {
    char *tmp = va("%sf%i\\%i\\h%i\\%i", first ? "" : "\\", i,
                   level.clients[clientNum].accuracy[i][0], i,
                   level.clients[clientNum].accuracy[i][1]);
...
void PlayerStore_store(const char *guid, playerState_t ps) {
  ...
  memcpy(playerstore[place2store].persistant, ps.persistant,
         sizeof(int[MAX_PERSISTANT]));
  memcpy(playerstore[place2store].accuracy,
         level.clients[ps.clientNum].accuracy, sizeof(playerstore[0].accuracy));
  playerstore[place2store].timePlayed =
      level.time - level.clients[ps.clientNum].pers.enterTime;
...
void PlayerStore_restore(const char *guid, playerState_t *ps) {
  ...
  for (i = 0; i < MAX_PLAYERS_STORED; i++) {
    if (Q_strequaln(guid, playerstore[i].guid, GUID_SIZE) &&
        playerstore[i].age != -1) {
      memcpy(ps->persistant, playerstore[i].persistant,
             sizeof(int[MAX_PERSISTANT]));
      memcpy(level.clients[ps->clientNum].accuracy, playerstore[i].accuracy,
             sizeof(playerstore[0].accuracy));
      level.clients[ps->clientNum].pers.enterTime =
          level.time - playerstore[i].timePlayed;
