...
==================
*/
qboolean G_TryPushingProxMine(gentity_t *check, gentity_t *pusher, vec3_t move,
                              vec3_t amove) {
  ...
  for (e = 0; e < listedEntities; e++) {
    ...
    if (check->s.eType == ET_MISSILE) {
      ...
      if (strequals(check->classname, "prox mine")) {
        // if this prox mine is attached to this mover try to move it with the
        // pusher
...
static void DamagePlayer(gentity_t *ent, gentity_t *player) {
  if (g_awardpushing.integer) {
    G_Damage(player, ent, ent->activator, NULL, NULL, ent->damage, 0,
             MOD_CRUSH);
  } else {
...
static void Reached_Train(gentity_t *ent) {
  ...
  if (ent->s.pos.trDuration < 1) {
    // Tequila comment: As trDuration is used later in a division, we need to
    // avoid that case now With null trDuration, the calculated rocks bounding
    // box becomes infinite and the engine think for a short time any entity is
    // riding that mover but not the world entity... In rare case, I found it
    // can also stuck every map entities after func_door are used.
    // The desired effect with very very big speed is to have instant move, so
    // any not null duration lower than a frame duration should be sufficient.
...
void SP_func_pendulum(gentity_t *ent) {
  ...
  freq = 1 / (M_PI * 2) *
         sqrt(g_gravity.value * g_gravityModifier.value / (3 * length));
