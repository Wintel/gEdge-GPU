...
static qboolean EntityFilterNoBotsOrHumanOnly(const gentity_t *item) {
  if (item->flags & FL_NO_BOTS || item->flags & FL_NO_HUMANS) {
    // We cannot select a nobots or nohuman spawnpoint. These can be outside the
    // map
    return qfalse;
  }
  return qtrue;
...
void Possession_SpawnFlag(void) {
  ...
  if (ent) {
    possessionFlag = ent;
    G_LogPrintf(
        "Info: The map contains a white flag. We do not spawn a new one.\n");
    return;
  }
  if (!ent) {
  ...
  if (!ent) {
    ...
    ent = SelectRandomEntityFilter("info_player_deathmatch",
