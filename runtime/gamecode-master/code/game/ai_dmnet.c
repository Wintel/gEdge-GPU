...
int BotGoForAir(bot_state_t *bs, int tfl, bot_goal_t *ltg, float range) {
  ...
  if (bs->lastair_time < FloatTime() - 6) {
    ...
    if (BotGetAirGoal(bs, &goal)) {
      ...
      while (trap_BotChooseNBGItem(bs->gs, bs->origin, bs->inventory, tfl, ltg,
                                   range)) {
...
int BotNearbyGoal(bot_state_t *bs, int tfl, bot_goal_t *ltg, float range) {
  ...
  if (BotCTFCarryingFlag(bs)
    ...
    if (trap_AAS_AreaTravelTimeToGoalArea(
            bs->areanum, bs->origin, bs->teamgoal.areanum, TFL_DEFAULT) < 300) {
...
int BotReachedGoal(bot_state_t *bs, bot_goal_t *goal) {
  if (goal->flags & GFL_ITEM) {
    ...
    if (trap_BotItemGoalInVisButNotVisible(bs->entitynum, bs->eye,
      ...
      goal->areanum, bs->tfl); if ((float) t * 0.009 < avoidtime) return qtrue;
      }
      */
      return qtrue;
    }
...
int BotGetItemLongTermGoal(bot_state_t *bs, int tfl, bot_goal_t *goal) {
  ...
  if (bs->ltg_time < FloatTime()) {
    ...
    trap_BotPopGoal(bs->gs);
    // BotAI_Print(PRT_MESSAGE, "%s: choosing new ltg\n", ClientName(bs->client,
    // netname, sizeof(netname))); choose a new goal BotAI_Print(PRT_MESSAGE,
    // "%6.1f client %d: BotChooseLTGItem\n", FloatTime(), bs->client);
...
*/
    ...
    } else { // the bot gets sorta stuck with all the avoid timings, shouldn't
             // happen though
             //
#ifdef DEBUG
      char netname[128];
...
int BotGetLongTermGoal(bot_state_t *bs, int tfl, int retreat,
  ...
  //              memcpy(&bs->teamgoal, &dom_points_bot[((rand()) %
  //              (level.domination_points_count))], sizeof(bot_goal_t));
...
int BotSelectActivateWeapon(bot_state_t *bs) {
  //
  if (bs->inventory[INVENTORY_MACHINEGUN] > 0 &&
      bs->inventory[INVENTORY_BULLETS] > 0)
    return WEAPONINDEX_MACHINEGUN;
  else if (bs->inventory[INVENTORY_SHOTGUN] > 0 &&
           bs->inventory[INVENTORY_SHELLS] > 0)
    return WEAPONINDEX_SHOTGUN;
  else if (bs->inventory[INVENTORY_PLASMAGUN] > 0 &&
  ...
  else if (bs->inventory[INVENTORY_CHAINGUN] > 0 &&
           bs->inventory[INVENTORY_BELT] > 0)
    return WEAPONINDEX_CHAINGUN;
  else if (bs->inventory[INVENTORY_NAILGUN] > 0 &&
           bs->inventory[INVENTORY_NAILS] > 0)
    return WEAPONINDEX_NAILGUN;
  else if (bs->inventory[INVENTORY_PROXLAUNCHER] > 0 &&
           bs->inventory[INVENTORY_MINES] > 0)
    return WEAPONINDEX_PROXLAUNCHER;
  else if (bs->inventory[INVENTORY_GRENADELAUNCHER] > 0 &&
           bs->inventory[INVENTORY_GRENADES] > 0)
    return WEAPONINDEX_GRENADE_LAUNCHER;
  else if (bs->inventory[INVENTORY_RAILGUN] > 0 &&
           bs->inventory[INVENTORY_SLUGS] > 0)
    return WEAPONINDEX_RAILGUN;
  else if (bs->inventory[INVENTORY_ROCKETLAUNCHER] > 0 &&
           bs->inventory[INVENTORY_ROCKETS] > 0)
    return WEAPONINDEX_ROCKET_LAUNCHER;
  else if (bs->inventory[INVENTORY_BFG10K] > 0 &&
  ...
  if (bs->kamikazebody) {
    ...
    if (!(moveresult->flags &
      ...
      if (moveresult->weapon) {
        //
        moveresult->flags |=
            MOVERESULT_MOVEMENTWEAPON | MOVERESULT_MOVEMENTVIEW;
...
int AINode_Seek_NBG(bot_state_t *bs) {
  ...
  if (bs->nbg_time < FloatTime()) {
    ...
    // NOTE: we canNOT reset the check_time to zero because it would create an
    // endless loop of node switches
...
int AINode_Battle_Retreat(bot_state_t *bs) {
  ...
  // update the attack inventory values
  BotUpdateBattleInventory(bs, bs->enemy);
  // if the bot doesn't want to retreat anymore... probably picked up some nice
  // items
  if (BotWantsToChase(bs)) {
