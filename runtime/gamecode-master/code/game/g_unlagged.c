...
void G_TimeShiftClient(gentity_t *ent, int time, qboolean debug,
  ...
  if (j != k) {
    ...
    if (j != ent->client->historyHead) {
      ...
      // interpolate between the two origins to give position at time index
      // "time"
...
void G_TimeShiftAllClients(int time, gentity_t *skip) {
  ...
  gentity_t *ent;
  qboolean debug =
      (skip != NULL && skip->client && skip->s.weapon == WP_RAILGUN);
...
void G_DoTimeShiftFor(gentity_t *ent) {
  ...
  if (g_delagHitscan.integer &&
    ...
    // Give the lightning gun some handicap (lag was part of weapon balance in
    // VQ3)
