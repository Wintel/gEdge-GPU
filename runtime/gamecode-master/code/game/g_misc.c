...
static void PortalDie(gentity_t *self, gentity_t *inflictor,
...
void DropPortalDestination(gentity_t *player) {
  ...
  player->client->ps.stats[STAT_HOLDABLE_ITEM] =
      BG_FindItem("Portal") - bg_itemlist;
}
...
static void PortalTouch(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  destination = NULL;
  while ((destination = G_Find(destination, FOFS(classname),
  ...
  if (!destination) {
    ...
    G_Damage(other, other, other, NULL, NULL, 100000, DAMAGE_NO_PROTECTION,
             MOD_TELEFRAG);
    return;
  }
