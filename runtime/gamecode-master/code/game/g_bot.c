...
qboolean G_BotConnect(int clientNum, qboolean restart) {
  ...
  if (!trap_AAS_Initialized() ||
      !BotAISetupClient(clientNum, &settings, restart)) {
...
static void G_AddBot(const char *name, float skill, const char *team, int delay,
  ...
  Info_SetValueForKey(userinfo, "characterfile",
...
void G_InitBots(qboolean restart) {
  ...
  trap_Cvar_Register(&bot_autominplayers, "bot_autominplayers", "0",
                     CVAR_SERVERINFO);
