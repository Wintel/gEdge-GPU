...
static char *ExtractLine(char *data) {
  /* Goal:
   ...
   that is no longer than MAX_LINE_LENGTH characters long.  Return value is
...
int main(int argc, char **argv) {
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (!strcmp(argv[i], "-o")) {
      ...
      /* Timbo of Tremulous pointed out -o not working; stock ID q3asm folded in
       * the change. Yay. */
    ...
    if (!strcmp(argv[i], "-v")) {
      ...
      errors. Verbosity turns on all messages, error or not. Motivation: not
      wanting to scrollback for pages to find asm error.
      */
