...
Tree value(Tree p) {
  ...
  if (p->type != voidtype &&
      (op == AND || op == OR || op == NOT || op == EQ || op == NE || op == LE ||
       op == LT || op == GE || op == GT))
...
static Tree postfix(Tree p) {
  for (;;)
    ...
    case INCR:
      p = tree(RIGHT, p->type,
               tree(RIGHT, p->type, p, incr(t, p, consttree(1, inttype))), p);
      t = gettok();
      break;
    case DECR:
      p = tree(RIGHT, p->type,
...
static Tree primary(void) {
  ...
  case SCON:
    if (ischar(tsym->type->type))
      ...
      tsym->u.c.v.p = memcpy(allocate(tsym->type->size, PERM), tsym->u.c.v.p,
                             tsym->type->size);
    ...
    if (tsym == NULL) {
      ...
      if (getchr() == '(') {
        ...
        if (q && !eqtype(q->type, p->type, 1))
          warning("implicit declaration of `%s' does not match previous "
                  "declaration at %w\n",
                  q->name, &q->src);
...
Tree cond(Tree p) {
  ...
  if (op == AND || op == OR || op == NOT || op == EQ || op == NE || op == LE ||
      op == LT || op == GE || op == GT)
    return p;
  p = pointer(p);
      ...
      if (src->op != dst->op)
          ...
          if (isfloat(dst)) {
            ...
            p = (*optree['+'])(
                ...
                (*optree['*'])(
                    MUL, two,
                    simplify(CVU, ssrc,
                             simplify(RSH, src, p, consttree(1, inttype)),
                             NULL)),
                simplify(CVU, ssrc,
                         simplify(BAND, src, p, consttree(1, unsignedtype)),
                         NULL));
          } else
        ...
        case FLOAT:
          if (isunsigned(dst)) {
            ...
            Tree c = cast(
                cnsttree(longdouble, (double)sdst->u.sym->u.limits.max.i + 1),
                src);
            p = condtree(
                simplify(GE, src, p, c),
                (*optree['+'])(
                    ...
                    cast(cnsttree(unsignedlong,
                                  (unsigned long)sdst->u.sym->u.limits.max.i +
                                      1),
                         dst)),
