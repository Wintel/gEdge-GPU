...
void compound(int loop, struct swtch *swp, int lev) {
  ...
  if (level_lcc == LOCAL && IR->wants_callb && isstruct(freturn(cfunc->type))) {
...
static void checkref(Symbol p, void *cl) {
  ...
  assert(!(level_lcc == GLOBAL && p->sclass == STATIC && !p->defined &&
           !isfunc(p->type)));
}
  ...
  if (t == '=') {
    ...
    if (isscalar(p->type) || (isstruct(p->type) && t != '{')) {
      ...
      if (isarray(p->type) && p->type->size == 0 && t1->type->size > 0)
        p->type =
            array(p->type->type, t1->type->size / t1->type->type->size, 0);
      e = idtree(t1);
    }
