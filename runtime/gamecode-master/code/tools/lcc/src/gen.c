...
static Symbol spillee(Symbol set, unsigned mask[], Node here) {
  ...
  assert(bestreg); /* Must be able to spill something. Reconfigure the register
          allocator to ensure that we can allocate a register for all nodes
          without spilling the node's necessary input regs. */
...
void spill(unsigned mask, int n, Node here) {
  ...
  if (mask & ~freemask[n]) {
    ...
    assert(/* It makes no sense for a node to clobber() its target. */
           ...
           here->syms[RX] == NULL || here->syms[RX]->x.regnode == NULL ||
