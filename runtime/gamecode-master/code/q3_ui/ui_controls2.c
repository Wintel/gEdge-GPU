...
static const char *autoswitch_items[] = {"NEVER",  "ALWAYS",     "NEW",
                                         "BETTER", "NEW&BETTER", NULL};

// OLD
// static vec4_t controls_binding_color  = {1.00f, 0.43f, 0.00f, 1.00f}; // bk:
// Win32 C4305
...
static configcvar_t g_configcvars[] = {
    ...
    {"m_filter", 0, 0},    {"cl_freelook", 0, 0}, {"cg_voipTeamOnly", 0, 0},
    {NULL, 0, 0}};
...
static void Controls_UpdateModel(int anim) {
  ...
  UI_PlayerInfo_SetInfo(&s_controls.playerinfo, s_controls.playerLegs,
                        s_controls.playerTorso, s_controls.playerViewangles,
                        s_controls.playerMoveangles, s_controls.playerWeapon,
...
static void Controls_Update(void) {
  ...
  // enable controls in active group (and count number of items for vertical
  // centering) bk001204 - parentheses
  for (j = 0; (control = controls[j]); j++) {
  ...
  if (s_controls.waitingforkey) {
    ...
    // enable action item
    ((menucommon_s *)(s_controls.menu.items[s_controls.menu.cursor]))->flags &=
        ~QMF_GRAYED;
...
static void Controls_DrawKeyBinding(void *self) {
  ...
  if (c) {
    UI_FillRect(a->generic.left, a->generic.top,
                a->generic.right - a->generic.left + 1,
                a->generic.bottom - a->generic.top + 1, listbar_color);

    UI_DrawString(x - SMALLCHAR_WIDTH, y, g_bindings[a->generic.id].label,
                  UI_RIGHT | UI_SMALLFONT, text_color_highlight);
    UI_DrawString(x + SMALLCHAR_WIDTH, y, name,
                  UI_LEFT | UI_SMALLFONT | UI_PULSE, text_color_highlight);

    if (s_controls.waitingforkey) {
      UI_DrawChar(x, y, '=', UI_CENTER | UI_BLINK | UI_SMALLFONT,
                  text_color_highlight);
      UI_DrawString(SCREEN_WIDTH * 0.50, SCREEN_HEIGHT * 0.80,
                    "Waiting for new key ... ESCAPE to cancel",
                    UI_SMALLFONT | UI_CENTER | UI_PULSE, colorWhite);
    } else {
      UI_DrawChar(x, y, 13, UI_CENTER | UI_BLINK | UI_SMALLFONT,
                  text_color_highlight);
      UI_DrawString(SCREEN_WIDTH * 0.50, SCREEN_HEIGHT * 0.78,
                    "Press ENTER or CLICK to change", UI_SMALLFONT | UI_CENTER,
                    colorWhite);
      UI_DrawString(SCREEN_WIDTH * 0.50, SCREEN_HEIGHT * 0.82,
                    "Press BACKSPACE to clear", UI_SMALLFONT | UI_CENTER,
                    colorWhite);
    }
  } else {
    if (a->generic.flags & QMF_GRAYED) {
      UI_DrawString(x - SMALLCHAR_WIDTH, y, g_bindings[a->generic.id].label,
                    UI_RIGHT | UI_SMALLFONT, text_color_disabled);
      UI_DrawString(x + SMALLCHAR_WIDTH, y, name, UI_LEFT | UI_SMALLFONT,
                    text_color_disabled);
    } else {
      UI_DrawString(x - SMALLCHAR_WIDTH, y, g_bindings[a->generic.id].label,
                    UI_RIGHT | UI_SMALLFONT, controls_binding_color);
      UI_DrawString(x + SMALLCHAR_WIDTH, y, name, UI_LEFT | UI_SMALLFONT,
...
static void Controls_StatusBar(void *self) {
  UI_DrawString(SCREEN_WIDTH * 0.50, SCREEN_HEIGHT * 0.80,
                "Use Arrow Keys or CLICK to change", UI_SMALLFONT | UI_CENTER,
                colorWhite);
}
...
static void Controls_GetConfig(void) {
  ...
  s_controls.autoswitch.curvalue =
      UI_ClampCvar(0, 4, Controls_GetCvarValue("cg_autoswitch"));
  ...
  s_controls.joythreshold.curvalue =
      UI_ClampCvar(0.05f, 0.75f, Controls_GetCvarValue("joy_threshold"));
  s_controls.freelook.curvalue =
      UI_ClampCvar(0, 1, Controls_GetCvarValue("cl_freelook"));
  s_controls.voip_teamonly.curvalue =
      UI_ClampCvar(0, 1, Controls_GetCvarValue("cg_voipTeamOnly"));
}
...
static void Controls_SetDefaults(void) {
  ...
  s_controls.voip_teamonly.curvalue =
      Controls_GetCvarDefault("cg_voipTeamOnly");
}
...
static void Controls_MenuEvent(void *ptr, int event) {
  ...
  case ID_DEFAULTS:
    if (event == QM_ACTIVATED) {
      UI_ConfirmMenu("SET TO DEFAULTS?", Controls_ResetDefaults_Draw,
                     Controls_ResetDefaults_Action);
    }
    break;
...
static void Controls_Menu_AddPText(menutext_s *menutext, int *y, int id,
                                   char *text) {
...
static void Controls_MenuInit(void) {
  ...
  s_controls.grapple.generic.flags =
      QMF_LEFT_JUSTIFY | QMF_PULSEIFFOCUS | QMF_GRAYED | QMF_HIDDEN;
  ...
  s_controls.nailgun.generic.flags =
      QMF_LEFT_JUSTIFY | QMF_PULSEIFFOCUS | QMF_GRAYED | QMF_HIDDEN;
  ...
  s_controls.proxmine.generic.flags =
      QMF_LEFT_JUSTIFY | QMF_PULSEIFFOCUS | QMF_GRAYED | QMF_HIDDEN;
  ...
  s_controls.chaingun.generic.flags =
      QMF_LEFT_JUSTIFY | QMF_PULSEIFFOCUS | QMF_GRAYED | QMF_HIDDEN;
  ...
  s_controls.mouselook.generic.flags =
      QMF_LEFT_JUSTIFY | QMF_HIGHLIGHT_IF_FOCUS | QMF_GRAYED | QMF_HIDDEN;
  ...
  s_controls.showacc.generic.flags =
      QMF_LEFT_JUSTIFY | QMF_PULSEIFFOCUS | QMF_GRAYED | QMF_HIDDEN;
  ...
  s_controls.voip_talk.generic.flags =
      QMF_LEFT_JUSTIFY | QMF_PULSEIFFOCUS | QMF_GRAYED | QMF_HIDDEN;
