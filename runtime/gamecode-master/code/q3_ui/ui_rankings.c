...
void Rankings_MenuInit(void) {
  ...
  s_rankings.logout.generic.flags =
      QMF_HIDDEN | QMF_INACTIVE | QMF_CENTER_JUSTIFY | QMF_PULSEIFFOCUS;
  ...
  if ((status == QGR_STATUS_VALIDATING) || (status == QGR_STATUS_PENDING) ||
...
===============
*/
void Rankings_Cache(void) { trap_R_RegisterShaderNoMip(RANKINGS_FRAME); }
