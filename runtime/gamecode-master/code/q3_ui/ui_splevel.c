...
=================
*/
static void PlayerIcon(const char *modelAndSkin, char *iconName,
                       int iconNameMaxSize) {
  char *skin;
...
static void UI_SPLevelMenu_SetBots(void) {
  ...
  Q_strncpyz(bots, Info_ValueForKey(levelMenuInfo.selectedArenaInfo, "bots"),
             sizeof(bots));
  ...
  while (*p && levelMenuInfo.numBots < 7) {
    ...
    if (botInfo) {
      levelMenuInfo.botPics[levelMenuInfo.numBots] =
          PlayerIconHandle(Info_ValueForKey(botInfo, "model"));
      Q_strncpyz(levelMenuInfo.botNames[levelMenuInfo.numBots],
                 Info_ValueForKey(botInfo, "name"), 10);
    } else {
...
=================
*/
static void UI_SPLevelMenu_SetMenuArena(int n, int level,
                                        const char *arenaInfo) {
  char map[MAX_QPATH];

  Q_strncpyz(map, Info_ValueForKey(arenaInfo, "map"), sizeof(map));

  Q_strncpyz(levelMenuInfo.levelNames[n], map,
             sizeof(levelMenuInfo.levelNames[n]));
  Q_strupr(levelMenuInfo.levelNames[n]);

  UI_GetBestScore(level, &levelMenuInfo.levelScores[n],
                  &levelMenuInfo.levelScoresSkill[n]);
...
static void UI_SPLevelMenu_ResetDraw(void) {
  ...
  UI_DrawProportionalString(SCREEN_WIDTH / 2, 356 + PROP_HEIGHT * 1,
                            ...
                            UI_CENTER | UI_SMALLFONT, color_yellow);
  UI_DrawProportionalString(SCREEN_WIDTH / 2, 356 + PROP_HEIGHT * 2,
                            "Do this only if you want to",
                            UI_CENTER | UI_SMALLFONT, color_yellow);
  UI_DrawProportionalString(SCREEN_WIDTH / 2, 356 + PROP_HEIGHT * 3,
...
static void UI_SPLevelMenu_ResetEvent(void *ptr, int event) {
  ...
  UI_ConfirmMenu("RESET GAME?", UI_SPLevelMenu_ResetDraw,
...
static void UI_SPLevelMenu_MenuDraw(void) {
  ...
  UI_DrawProportionalString(18, 38, va("Tier %i", selectedArenaSet + 1),
  ...
  if (selectedArenaSet > currentSet) {
    UI_DrawProportionalString(320, 216, "ACCESS DENIED", UI_CENTER | UI_BIGFONT,
                              color_red);
    return;
  }
  ...
  for (n = 0; n < levelMenuInfo.numMaps; n++) {
    ...
    UI_DrawString(x + 64, y + 96, levelMenuInfo.levelNames[n],
    ...
    if (levelMenuInfo.levelScores[n] == 1) {
      UI_DrawHandlePic(
          x, y, 128, 96,
          levelMenuInfo
              .levelCompletePic[levelMenuInfo.levelScoresSkill[n] - 1]);
    }

    if (n == selectedArena) {
      if (Menu_ItemAtCursor(&levelMenuInfo.menu) ==
          &levelMenuInfo.item_maps[n]) {
        trap_R_SetColor(color);
      }
      UI_DrawHandlePic(x - 1, y - 1, 130, 130 - 14,
                       levelMenuInfo.levelSelectedPic);
      trap_R_SetColor(NULL);
    } else if (Menu_ItemAtCursor(&levelMenuInfo.menu) ==
               &levelMenuInfo.item_maps[n]) {
      trap_R_SetColor(color);
      UI_DrawHandlePic(x - 31, y - 30, 256, 256 - 27,
  ...
  for (n = 0; n < levelMenuInfo.numBots; n++) {
    ...
    UI_DrawString(x, y + 64, levelMenuInfo.botNames[n], UI_SMALLFONT | UI_LEFT,
                  color_orange);
  }
}
...
void UI_SPLevelMenu_Cache(void) {
  ...
  levelMenuInfo.levelFocusPic =
      trap_R_RegisterShaderNoMip(ART_LEVELFRAME_FOCUS);
  levelMenuInfo.levelCompletePic[0] =
      trap_R_RegisterShaderNoMip(ART_MAP_COMPLETE1);
  levelMenuInfo.levelCompletePic[1] =
      trap_R_RegisterShaderNoMip(ART_MAP_COMPLETE2);
  levelMenuInfo.levelCompletePic[2] =
      trap_R_RegisterShaderNoMip(ART_MAP_COMPLETE3);
  levelMenuInfo.levelCompletePic[3] =
      trap_R_RegisterShaderNoMip(ART_MAP_COMPLETE4);
  levelMenuInfo.levelCompletePic[4] =
      trap_R_RegisterShaderNoMip(ART_MAP_COMPLETE5);
}
...
static void UI_SPLevelMenu_Init(void) {
  ...
  levelMenuInfo.item_rightarrow.generic.callback =
      UI_SPLevelMenu_RightArrowEvent;
  ...
  levelMenuInfo.awardLevels[AWARD_FRAGS] =
      100 * (levelMenuInfo.awardLevels[AWARD_FRAGS] / 100);
  ...
  for (n = 0; n < 6; n++) {
    if (levelMenuInfo.awardLevels[n]) {
      ...
      levelMenuInfo.item_awards[count].generic.flags =
          QMF_LEFT_JUSTIFY | QMF_SILENT | QMF_MOUSEONLY;
      ...
      levelMenuInfo.item_awards[count].generic.callback =
          UI_SPLevelMenu_AwardEvent;
...
=================
*/
void UI_SPLevelMenu_ReInit(void) { levelMenuInfo.reinit = qtrue; }
