...
static char *serverinfo_artlist[] = {SERVERINFO_FRAMEL, SERVERINFO_FRAMER,
...
static void ServerInfo_MenuDraw(void) {
  ...
  while (s && i < s_serverinfo.numlines) {
    ...
    UI_DrawString(SCREEN_WIDTH * 0.50 - 8, y, key, UI_RIGHT | UI_SMALLFONT,
                  color_red);
    UI_DrawString(SCREEN_WIDTH * 0.50 + 8, y, value, UI_LEFT | UI_SMALLFONT,
                  text_color_normal);
