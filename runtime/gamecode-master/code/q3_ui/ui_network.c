...
const char *rate_items[] = {"<= 28.8K", "33.6K",          "56K",
                            "ISDN",     "LAN/Cable/xDSL", NULL};
...
static void UI_NetworkOptionsMenu_Event(void *ptr, int event) {
  ...
  case ID_ALLOWDOWNLOAD:
    trap_Cvar_SetValue("cl_allowDownload",
                       networkOptionsInfo.allowdownload.curvalue);
...
static void UI_NetworkOptionsMenu_Init(void) {
  ...
  networkOptionsInfo.allowdownload.generic.flags =
      QMF_PULSEIFFOCUS | QMF_SMALLFONT;
  networkOptionsInfo.allowdownload.generic.callback =
      UI_NetworkOptionsMenu_Event;
  ...
  Menu_AddItem(&networkOptionsInfo.menu,
               (void *)&networkOptionsInfo.allowdownload);
  ...
  networkOptionsInfo.lagometer.curvalue =
      trap_Cvar_VariableValue("cg_lagometer") != 0;
}
