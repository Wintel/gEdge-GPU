...
static char *playermodel_artlist[] = {
    ...
    MODEL_FRAMEL,  MODEL_FRAMER,  MODEL_PORTS,  MODEL_ARROWS,
    MODEL_ARROWSL, MODEL_ARROWSR, NULL};
...
static void PlayerModel_UpdateGrid(void) {
  ...
  if (s_playermodel.selectedmodel / MAX_MODELSPERPAGE ==
...
static void PlayerModel_UpdateModel(void) {
  ...
  UI_PlayerInfo_SetInfo(&s_playermodel.playerinfo, LEGS_IDLE, TORSO_STAND,
                        viewangles, moveangles, WP_MACHINEGUN, qfalse);
}
...
static void PlayerModel_DrawPlayer(void *self) {
  ...
  if (trap_MemoryRemaining() <= LOW_MEMORY) {
    UI_DrawProportionalString(b->generic.x, b->generic.y + b->height / 2,
                              "LOW MEMORY", UI_LEFT, color_red);
    return;
  }

  UI_DrawPlayer(b->generic.x, b->generic.y, b->width, b->height,
                &s_playermodel.playerinfo, uis.realtime / 2);
}
...
static void PlayerModel_BuildList(void) {
  ...
  for (i = 0; i < numdirs && s_playermodel.nummodels < MAX_PLAYERMODELS;
    ...
    numfiles = trap_FS_GetFileList(va("models/players/%s", dirptr), "tga",
                                   filelist, 2048);
    fileptr = filelist;
    for (j = 0; j < numfiles && s_playermodel.nummodels < MAX_PLAYERMODELS;
         j++, fileptr += filelen + 1) {
...
static void PlayerModel_MenuInit(void) {
  ...
  for (i = 0, k = 0; i < PLAYERGRID_ROWS; i++) {
    ...
    for (j = 0; j < PLAYERGRID_COLS; j++, k++) {
      ...
      s_playermodel.picbuttons[k].generic.flags =
...
void UI_PlayerModelMenu(void) {
  ...
  Menu_SetCursorToItem(
      &s_playermodel.menu,
      &s_playermodel.pics[s_playermodel.selectedmodel % MAX_MODELSPERPAGE]);
}
