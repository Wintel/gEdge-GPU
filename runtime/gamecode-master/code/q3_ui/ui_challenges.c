...
static void UI_Challenges_Init(void) {
  ...
  challenges.notice.generic.flags =
      QMF_CENTER_JUSTIFY | QMF_INACTIVE | QMF_SMALLFONT;
  ...
  challenges.notice2.generic.flags =
      QMF_CENTER_JUSTIFY | QMF_INACTIVE | QMF_SMALLFONT;
  ...
  challenges.headlines[mainSelection].generic.flags =
      QMF_RIGHT_JUSTIFY | QMF_INACTIVE;
  ...
  y = 240 - (int)((((float)challenges.numberOfEntries) / 2.0) *
                  (float)(BIGCHAR_HEIGHT + 2));
  for (i = 0; i < MAX_ENTRIES && i < challenges.numberOfEntries; i++) {
    ...
    strncpy(challenges.entryIntString[i], va("%u", challenges.entryInt[i]),
            MAX_INT_AS_STRING);
    y += BIGCHAR_HEIGHT + 2;
  }
