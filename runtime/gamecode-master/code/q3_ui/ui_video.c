...
static void DriverInfo_MenuDraw(void) {
  ...
  UI_DrawString(320, 80 + 16, uis.glconfig.vendor_string,
                UI_CENTER | UI_SMALLFONT, text_color_normal);
  UI_DrawString(320, 96 + 16, uis.glconfig.version_string,
                UI_CENTER | UI_SMALLFONT, text_color_normal);
  UI_DrawString(320, 112 + 16, uis.glconfig.renderer_string,
                UI_CENTER | UI_SMALLFONT, text_color_normal);
  UI_DrawString(320, 152 + 16,
                va("color(%d-bits) Z(%d-bits) stencil(%d-bits)",
                   uis.glconfig.colorBits, uis.glconfig.depthBits,
  ...
  for (i = 0; i < s_driverinfo.numstrings / 2; i++) {
    UI_DrawString(320 - 4, y, s_driverinfo.strings[i * 2],
                  UI_RIGHT | UI_SMALLFONT, text_color_normal);
    UI_DrawString(320 + 4, y, s_driverinfo.strings[i * 2 + 1],
                  UI_LEFT | UI_SMALLFONT, text_color_normal);
    y += SMALLCHAR_HEIGHT;
  }

  if (s_driverinfo.numstrings & 1)
    UI_DrawString(320, y, s_driverinfo.strings[s_driverinfo.numstrings - 1],
                  UI_CENTER | UI_SMALLFONT, text_color_normal);
}
...
static void UI_DriverInfo_Menu(void) {
  ...
  // NOTE: could have pushed the size of stringbuff, but the list is already out
  // of the screen (no matter what your resolution)
...
static InitialVideoOptions_s s_ivo_templates[] = {
    ...
    {
        4, qtrue, 2, 0, qfalse, qfalse, qfalse, 2, 1, 1, 0, 0,
        qtrue // JDC: this was tq 3
    },
...
static const char *builtinResolutions[] = {
    ...
    "960x720",   "1024x768", "1152x864", "1280x1024", "1600x1200",
    "2048x1536", "856x480",  NULL};

static const char *knownRatios[][2] = {{"1.25:1", "5:4"},   {"1.33:1", "4:3"},
                                       {"1.50:1", "3:2"},   {"1.56:1", "14:9"},
                                       {"1.60:1", "16:10"}, {"1.67:1", "5:3"},
...
static void GraphicsOptions_GetResolutions(void) {
  ...
  if (*resbuf) {
    ...
    unsigned int i = 0;
    while (s &&
           i < sizeof(detectedResolutions) / sizeof(detectedResolutions[0]) -
                   1) {
...
static void GraphicsOptions_CheckConfig(void) {
  ...
  for (i = 0; i < NUM_IVO_TEMPLATES - 1; i++) {
    if (s_ivo_templates[i].driver != s_graphicsoptions.driver.curvalue)
      continue;
    if (GraphicsOptions_FindDetectedResolution(s_ivo_templates[i].mode) !=
    ...
    if (s_ivo_templates[i].aniso != s_graphicsoptions.aniso.curvalue)
      continue;
    //		if ( s_ivo_templates[i].texturebits !=
    //s_graphicsoptions.texturebits.curvalue ) 			continue;
...
static void GraphicsOptions_ApplyChanges(void *unused, int notification) {
  ...
  if (resolutionsDetected) {
    ...
    if (s_graphicsoptions.mode.curvalue == -1 ||
        s_graphicsoptions.mode.curvalue >=
            sizeof(detectedResolutions) / sizeof(detectedResolutions[0]))
      s_graphicsoptions.mode.curvalue = 0;

    mode =
        GraphicsOptions_FindBuiltinResolution(s_graphicsoptions.mode.curvalue);
    if (mode == -1) {
      char w[16], h[16];
      Q_strncpyz(w, detectedResolutions[s_graphicsoptions.mode.curvalue],
                 sizeof(w));
      *strchr(w, 'x') = 0;
      Q_strncpyz(
          h,
          strchr(detectedResolutions[s_graphicsoptions.mode.curvalue], 'x') + 1,
          sizeof(h));
  ...
  if (s_graphicsoptions.aniso.curvalue) {
    trap_Cvar_SetValue("r_ext_max_anisotropy",
                       s_graphicsoptions.aniso.curvalue * 2);
...
static void GraphicsOptions_Event(void *ptr, int event) {
  ...
  case ID_LIST:
    ...
    s_graphicsoptions.mode.curvalue =
        GraphicsOptions_FindDetectedResolution(ivo->mode);
    s_graphicsoptions.ratio.curvalue =
        resToRatio[s_graphicsoptions.mode.curvalue];
...
static void GraphicsOptions_SetMenuItems(void) {
  ...
  if (s_graphicsoptions.mode.curvalue < 0) {
    if (resolutionsDetected) {
      ...
      buf[strlen(buf) + 1] = 0;
      buf[strlen(buf)] = 'x';
      trap_Cvar_VariableStringBuffer("r_customheight", buf + strlen(buf),
  ...
  s_graphicsoptions.allow_extensions.curvalue =
      trap_Cvar_VariableValue("r_allowExtensions");
  ...
  if (trap_Cvar_VariableValue("r_ext_texture_filter_anisotropic")) {
    s_graphicsoptions.aniso.curvalue =
        trap_Cvar_VariableValue("r_ext_max_anisotropy") / 2;
  }
  ...
  s_graphicsoptions.lighting.curvalue =
      trap_Cvar_VariableValue("r_vertexLight") != 0;
  ...
  if (Q_strequal(UI_Cvar_VariableString("r_textureMode"),
...
void GraphicsOptions_MenuInit(void) {
  ...
  static const char *s_graphics_options_names[] = {
      "Very High Quality", "High Quality", "Normal", "Fast",
      "Fastest",           "Custom",       NULL};

  static const char *lighting_names[] = {"Lightmap (Normal)", "Vertex (Low)",
                                         NULL};

  static const char *filter_names[] = {"Bilinear", "Trilinear", NULL};

  static const char *aniso_names[] = {"Off", "2x", "4x", "6x", "8x", NULL};
  ...
  s_graphicsoptions.apply.generic.flags =
      QMF_RIGHT_JUSTIFY | QMF_PULSEIFFOCUS | QMF_HIDDEN | QMF_INACTIVE;
  ...
  Menu_AddItem(&s_graphicsoptions.menu,
               (void *)&s_graphicsoptions.allow_extensions);
