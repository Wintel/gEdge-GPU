  ...
  for (n = 0; n < ui_numArenas; n++) {
    ...
    if (*type) {
      if (strstr(type, "single")) {
        ...
        Info_SetValueForKey(ui_arenaInfos[n], "num",
                            va("%i", singlePlayerNum++));
        continue;
      }
    }
...
Returns the player's best finish on a given level, 0 if the have not played the
level
===============
*/
void UI_GetBestScore(int level, int *score, int *skill) {
  ...
  for (n = 1; n <= 5; n++) {
    trap_Cvar_VariableStringBuffer(va("g_spScores%i", n), scores,
                                   MAX_INFO_VALUE);
...
void UI_SetBestScore(int level, int score) {
  ...
  // get scores
  trap_Cvar_VariableStringBuffer(va("g_spScores%i", skill), scores,
                                 MAX_INFO_VALUE);
