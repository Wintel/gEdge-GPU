...
static char *art_artlist[] = {ART_FRAMEL, ART_FRAMER, ART_BACK0, ART_BACK1,
...
static void PlayerSettings_DrawName2(void *self) {
  ...
  if (focus) {
    ...
    UI_DrawChar(basex + f->field.cursor * SMALLCHAR_WIDTH, y, c, style,
                color_white);
  }
...
static void FirstConnect_StatusBar_Name(void *ptr) {
  UI_DrawString(320, 440, "Your nickname", UI_CENTER | UI_SMALLFONT,
                colorWhite);
}
...
static void FirstConnect_StatusBar_Rate(void *ptr) {
  UI_DrawString(320, 440, "Your connection speed", UI_CENTER | UI_SMALLFONT,
                colorWhite);
}
...
static void FirstConnect_StatusBar_Delag(void *ptr) {
  UI_DrawString(320, 440, "Reliable Instanthit weapons",
...
static void FirstConnect_StatusBar_Download(void *ptr) {
  UI_DrawString(320, 440, "Auto download missing maps and mods",
                UI_CENTER | UI_SMALLFONT, colorWhite);
}
...
static void FirstConnect_Event(void *ptr, int event) {
  ...
  case ID_ALLOWDOWNLOAD:
    trap_Cvar_SetValue("cl_allowDownload",
                       s_firstconnect.allowdownload.curvalue);
...
static void FirstConnect_SetMenuItems(void) {
  int rate;
  // name
  Q_strncpyz(s_firstconnect.name.field.buffer, UI_Cvar_VariableString("name"),
             sizeof(s_firstconnect.name.field.buffer));
  ...
  s_firstconnect.delaghitscan.curvalue =
...
void FirstConnect_MenuInit(void) {
  ...
  s_firstconnect.allowdownload.generic.statusbar =
      FirstConnect_StatusBar_Download;
  ...
  s_firstconnect.info.string =
      "Note: All settings can be changed later in SETUP";
