...
static char *cinematics[] = {"idlogo", "intro", "tier1", "tier2", "tier3",
                             "tier4",  "tier5", "tier6", "tier7", "end"};
...
void UI_CinematicsMenu_f(void) {
  ...
  n = atoi(UI_Argv(1));
  UI_CinematicsMenu();
  Menu_SetCursorToItem(&cinematicsMenuInfo.menu,
                       cinematicsMenuInfo.menu.items[n + 3]);
}
