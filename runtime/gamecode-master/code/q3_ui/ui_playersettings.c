...
static void PlayerSettings_DrawHandicap(void *self) {
  ...
  UI_DrawProportionalString(item->generic.x, item->generic.y, "Handicap", style,
                            color);
  UI_DrawProportionalString(item->generic.x + 64, item->generic.y + PROP_HEIGHT,
                            handicap_items[item->curvalue], style, color);
}
...
static void PlayerSettings_DrawEffects(void *self) {
  ...
  UI_DrawProportionalString(item->generic.x, item->generic.y, "Effects", style,
                            color);

  UI_DrawHandlePic(item->generic.x + 64, item->generic.y + PROP_HEIGHT + 8, 128,
                   8, s_playersettings.fxBasePic);
  UI_DrawHandlePic(item->generic.x + 64 + item->curvalue * 16 + 8,
                   item->generic.y + PROP_HEIGHT + 6, 16, 12,
...
static void PlayerSettings_DrawEffects2(void *self) {
  ...
  UI_DrawHandlePic(item->generic.x + 64, item->generic.y + 8, 128, 8,
                   s_playersettings.fxBasePic);
  UI_DrawHandlePic(item->generic.x + 64 + item->curvalue * 16 + 8,
                   item->generic.y + 6, 16, 12,
                   s_playersettings.fxPic[item->curvalue]);
}
...
static void PlayerSettings_DrawPlayer(void *self) {
  ...
  if (!strequals(buf, s_playersettings.playerModel)) {
    ...
    viewangles[PITCH] = 0;
    viewangles[ROLL] = 0;
    UI_PlayerInfo_SetInfo(&s_playersettings.playerinfo, LEGS_IDLE, TORSO_STAND,
                          viewangles, vec3_origin, WP_MACHINEGUN, qfalse);
  }
...
static void PlayerSettings_StatusBar(void *ptr) {
  UI_DrawString(320, 400, "Lower handicap makes you weaker",
                UI_CENTER | UI_SMALLFONT, colorWhite);
  UI_DrawString(320, 420, "giving you more challenge", UI_CENTER | UI_SMALLFONT,
                colorWhite);
}
