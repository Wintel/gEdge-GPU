...
vec4_t listbar_color        = {1.00f, 0.43f, 0.00f, 0.30f};	// transluscent
orange vec4_t text_color_status    = {1.00f, 1.00f, 1.00f, 1.00f};	//
bright white
...
=================
*/
static void Text_Init(menutext_s *t) { t->generic.flags |= QMF_INACTIVE; }
...
static sfxHandle_t Slider_Key(menuslider_s *s, int key) {
  ...
  case K_MOUSE1:
    ...
    oldvalue = s->curvalue;
    s->curvalue = (x / (float)(SLIDER_RANGE * SMALLCHAR_WIDTH)) *
...
static void Slider_Draw(menuslider_s *s) {
  ...
  UI_DrawHandlePic((int)(x + 2 * SMALLCHAR_WIDTH +
...
static void Slider_Draw(menuslider_s *s) {
  ...
  UI_DrawChar((int)(x + 2 * SMALLCHAR_WIDTH +
                    (SLIDER_RANGE - 1) * SMALLCHAR_WIDTH * s->range),
...
static void SpinControl_Draw(menulist_s *s) {
  ...
  UI_DrawString(x - SMALLCHAR_WIDTH, y, s->generic.name, style | UI_RIGHT,
                color);
  UI_DrawString(x + SMALLCHAR_WIDTH, y, s->itemnames[s->curvalue],
                style | UI_LEFT, color);
}
...
void Menu_Cache(void) {
  ...
  uis.menuBackNoLogoShader =
      trap_R_RegisterShaderNoMip("menubacknologo_blueish");
