...
static void UI_DisplayDownloadInfo(const char *downloadName) {
  ...
  leftWidth = width =
      UI_ProportionalStringWidth(dlText) * UI_ProportionalSizeScale(style);
  width = UI_ProportionalStringWidth(etaText) * UI_ProportionalSizeScale(style);
  if (width > leftWidth)
    leftWidth = width;
  width =
      UI_ProportionalStringWidth(xferText) * UI_ProportionalSizeScale(style);
  if (width > leftWidth)
    leftWidth = width;
  leftWidth += 16;
  ...
  if (downloadCount < 4096 || !downloadTime) {
    ...
    // bk010108
    // float elapsedTime = (float)(uis.realtime - downloadTime); // current -
    // start (msecs) elapsedTime = elapsedTime * 0.001f; // in seconds if (
    ...
    if (downloadSize && xferRate) {
      ...
      UI_PrintTime(dlTimeBuf, sizeof dlTimeBuf,
                   n); // bk010104
                       //(n - (((downloadCount/1024) * n) /
                       //(downloadSize/1024))) * 1000);

      UI_DrawProportionalString(leftWidth, 160, dlTimeBuf, style, color_white);
      UI_DrawProportionalString(
          leftWidth, 192, va("(%s of %s copied)", dlSizeBuf, totalSizeBuf),
          style, color_white);
    } else {
      UI_DrawProportionalString(leftWidth, 160, "estimating", style,
                                color_white);
      if (downloadSize) {
        UI_DrawProportionalString(
            leftWidth, 192, va("(%s of %s copied)", dlSizeBuf, totalSizeBuf),
        ...
        UI_DrawProportionalString(leftWidth, 192, va("(%s copied)", dlSizeBuf),
                                  style, color_white);
      }
    }

    if (xferRate) {
      UI_DrawProportionalString(leftWidth, 224, va("%s/Sec", xferRateBuf),
                                style, color_white);
    }
  }
}
...
void UI_DrawConnectScreen(qboolean overlay) {
  ...
  if (trap_GetConfigString(CS_SERVERINFO, info, sizeof(info))) {
    UI_DrawProportionalString(
        320, 16, va("Loading %s", Info_ValueForKey(info, "mapname")),
  ...
  UI_DrawProportionalString(320, 64, va("Connecting to %s", cstate.servername),
                            ...
                            menu_text_color);
  // UI_DrawProportionalString( 320, 96, "Press Esc to abort",
  ...
  if (cstate.connState < CA_CONNECTED) {
    UI_DrawProportionalString_AutoWrapped(
        320, 192, 630, 20, cstate.messageString,
        UI_CENTER | UI_SMALLFONT | UI_DROPSHADOW, menu_text_color);
  }
  ...
  UI_DrawProportionalString(
      320, 128, s, UI_CENTER | UI_SMALLFONT | UI_DROPSHADOW, color_white);
