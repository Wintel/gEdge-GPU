...
static const char *master_items[] = {"Local+Internet", "Local+Internet",
                                     "Local",          "Internet",
...
static char *gamenames[] = {"DM ",          // deathmatch
                            "1v1",          // tournament
                            "SP ",          // single player
...
static char *netnames[] = {"???", "IP4", "IP6", NULL};
...
typedef struct {
  char buff[MAX_LISTBOXWIDTH +
            64]; //	+60 gives room for color codes... Sago: I need four more
...
static void ArenaServers_UpdatePicture(void) {
  ...
  if (!g_arenaservers.list.numitems) {
    ...
    servernodeptr =
        g_arenaservers.table[g_arenaservers.list.curvalue].servernode;
...
/*
=================
        Q_strcpyColor - This function will return the real length of the string
if numChars len of character data is desired. It looks for color codes and adds
2 to the length for each combo found. This is used to make color strings show up
correctly in column formatted environments. Otherwise, the columns will be off 2
* num of color codes.
=================
*/
...
static void ArenaServers_UpdateMenu(void) {
  ...
  if (g_arenaservers.numqueriedservers > 0) {
    ...
    if (g_arenaservers.refreshservers &&
      ...
      Com_sprintf(g_arenaservers.status.string, MAX_STATUSLENGTH,
                  "%d of %d Arena Servers.", g_arenaservers.currentping,
                  g_arenaservers.numqueriedservers);
      g_arenaservers.statusbar.string = "Press SPACE to stop";
      qsort(g_arenaservers.serverlist, *g_arenaservers.numservers,
            sizeof(servernode_t), ArenaServers_Compare);
    } else {
    ...
    if (g_arenaservers.refreshservers) {
      ...
      g_arenaservers.showfull.generic.flags |= QMF_GRAYED;
      g_arenaservers.list.generic.flags |= QMF_GRAYED;
      // g_arenaservers.refresh.generic.flags	|= QMF_GRAYED; // Allow to press
      // Refresh once again to stop
      g_arenaservers.go.generic.flags |= QMF_GRAYED;
    } else {
      if (g_arenaservers.numqueriedservers < 0) {
  ...
  for (i = 0, j = 0; i < count; i++, servernodeptr++) {
    ...
    // If "Show only humans" and "Hide empty server" are enabled hide servers
    // that only have bots
    ...
    /*
    Com_sprintf( buff, MAX_LISTBOXWIDTH, "%-20.20s %-12.12s %2d/%2d %-8.8s %3s
    %s%3d ", servernodeptr->hostname, servernodeptr->mapname,
    servernodeptr->numclients, servernodeptr->maxclients,
    servernodeptr->gamename, netnames[servernodeptr->nettype], pingColor,
    servernodeptr->pingtime ); //, servernodeptr->bPB ? "Yes" : "No"
     */
    b = buff;
    *b++ = '^';
    *b++ = '7';
    ...
    *b++ = ' ';
    ...
    if (g_onlyhumans == 0)
      Com_sprintf(b, 8, "%2d/%2d ", servernodeptr->numclients,
                  servernodeptr->maxclients);
    else
      Com_sprintf(b, 8, "%2d/%2d ", servernodeptr->humanclients,
                  servernodeptr->maxclients);
    b += 6;
...
static void ArenaServers_Remove(void) {
  ...
  for (i = 0; i < g_arenaservers.numfavoriteaddresses; i++) {
    if (Q_strequal(g_arenaservers.favoriteaddresses[i],
      ...
      memset(&g_arenaservers
                  .favoriteaddresses[g_arenaservers.numfavoriteaddresses],
             0, MAX_ADDRESSLENGTH);
      break;
    }
  }
  ...
  for (i = 0; i < g_numfavoriteservers; i++) {
    if (&g_favoriteserverlist[i] == servernodeptr) {
      ...
      memset(&g_favoriteserverlist[g_numfavoriteservers], 0,
             sizeof(servernode_t));
      break;
    }
  }
...
static void ArenaServers_Insert(char *adrstr, char *info, int pingtime) {
  ...
  if (*g_arenaservers.numservers >= g_arenaservers.maxservers) {
    // list full;
    servernodeptr =
        g_arenaservers.serverlist + (*g_arenaservers.numservers) - 1;
  } else {
    // next slot
  ...
  Q_strncpyz(servernodeptr->hostname, Info_ValueForKey(info, "hostname"),
             MAX_HOSTNAMELENGTH);
  Q_CleanStrWithColor(servernodeptr->hostname);
  Q_strupr(servernodeptr->hostname);

  Q_strncpyz(servernodeptr->mapname, Info_ValueForKey(info, "mapname"),
             MAX_MAPNAMELENGTH);
  ...
  if (*s) {
    ...
    Q_strncpyz(servernodeptr->gamename, gamenames[i],
               sizeof(servernodeptr->gamename));
  }
  servernodeptr->local =
      (g_servertype == UIAS_LOCAL || g_servertype == UIAS_ALL_LOCAL);
}
...
void ArenaServers_InsertFavorites(void) {
  ...
  for (i = 0; i < g_arenaservers.numfavoriteaddresses; i++) {
    ...
    for (j = 0; j < g_numfavoriteservers; j++)
      if (Q_strequal(g_arenaservers.favoriteaddresses[i],
                     g_favoriteserverlist[j].adrstr))
        break;

    if (j >= g_numfavoriteservers) {
      // not in list, add it
      ArenaServers_Insert(g_arenaservers.favoriteaddresses[i], info,
                          ArenaServers_MaxPing());
    }
  }
}
...
void ArenaServers_LoadFavorites(void) {
  ...
  // copy the old
  memcpy(templist, g_favoriteserverlist,
         sizeof(servernode_t) * MAX_FAVORITESERVERS);
  ...
  for (i = 0; i < MAX_FAVORITESERVERS; i++) {
    trap_Cvar_VariableStringBuffer(va("server%d", i + 1), adrstr,
                                   MAX_ADDRESSLENGTH);
    if (!adrstr[0])
      continue;
    ...
    if (j < numtempitems) {
      ...
      memcpy(&g_favoriteserverlist[g_numfavoriteservers], &templist[j],
             sizeof(servernode_t));
      found = qtrue;
    } else {
      // add new server
      Q_strncpyz(g_favoriteserverlist[g_numfavoriteservers].adrstr, adrstr,
                 MAX_ADDRESSLENGTH);
      g_favoriteserverlist[g_numfavoriteservers].pingtime =
          ArenaServers_MaxPing();
    }
...
static void ArenaServers_DoRefresh(void) {
  ...
  for (i = 0; i < MAX_PINGREQUESTS &&
    ...
    if (g_servertype == UIAS_FAVORITES) {
      strcpy(adrstr,
             g_arenaservers.favoriteaddresses[g_arenaservers.currentping]);
    } else {
      trap_LAN_GetServerAddressString(ArenaServers_SourceForLAN(),
                                      g_arenaservers.currentping, adrstr,
                                      MAX_ADDRESSLENGTH);
    }
...
*/
static void ArenaServers_StartRefresh(void) {
  memset(g_arenaservers.serverlist, 0,
         g_arenaservers.maxservers * sizeof(table_t));
...
static void ArenaServers_StartRefreshNoClearList(void) {
  ...
  if (g_servertype == UIAS_LOCAL || g_servertype == UIAS_ALL_LOCAL) {
    if (g_servertype == UIAS_ALL_LOCAL)
      g_arenaservers.refreshtime =
          uis.realtime + 2000; // Less ping time for local servers
  ...
  if ((g_servertype >= UIAS_GLOBAL1 && g_servertype <= UIAS_GLOBAL5) ||
      g_servertype == UIAS_ALL_GLOBAL) {
    int masterserver =
        (g_servertype == UIAS_ALL_GLOBAL ? 0 : g_servertype - UIAS_GLOBAL1);
...
static void ArenaServers_Event(void *ptr, int event) {
  ...
  case ID_ONLY_HUMANS:
    trap_Cvar_SetValue("ui_browserOnlyHumans",
                       g_arenaservers.onlyhumans.curvalue);
    g_onlyhumans = g_arenaservers.onlyhumans.curvalue;
    ArenaServers_UpdateMenu();
    break;

  case ID_HIDE_PRIVATE:
    // trap_Cvar_SetValue( "ui_browserHidePrivate",
    // g_arenaservers.hideprivate.curvalue );
...
static void ArenaServers_MenuInit(void) {
  ...
  g_servertype =
      Com_Clamp(UIAS_ALL_LOCAL, UIAS_FAVORITES, ui_browserMaster.integer);
  if (g_servertype == UIAS_ALL_GLOBAL)
    g_servertype = UIAS_ALL_LOCAL;
  ...
  g_arenaservers.onlyhumans.curvalue =
      Com_Clamp(0, 1, ui_browserOnlyHumans.integer);
  g_onlyhumans = ui_browserOnlyHumans.integer;

  g_arenaservers.hideprivate.curvalue =
      1;             // Com_Clamp( 0, 1, ui_browserOnlyHumans.integer );
