...
#define QMF_NODEFAULTINIT                                                      \
  (unsigned int)0x00008000 // skip default initialization
...
// #define TRAP_FREEFORALL			(trap_Cvar_VariableValue(
// "g_gametype" ) == GT_LMS)
...
extern void UI_ConfirmMenu_Style(const char *question, int style,
                                 void (*draw)(void),
                                 void (*action)(qboolean result));
...
extern void UI_DrawProportionalString(int x, int y, const char *str, int style,
                                      vec4_t color);
extern void UI_DrawProportionalString_AutoWrapped(int x, int ystart, int xmax,
                                                  int ystep, const char *str,
...
qboolean trap_VerifyCDKey(const char *key,
                          const char *chksum); // bk001208 - RC4
