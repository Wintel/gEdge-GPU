...
static char *votemenu_custom_artlist[] = {VOTEMENU_BACK0, VOTEMENU_BACK1,
...
static void VoteMenu_custom_Event(void *ptr, int event) {
  ...
  default:
    ...
    trap_Cmd_ExecuteText(
        EXEC_APPEND,
        va("callvote custom %s",
           s_votemenu_custom.bEntry[s_votemenu_custom.selection - ID_CUSTOM0]
               .string));
    UI_PopMenu();
    UI_PopMenu();
    break;
...
void UI_VoteCustomMenuInternal(void) {
  ...
  trap_Cvar_VariableStringBuffer("cg_vote_custom_commands", custominfo,
                                 sizeof(custominfo));
  ...
  for (i = 0; i < CUSTOM_MENU_MAX_ENTRIES; i++) {
    ...
    Q_strncpyz(s_votemenu_custom.text[i], token,
               sizeof(s_votemenu_custom.text[0]));
    setCustomMenutext(&s_votemenu_custom.bEntry[i], y, ID_CUSTOM0 + i,
                      s_votemenu_custom.text[i]);
