...
static void Main_MenuDraw(void) {
  ...
  adjust = 0; // JDC: Kenneth asked me to stop this 1.0 * sin(
              // (float)uis.realtime / 1000 );
  ...
  if (strlen(s_errorMessage.errorMessage)) {
    UI_DrawProportionalString_AutoWrapped(
        320, 192, 600, 20, s_errorMessage.errorMessage,
  ...
  UI_DrawString(320, 400, "OpenArena(c) 2005-2018 OpenArena Team",
                UI_CENTER | UI_SMALLFONT, color);
  UI_DrawString(
      320, 414,
      "OpenArena comes with ABSOLUTELY NO WARRANTY; this is free software",
      UI_CENTER | UI_SMALLFONT, color);
  UI_DrawString(
      320, 428,
      "and you are welcome to redistribute it under certain conditions;",
      UI_CENTER | UI_SMALLFONT, color);
  UI_DrawString(320, 444, "read COPYING for details.", UI_CENTER | UI_SMALLFONT,
                color);
  ...
  if ((int)trap_Cvar_VariableValue("protocol") != OA_STD_PROTOCOL) {
    UI_DrawString(
        0, 480 - 14,
        va("^7Protocol: %i", (int)trap_Cvar_VariableValue("protocol")),
        UI_SMALLFONT, color);
  }
}
...
void UI_MainMenu(void) {
  ...
  trap_Cvar_SetValue(
      "handicap",
      100); // Reset handicap during server change, it must be ser per game
  ...
  QMF_CENTER_JUSTIFY|QMF_PULSEIFFOCUS; s_main.cinematics.generic.x
  = 320; s_main.cinematics.generic.y				= y;
