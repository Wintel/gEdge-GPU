...
static char *specifyleague_artlist[] = {
    SPECIFYLEAGUE_FRAMEL,   SPECIFYLEAGUE_FRAMER,
    SPECIFYLEAGUE_ARROWS0,  SPECIFYLEAGUE_UP,
    SPECIFYLEAGUE_DOWN,     SPECIFYLEAGUE_BACK0,
    SPECIFYLEAGUE_BACK1,    GLOBALRANKINGS_LOGO,
    GLOBALRANKINGS_LETTERS, NULL};
...
static void SpecifyLeague_GetList() {
  ...
  Q_strncpyz(playername, s_specifyleague.rankname.field.buffer,
             sizeof(playername));
...
static void SpecifyLeague_Event(void *ptr, int event) {
  ...
  case ID_SPECIFYLEAGUENAME:
    if ((event == QM_LOSTFOCUS) &&
        (Q_strncmp(playername, s_specifyleague.rankname.field.buffer,
  ...
  case ID_SPECIFYLEAGUEBACK:
    if (event == QM_ACTIVATED) {
      trap_Cvar_Set("sv_leagueName",
                    league_table[s_specifyleague.list.curvalue].leaguename);
      UI_PopMenu();
    }
    break;
  }
...
void SpecifyLeague_MenuInit(void) {
  ...
  s_specifyleague.up.generic.flags =
  ...
  s_specifyleague.down.generic.flags =
      QMF_LEFT_JUSTIFY | QMF_PULSEIFFOCUS | QMF_MOUSEONLY;
