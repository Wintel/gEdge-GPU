...
static void VoteMapMenu_Event(void *ptr, int event) {
  ...
  default:
    ...
    if (!s_votemenu_map.selected ||
        mappage.mapname[s_votemenu_map.selected - ID_MAPNAME0][0] == 0)
      return;
...
static void UI_VoteMapMenu_UpEvent(void *ptr, int event) {
  ...
  trap_Cmd_ExecuteText(EXEC_APPEND,
                       va("getmappage %d", mappage.pagenumber - 1));
}
...
void UI_VoteMapMenuInternal(void) {
  ...
  for (i = 1; i < SIZE_OF_LIST; i++) {
    ...
    setMapMenutext(&s_votemenu_map.maps[i], y, ID_MAPNAME0 + i,
                   mappage.mapname[i]);
  }
...
void UI_VoteMapMenu(void) {
  ...
  // We need to initialize the list or it will be impossible to click on the
  // items
  for (i = 0; i < SIZE_OF_LIST; i++) {
    Q_strncpyz(mappage.mapname[i], "----", 5);
  }
  trap_Cmd_ExecuteText(EXEC_APPEND, "getmappage 0");
  trap_Cvar_Set("cl_paused",
                "0"); // We cannot send server commands while paused!
