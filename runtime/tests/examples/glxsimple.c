...
static int sngBuf[] = {GLX_RGBA, GLX_RED_SIZE,  1, GLX_GREEN_SIZE,
                       1,        GLX_BLUE_SIZE, 1, GLX_DEPTH_SIZE,
                       12,       None};

static int dblBuf[] = {GLX_RGBA, GLX_RED_SIZE,     1,   GLX_GREEN_SIZE,
                       1,        GLX_BLUE_SIZE,    1,   GLX_DEPTH_SIZE,
...
void main(int argc, char **argv) {
  ...
  */
  ...
  if (!(cx = glXCreateContext(dpy, vi, None, /* no sharing of display lists */
  ...
  */
  ...
  XSetStandardProperties(dpy, win, "glxsimple", "glxsimple", None, argv, argc,
                         NULL);
  ...
  while (1) {
    do {
      ...
      case ConfigureNotify:
        ...
        glScissor(0, 0, event.xconfigure.width, event.xconfigure.height);
        /* fall through */
      case Expose:
        needRedraw = True;
        break;
    ...
    /* Once the outstanding events have been read, we update the OpenGL
       modelview matrix if the recalcModelView flag is set.  This ensures that
       any rotations from button presses will be applied.  Any time the
       modelview matrix changes, the scene should also be redrawn so needRedraw
       is set.

       After updating the modelview matrix, determine whether a redraw is needed
       by checking the needRedraw flag.  If it is, call the redraw routine to
       update the window's scene.
    */
