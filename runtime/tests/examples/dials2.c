...
void drawCircle(int x, int y, int r, int dir) {
  float angle;
...
void displayDials(void) {
  int i;

  for (i = 0; i < NUM_DIALS; i++) {
    glColor3f(0, 1, 0);
    drawCircle(60 + ((i + 1) % 2) * 100, 60 + (i / 2) * 100, 40,
               dials[NUM_DIALS - 1 - i] - 90);
  }
}
...
void dodial(int dial, int value) {
